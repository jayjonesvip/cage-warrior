...
const dailyNews=globalThis.CAGE_DAILY_NEWS.create({element:$('#dailyCageNews'),tips:STRINGS.ticker,canShow:()=>!document.hidden&&!postFightPresentationBusy(),onDismiss:()=>requestAnimationFrame(showPostFightFollowup),client:SHARED_FEED,storage:localStorage,onSyncError:error=>{console.warn('Fight result sync pending:',error);toast('FIGHT RESULT SYNC PENDING · YOUR RESULT IS KEPT ON THIS DEVICE','#ffcf78')}});
  ...
  const ICON_ASSET_VERSION = '2.7.201';
...
$,logic:LOGIC,getState:()=>state,getRank:()=>rankName(),getChampionship:()=>sharedChampionship,setChampionship:setSharedChampionship,sharedFeed:SHARED_FEED,sharedUi:SHARED_UI,trackEvent,tap:()=>sfx.tap(),onEntered:()=>setTimeout(()=>{showCareerRestoreNotice();showRecoveryReport();if(!offerRookieShowcase())offerFirstContractOpponent();dailyNews.visit(currentScreen==='home')},180),onChampionshipChange:renderOpponents
  ...
  function updateUI(){
    ...
    const progressText=$('#progressText');progressText.textContent=state.attributePoints?`${state.attributePoints} POINT${state.attributePoints===1?'':'S'}`:'NO POINTS';$('#cageStatus').textContent=cageStatus();$('#heroLevel').textContent=`LVL ${state.level}`;
...
function sharedProfilePayload(){return {combatStats:Object.fromEntries(['power','speed','chin','cardio'].map(key=>[key,effectiveStat(key)])),defendingPlan:LOGIC.normalizeFightPlan(state.fightPlanPreference),fightSkinAura:Math.floor(effectiveAura()),city:state.fighterCity,archetype:state.fighterStyle,fighterAvatar:state.fighterAvatar,level:state.level,wins:state.wins,losses:state.losses,draws:state.draws||0,attributeTotal:Object.values(state.stats).reduce((sum,value)=>sum+Math.max(0,Number(value)||0),0),rankingHistory:state.rankingHistory}}
  ...
  function fightWinRewardPreview(opponent){
    ...
    const points=LOGIC.victoryAttributePointReward(state.level,opponent.tier,{...ranks,opponentWinsToday:opponentWinsToday(opponent)}),playerRating=state.stats.power+state.stats.speed+state.stats.chin+state.stats.cardio,opponentRating=opponent.power+opponent.speed+opponent.chin+opponent.cardio,upset=opponentRating>=playerRating+4,titleWon=opponent.globalChampionship&&!opponent.championDefense,winsToday=opponentWinsToday(opponent),xpTier=fightRewardTier(opponent);
  ...
  function renderOpponents(){
    refreshOpponents();
    const fightsLeft=sessionsLeft('fight',DAILY_FIGHT_LIMIT),ranking=currentFighterList(),showcase=opponents.filter(item=>item.rookieShowcase),contract=opponents.filter(item=>item.firstContract),ranked=opponents.filter(item=>item.network&&!combatStatsPending(item)),rankedCount=ranked.length+(ranking.profile&&ranking.position?1:0),circuit=opponents.filter(item=>!item.network&&!item.rookieShowcase&&!item.firstContract),circuitRematches=circuit.filter(item=>(item.winsVsPlayer||0)>0).length;
    $('#fightResetClock').hidden=fightsLeft>0;
    setLimitBadge('#fightLimitText',`${fightsLeft} ${fightsLeft===1?'FIGHT':'FIGHTS'} LEFT`);
    $('#rosterSummary').textContent=`${contract.length?`${contract.length} CONTRACT · `:''}${circuit.length} CIRCUIT · ${rankedCount} FIGHTERS`;
    const showcaseRows=showcase.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join(''),contractRows=contract.length?`<div class="page-subhead fight-ranking-group circuit-group"><b>YOUR FIRST CONTRACT</b><span>VASO WIN UNLOCKED THIS FIGHT</span></div>${contract.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'',circuitSummary=circuitRematches?`${circuit.length-circuitRematches} FRESH · ${circuitRematches} REMATCH · PRO RECORD`:`${circuit.length} FRESH MATCHUPS · FULL XP · PRO RECORD`,circuitRows=circuit.length?`<div class="page-subhead fight-ranking-group circuit-group"><b>ON-LEVEL CAGE CIRCUIT</b><span>START HERE · ${circuitSummary}</span></div>${circuit.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'';
    const entries=ranked.map(opponent=>({id:opponent.sourceProfileId,level:opponent.tier,attributeTotal:opponent.attributeTotal,order:opponent.fighterListOrder,champion:opponent.isChampion===true,render:levelPosition=>renderFightLadderRow(opponent,fightsLeft,levelPosition)}));
    ...
    const wrapGroup=html=>html?`<div class="fight-ranking-section">${html}</div>`:'',levelGroups=[];
    for(const entry of visibleEntries){let group=levelGroups.at(-1);if(!group||group.level!==entry.level){group={level:entry.level,rows:[]};levelGroups.push(group)}group.rows.push(entry.html)}
    const rankedRows=levelGroups.map(group=>wrapGroup(`<div class="page-subhead fight-ranking-group ranked-group"><b>LEVEL ${group.level}</b><span>WIN % · WINS · H2H</span></div>${group.rows.join('')}`)).join('')+loadMore;
