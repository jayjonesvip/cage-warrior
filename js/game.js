(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.142';
  ...
  function networkOpponentFromProfile(profile,tier){
    ...
    return {key:`network-${id}`,network:true,seeded:profile.seeded===true,sourceProfileId:id,networkHandle:handle,networkCity:String(profile.city||''),networkPortrait:avatar.asset,fighterAvatar:avatar.id,aura:clamp(Math.floor(Number(profile.fight_skin_aura))||0,0,100),name,tag:arch.tag,archetype:arch.id,tendency:arch.tendency,scout:arch.scout,tier,min:tier,max:99,...ratings,fans:Math.round(22*Math.pow(1.48,tier-1)),color:fighterAccent(profile.city),look:seed%10,wins:Math.max(0,Math.floor(Number(profile.wins))||0),losses:Math.max(0,Math.floor(Number(profile.losses))||0),winsVsPlayer:0,lossesToPlayer:0,meetings:0,rematchAccepted:false,recordInitialized:true,createdAt:Date.now()};
  }
  ...
  function renderTapeFightSkin(selector,aura){
    ...
    container.innerHTML=`<div class="tape-fight-skin-items">${[['gloves','Gloves'],['wraps','Hand wraps'],['mouthguard','Mouthguard'],['shorts','Shorts']].map(([key,label])=>`<img src="${skin[key+'Asset']}?v=${ICON_ASSET_VERSION}" alt="${skin.colorName} ${label.toLowerCase()}">`).join('')}</div><small>${skin.label} <span>FIGHT KIT</span></small>`;
  }
  function fillTape(f){
    renderTapeFightSkin('#tapePlayerSkin',f.playerAura);
    renderTapeFightSkin('#tapeOpponentSkin',f.o.network?f.o.aura:0);
