(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.71';
  ...
  let gearFilter = (()=>{try{const saved=sessionStorage.getItem(GEAR_FILTER_KEY);return ['combat','bling','all'].includes(saved)?saved:'all'}catch{return 'all'}})();
  let fight = null;
  let fightTimers = [];
  ...
  function renderGear(){
    ...
    const visibleOrder=gearFilter==='combat'?['Fight Gear']:gearFilter==='bling'?['Bling']:order,sections=visibleOrder.map(cat=>{const items=owned.filter(g=>g.category===cat);if(!items.length)return '';const categoryTotal=gearItems.filter(g=>g.category===cat).length,collectionStatus=`${items.length} / ${categoryTotal} COLLECTIBLES`,fightGear=cat==='Fight Gear',loadoutStatus=fightGear?`${state.equippedGear.length}/${loadoutLimit} EQUIPPED${loadoutProgress}`:`${state.equippedPerks.length}/${perkLimit} PERKS EQUIPPED${perkProgress}`,status=`<span>${collectionStatus}</span><small>${loadoutStatus}</small>`,activeItems=fightGear?[]:items.filter(item=>state.equippedPerks.includes(item.id)),followerBonus=activeItems.reduce((sum,item)=>sum+(Number(item.prestige)||0),0),recoveryBonus=activeItems.reduce((best,item)=>Math.max(best,Number(item.healthRecoverySpeed)||0),0),activeEffects=[followerBonus?`+${followerBonus}% FIGHT FOLLOWERS`:'',recoveryBonus?`HEALTH EVERY ${Math.round((HEALTH_RECOVERY_INTERVAL-recoveryBonus)/1000)} SEC`:''].filter(Boolean),note=fightGear?'Fight Gear uses separate combat slots. Duplicates do not stack.':activeEffects.length?`ACTIVE · ${activeEffects.join(' · ')}`:'Equip a collectible below to activate its career perk.';return `<section class="shop-section" aria-labelledby="gear-${cat.replace(/[^a-z]+/gi,'-').toLowerCase()}"><div class="shop-head"><div><b id="gear-${cat.replace(/[^a-z]+/gi,'-').toLowerCase()}">${cat}</b><small>${subtitles[cat]}</small></div><span class="shop-status">${status}</span></div><div class="loadout-note${fightGear?'':' perk-category-note'}">${note}</div><div class="gear-grid">${items.map(g=>collectibleCardHtml(g)).join('')}</div></section>`}).join('');
  ...
  function setGearFilter(filter){if(!['combat','bling','all'].includes(filter)||gearFilter===filter)return;gearFilter=filter;try{sessionStorage.setItem(GEAR_FILTER_KEY,filter)}catch{/* session-only fallback remains in memory */}renderGear();sfx.tap()}
