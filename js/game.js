(async () => {
  ...
  const ICON_ASSET_VERSION = '2.5.222';
  ...
  const defaultState = {
    version:24,name:'ROOKIE',nameLocked:false,firstFightDone:false,debutFightId:'',setupSeed:Math.floor(Math.random()*0xffffffff),setupDeckCursor:0,setupRecommendedAvatar:'',setupRecommendedStyle:'',setupPath:'recommended',setupViewingAll:false,cash:0,careerEarnings:0,fans:0,level:1,xp:0,wins:0,losses:0,winStreak:0,bestStreak:0,
  ...
  function trackEvent(eventName,parameters={}){
    ...
    if(eventName==='fight_completed'&&parameters.first_fight===undefined)parameters=Object.assign({first_fight:fight?.firstFight===true},parameters);
  ...
  const ROOKIE_SHOWCASE={
    ...
    boutLabel:'LOCAL DEBUT · CARD FILLER',cardLabel:'DEBUT FIGHT',actionLabel:'SET FIGHT PLAN',
    scout:'A local-card replacement booked to test a new prospect. Trust your natural style and make the debut count.'
  };
  ...
  function normalizeOpponentArchetype(o){
    if(!o)return;if(o.rookieShowcase){const debut=LOGIC.debutOpponent({baseStats:state.fighterBaseStats||state.stats,playerArchetype:state.fighterStyle,seed:state.setupSeed,key:ROOKIE_SHOWCASE.key,name:ROOKIE_SHOWCASE.name});Object.assign(o,debut,{country:ROOKIE_SHOWCASE.country,tag:'LOCAL CARD FILLER',scout:ROOKIE_SHOWCASE.scout,portraitAsset:`assets/opponents/vaso-jose.png?v=${ICON_ASSET_VERSION}`});return}const id=normalizeMajorArchetype(o.archetype)||normalizeMajorArchetype(o.tendency)||'striker',arch=opponentArchetypes.find(a=>a.id===id)||opponentArchetypes[0];o.archetype=arch.id;o.tendency=arch.id;o.tag=arch.tag;o.scout=arch.scout;
  }
  ...
  function rookieShowcaseOpponent(){
    return {...LOGIC.debutOpponent({baseStats:state.fighterBaseStats||state.stats,playerArchetype:state.fighterStyle,seed:state.setupSeed,key:ROOKIE_SHOWCASE.key,name:ROOKIE_SHOWCASE.name}),country:ROOKIE_SHOWCASE.country,tag:'LOCAL CARD FILLER',scout:ROOKIE_SHOWCASE.scout,portraitAsset:`assets/opponents/vaso-jose.png?v=${ICON_ASSET_VERSION}`,reward:125,fans:22,color:'#4aa7d8',look:7,wins:0,losses:8,winsVsPlayer:0,lossesToPlayer:0,meetings:0,rematchAccepted:false,recordInitialized:true,rookieShowcase:true,createdAt:Date.now()};
  }
  function ensureRookieShowcaseOpponent(){
    if(!state.nameLocked||state.firstFightDone)return state.roster.find(o=>o.key===ROOKIE_SHOWCASE.key)||null;
    const existing=state.roster.find(o=>o.key===ROOKIE_SHOWCASE.key);if(existing){normalizeOpponentArchetype(existing);return existing}
  ...
  function ensureRoster(){
    ...
    const active=state.roster.filter(o=>o.tier===state.level&&!o.network&&(o.lossesToPlayer||0)===0).length;for(let i=active;i<4;i++)state.roster.push(generateOpponent(state.level));ensureRookieShowcaseOpponent();
  ...
  const FIGHT_ROUNDS=fightRule('fightStructure.scheduledRounds',3),ENERGY_SEGMENT=fightRule('energyEconomy.energySegmentSize',25),FIGHT_ENERGY_COST=fightRule('energyEconomy.fightEnergyCost',25),MINIMUM_ACTION_ENERGY_EXCLUSIVE=fightRule('energyEconomy.minimumEnergyToStartConsumingActionExclusive',0),REST_SECONDS_PER_ENERGY_SEGMENT=fightRule('energyEconomy.restSecondsPerMissingEnergySegment',3),ENERGY_DRINK_GAIN=fightRule('energyEconomy.energyDrinkEnergyRestored',25),ENERGY_DRINK_DURATION_SECONDS=fightRule('energyEconomy.energyDrinkDurationSeconds',2),DAILY_DROP_ENERGY=fightRule('energyEconomy.dailyDropEnergyRestored',25),DAILY_FIGHT_LIMIT=fightRule('fightStructure.dailyFightLimit',10),MINIMUM_FIGHT_HEALTH=fightRule('fightStructure.minimumHealthForMedicalClearance',20),DAILY_TRAINING_LIMIT=fightRule('dailyDevelopmentLimits.ordinaryAttributeTrainingSessionLimit',3),DAILY_SPARRING_LIMIT=fightRule('dailyDevelopmentLimits.sparringSessionLimit',1);
  ...
  hustleDefs.forEach(action=>action.cost=ODD_JOB_ENERGY_COST);publicityDefs.forEach(action=>action.cost=PUBLICITY_ENERGY_COST);const restDefinition=recoveryDefs.find(action=>action.freeRest),energyDrinkDefinition=recoveryDefs.find(action=>action.id==='surgecore-energy-drink');if(restDefinition){restDefinition.energy=100;restDefinition.meterSeconds=REST_SECONDS_PER_ENERGY_SEGMENT}if(energyDrinkDefinition){energyDrinkDefinition.energy=ENERGY_DRINK_GAIN;energyDrinkDefinition.meterSeconds=ENERGY_DRINK_DURATION_SECONDS}
  ...
  function normalizeState(parsed){
      ...
      s.fighterStyle=normalizeMajorArchetype(s.fighterStyle);
       s.rosterSerial=Math.max(0,Number(s.rosterSerial)||0);s.fighterStyle=['striker','grappler'].includes(s.fighterStyle)?s.fighterStyle:'';s.fighterCity=['phoenix','los-angeles','chicago','new-york','miami','houston','cleveland','seattle','new-orleans','hawaii','boston','atlanta','san-francisco','denver','tampa-bay','philadelphia','san-antonio','las-vegas','portland','baltimore'].includes(s.fighterCity)?s.fighterCity:'';s.fighterAvatar=fighterAvatars.some(a=>a.id===s.fighterAvatar)?s.fighterAvatar:'';const avatar=fighterAvatars.find(a=>a.id===s.fighterAvatar);s.fighterBaseStats=avatar&&validFighterAllocation(s.fighterBaseStats)?Object.assign({},s.fighterBaseStats):avatar?Object.assign({},avatar.stats):null;s.setupSeed=(Number(source.setupSeed)>>>0)||s.setupSeed;s.setupDeckCursor=clamp(Math.floor(Number(source.setupDeckCursor))||0,0,1000000);s.setupRecommendedAvatar=fighterAvatars.some(a=>a.id===source.setupRecommendedAvatar)?source.setupRecommendedAvatar:'';s.setupRecommendedStyle=['striker','grappler'].includes(source.setupRecommendedStyle)?source.setupRecommendedStyle:'';s.setupPath=['recommended','all_fighters'].includes(source.setupPath)?source.setupPath:'recommended';delete s.milestones;s.roster=Array.isArray(s.roster)?s.roster.filter(o=>!o.championship&&!o.globalChampionship):[];s.equippedGear=Array.isArray(s.equippedGear)?s.equippedGear.filter(id=>s.gear.includes(id)):[];s.leagueInitialized=source.leagueInitialized===true;
       const coreReady=!!(s.fighterStyle&&s.fighterCity&&s.fighterAvatar&&validFighterAllocation(s.fighterBaseStats)),legacyHandle=normalizeIdentityName(source.socialHandle),legacyName=normalizeIdentityName(source.name);s.nameLocked=coreReady&&(source.nameLocked===undefined?true:source.nameLocked===true);s.name=s.nameLocked?(legacyHandle||legacyName||'cagefighter'):'ROOKIE';s.firstFightDone=source.firstFightDone===true||s.wins+s.losses>0;s.debutFightId=s.nameLocked&&!s.firstFightDone?(typeof source.debutFightId==='string'&&source.debutFightId?source.debutFightId:ROOKIE_SHOWCASE.key):'';delete s.rookieShowcasePending;delete s.socialHandle;
       s.firstFightDone=LOGIC.firstFightCompleted(s,source);if(s.firstFightDone)s.debutFightId='';s.setupViewingAll=source.setupViewingAll===true;s.version=24;
      return s;
  }
  function loadState(){
  ...
  function recommendationDeck(){return LOGIC.deterministicDeck(fighterAvatars.map(avatar=>avatar.id),state.setupSeed)}
  function currentRecommendedAvatar(){const explicit=fighterAvatars.find(avatar=>avatar.id===state.setupRecommendedAvatar);if(explicit)return explicit;const deck=recommendationDeck();return fighterAvatars.find(avatar=>avatar.id===deck[state.setupDeckCursor%Math.max(1,deck.length)])||fighterAvatars[0]}
  function currentRecommendedStyle(){const avatar=currentRecommendedAvatar();return state.setupRecommendedStyle||LOGIC.suggestedFighterArchetype(avatar?.stats)}
  ...
  function awardDebutCommonCollectible(opponent){
    const pityBefore=state.gearWinsSinceDrop,random=seededRandom(hashSeed(`${state.gearSeed}|${opponent.key}|debut-common-v1`)),pool=eligibleGearAtLevel(state.level,'COMMON');if(!pool.length)return null;
    const item=pool[Math.floor(random()*pool.length)],isNew=!state.gear.includes(item.id);if(isNew)state.gear.push(item.id);state.gearCounts[item.id]=gearCount(item.id)+1;state.gearWinsSinceDrop=pityBefore;ensureLoadout();return {item,rarity:'COMMON',count:state.gearCounts[item.id],isNew,guaranteed:true,reason:'DEBUT COMMON'};
  }
  ...
  function recoveryFee(treatment){if(treatment?.freeRest)return 0;if(Number.isFinite(Number(treatment?.flatFee)))return Math.max(0,Math.floor(Number(treatment.flatFee)));return Math.max(0,Number(treatment?.health)||0)*state.level*RECOVERY_DOLLARS_PER_HEALTH_POINT_PER_FIGHTER_LEVEL}
  function recoveryTreatmentForCurrentEnergy(treatment){if(!treatment?.freeRest)return treatment;const missingEnergy=clamp(state.maxEnergy-state.energy,0,state.maxEnergy),missingSegments=Math.ceil(missingEnergy/ENERGY_SEGMENT);return Object.assign({},treatment,{energy:missingEnergy,meterSeconds:missingSegments*REST_SECONDS_PER_ENERGY_SEGMENT})}
  ...
  function flashTrainingStats(skills,gain=1,perfect=false){
    const punch=`+${formatGain(gain)}`;
    skills.forEach(skill=>{const card=$(`#${skill}Stat`)?.closest('.hud-attribute');if(!card)return;card.dataset.statPunch=punch;card.classList.remove('stat-rewarded','perfect-session');void card.offsetWidth;card.classList.toggle('perfect-session',perfect);card.classList.add('stat-rewarded');clearTimeout(card._rewardTimer);card._rewardTimer=setTimeout(()=>{card.classList.remove('stat-rewarded','perfect-session');delete card.dataset.statPunch},1900)});
  }
  ...
  function renderCareer(){
    ...
    const homeRanking=ready?currentRanking():null;$('#homeRankText').textContent=homeRanking?.position?`YOUR RANK #${homeRanking.position}`:'VIEW RANKINGS';const sponsor=state.activeEndorsement?endorsementDefs.find(d=>d.id===state.activeEndorsement.id):null,sponsorBadge=$('#heroSponsor'),hero=$('.hero'),sponsorWallpaper=$('#heroSponsorWallpaper');sponsorBadge.hidden=!sponsor;sponsorBadge.innerHTML=sponsor?`${gameIcon(sponsor.id,sponsor.icon)}<span class="hero-sponsor-copy"><small>SPONSORED BY</small><b>${sponsor.brand}</b><em>${state.activeEndorsement.fightsLeft} FIGHTS LEFT</em></span>`:'';hero.classList.toggle('sponsored',!!sponsor);sponsorWallpaper.hidden=!sponsor;sponsorWallpaper.style.backgroundImage=sponsor?`url("assets/icons/${sponsor.id}.png?v=${ICON_ASSET_VERSION}")`:'';
    homeDropOffer.hidden=!ready||!state.firstFightDone;$('#debutCareerGuide').hidden=!ready||state.firstFightDone;$('#careerGuide').hidden=ready&&!state.firstFightDone;
    ...
    if(avatar)$('#heroFighterArt').src=avatar.asset;renderRecommendedFighter();
  }
  function renderRecommendedFighter(){
    const recommended=currentRecommendedAvatar(),style=currentRecommendedStyle(),all=state.setupViewingAll;$('#recommendedFighterStep').hidden=all;$('#allFightersStep').hidden=!all;if(!recommended)return;
    const index=fighterAvatars.indexOf(recommended)+1;$('#recommendedFighterImage').src=recommended.asset;$('#recommendedFighterNumber').textContent=`FIGHTER ${String(index).padStart(2,'0')}`;$('#recommendedFighterStats').innerHTML=[['PWR','power'],['SPD','speed'],['CHN','chin'],['CAR','cardio']].map(([label,key])=>`<span>${label} ${formatStat(recommended.stats[key])}</span>`).join('');$('#recommendedFighterCard').setAttribute('aria-label',`Use Fighter ${index}, Power ${recommended.stats.power}, Speed ${recommended.stats.speed}, Chin ${recommended.stats.chin}, Cardio ${recommended.stats.cardio}, ${style}`);$('#recommendedStyleChoices').innerHTML=fighterStyles.map(entry=>`<button class="build-choice ${entry.id===style?'selected':''}" type="button" data-recommended-style="${entry.id}" aria-pressed="${entry.id===style}"><b>${entry.name}</b><small>${entry.text}</small></button>`).join('');
  }
  ...
  function selectRecommendedStyle(id){if(state.fighterAvatar||!fighterStyles.some(style=>style.id===id))return;state.setupRecommendedStyle=id;trackEvent('career_setup_step',{step:'archetype_preview',selection:id,setup_path:state.setupPath});sfx.tap();updateUI()}
  function stageRecommendedAvatar(id){if(state.fighterAvatar||!state.fighterCity)return;const avatar=fighterAvatars.find(item=>item.id===id);if(!avatar||!validFighterAllocation(avatar.stats))return;state.setupRecommendedAvatar=id;state.setupRecommendedStyle=LOGIC.suggestedFighterArchetype(avatar.stats);state.setupPath='all_fighters';state.setupViewingAll=false;trackEvent('career_setup_step',{step:'all_fighters_selection',selection:id,setup_path:'all_fighters'});sfx.tap();updateUI();requestAnimationFrame(()=>$('#recommendedFighterCard').focus())}
  function shuffleRecommendedFighter(){if(state.fighterAvatar||!state.fighterCity)return;state.setupDeckCursor++;state.setupRecommendedAvatar='';state.setupRecommendedStyle='';state.setupPath='recommended';trackEvent('career_setup_step',{step:'fighter_shuffle',setup_path:'recommended'});sfx.tap();updateUI()}
  function showAllFighters(){if(state.fighterAvatar||!state.fighterCity)return;state.setupPath='all_fighters';state.setupViewingAll=true;trackEvent('career_setup_step',{step:'see_all_fighters',setup_path:'all_fighters'});sfx.tap();updateUI();requestAnimationFrame(()=>$('#fighterChoices .avatar-card')?.focus())}
  function showRecommendedFighter(){if(state.fighterAvatar||!state.fighterCity)return;state.setupViewingAll=false;sfx.tap();updateUI();requestAnimationFrame(()=>$('#recommendedFighterCard').focus())}
  function acceptRecommendedFighter(){
    if(state.fighterAvatar||!state.fighterCity)return;const avatar=currentRecommendedAvatar(),style=currentRecommendedStyle();if(!avatar||!validFighterAllocation(avatar.stats)||!fighterStyles.some(entry=>entry.id===style))return;
    const keys=['power','speed','chin','cardio'],earned=Object.fromEntries(keys.map(key=>[key,Math.max(0,(Number(state.stats[key])||5)-5)]));state.fighterAvatar=avatar.id;state.fighterBaseStats=Object.assign({},avatar.stats);state.stats=Object.fromEntries(keys.map(key=>[key,avatar.stats[key]+earned[key]]));state.fighterStyle=style;identitySuggestion=randomIdentitySuggestion();trackEvent('career_setup_step',{step:'fighter_and_archetype',selection:avatar.id,archetype:style,setup_path:state.setupPath});sfx.win();confettiBurst();toast(`FIGHTER ${String(fighterAvatars.indexOf(avatar)+1).padStart(2,'0')} · ${style.toUpperCase()} LOCKED IN`,'#76dcff');updateUI();requestAnimationFrame(()=>$('#lockFighterNameBtn').focus())
  }
  ...
  async function lockFighterIdentity(){
    ...
    try{
      const candidates=manualRequested?[requested]:identityClaimCandidates(requested),profile=await SHARED_FEED.claimIdentity(Object.assign(sharedProfilePayload(),{candidates}));if(!profile?.id||!normalizeIdentityName(profile.handle))throw new Error('Unique fighter name was not returned.');
       state.name=normalizeIdentityName(profile.handle);state.nameLocked=true;state.firstFightDone=false;state.debutFightId=ROOKIE_SHOWCASE.key;state.socialProfileId=profile.id;identitySuggestion='';identityManualMode=false;identityManualValue='';ensureRookieShowcaseOpponent();trackEvent('career_setup_step',{step:'name',setup_path:state.setupPath});trackEvent('career_started',{archetype:state.fighterStyle,city:state.fighterCity,avatar:state.fighterAvatar,setup_path:state.setupPath});createSocialAccount();saveState();sfx.win();confettiBurst();toast(requested===state.name?`@${state.name} IS READY`:`@${requested} WAS TAKEN · @${state.name} IS YOURS`,'#76dcff');updateUI();connectSharedSocial(true);
  ...
  function renderTrain(){
    ...
    const recoveryOptions=recoveryDefs.map((definition,i)=>{const a=recoveryTreatmentForCurrentEnergy(definition),treatmentFee=recoveryFee(a),quote=LOGIC.recoveryQuote(state,a,treatmentFee);return {a,i,treatmentFee,quote}}),energyOptions=recoveryOptions.filter(({a})=>a.energy>0),treatmentOptions=recoveryOptions.filter(({a})=>a.health>0),restAvailable=state.energy<state.maxEnergy,recoveryActionHtml=({a,i,treatmentFee,quote})=>{const gains=[a.energy?`+${a.energy}% ENERGY`:'',a.health?`+${a.health} HEALTH`:''].filter(Boolean).map(gain=>`<span class="recovery-gain">${gain}</span>`).join(''),seconds=Math.max(0,Number(a.meterSeconds)||2),status=a.freeRest?`FREE &middot; ${seconds} SECONDS`:quote.reason==='cash'?`NEED $${treatmentFee}`:quote.reason==='full'?(a.energy?'ENERGY FULL':'HEALTH FULL'):`-$${treatmentFee} &middot; ${seconds} SECONDS`;return `<button class="action${a.energy?' rest-action':''}" data-recovery="${i}" ${quote.ok?'':'disabled'}><div class="ico">${gameIcon(a.id,a.icon)}</div><div><h3>${a.title}</h3><p>${a.text}</p></div><div class="cost"><b class="recovery-gains">${gains}</b><small>${status}</small></div></button>`};
    $('#trainRestCard').hidden=!restAvailable;$('#restActions').innerHTML=restAvailable?energyOptions.map(recoveryActionHtml).join(''):'';
  ...
  function renderOpponents(){
    ...
    const renderCard=o=>{
      ...
      const fighterClass=o.rookieShowcase?ROOKIE_SHOWCASE.cardLabel:o.network?'RANKED FIGHTER':'UNRANKED FIGHTER',frontClass=o.rookieShowcase?'SHOWCASE':o.network?'RANKED':'UNRANKED';
      ...
      const xpTier=opponentXpTier(o),xpSupport=xpTier.tier==='repeat'?'50% XP · SAME-DAY RUNBACK':xpTier.tier==='exhausted'?'NO XP · PAYOUT EXHAUSTED':xpTier.tier==='lower_level'?'NO XP · LOWER LEVEL':'FULL XP',matchupAction=o.rookieShowcase?'VIEW SHOWCASE':hasHistory?'RUN IT BACK':'VIEW MATCHUP',restriction=dailyExhausted?'NEW FIGHTS AT LOCAL MIDNIGHT':status==='locked'?`LEVEL TOO HIGH · REQUIRES LEVEL ${o.tier}`:tauntable?'REMATCH REQUIRES A TAUNT':o.rookieShowcase?ROOKIE_SHOWCASE.headline:xpSupport,rematchLabel=o.rookieShowcase?'FIRST FIGHT':hasHistory?(dailyExhausted?'REMATCH PAUSED · DAILY LIMIT':xpTier.tier==='exhausted'||xpTier.tier==='lower_level'?'REMATCH · NO XP':tauntable?'REMATCH LOCKED':'REMATCH AVAILABLE'):'FIRST MATCHUP';
      ...
      return `<article class="opponent ${status} ${o.network?'network':''} ${o.portraitAsset?'portrait':''} ${rivalFight?'rematch':''} ${dailyExhausted?'daily-exhausted':''}"${o.network?` style="--hometown-accent:${fighterAccent(o.networkCity)};${fighterPortraitStyle(o.networkHandle)}"`:''} data-card-flip="true" data-card-name="${safeName}" tabindex="0" aria-label="${safeName} fighter card${dailyExhausted?', daily fight limit reached':rivalFight?', rival fight available':tauntable?', taunt available':''}. Tap for details." aria-pressed="false">
        <div class="opponent-flip">
          <div class="opponent-side opponent-front" aria-hidden="false">
            ...
            <div class="opp-face">
              <img class="opp-sprite" src="${silhouette}" alt="${safeName} ${o.network||o.portraitAsset?'portrait':'silhouette'}" loading="lazy">
  ...
  function handleTrain(i){
    ...
    trackEvent('training_completed',{training_id:a.id,training_type:'gym',coach_used:coach,perfect_session:perfect,repeated_session:repeatCount>0,injury_id:'none',energy_spent:energySpent,cash_spent:quote.cashCost,stat_gain:gain,risk_bonus:0,sessions_used:quote.sessions});updateUI();flashTrainingStats([a.stat],gain,perfect);
  }
  ...
  function handleRecovery(i){
    ensureDailyCounters();const treatment=recoveryTreatmentForCurrentEnergy(recoveryDefs[i]);if(!treatment||activeRecoverySession)return;const fee=recoveryFee(treatment),quote=LOGIC.recoveryQuote(state,treatment,fee);
    if(quote.reason==='cash'){toast(`${treatment.energy?'Energy drink':'Recovery treatment'} costs $${fee}. Pick up an odd job first.`,'#ffcc75');return}
    if(quote.reason==='full'){toast(treatment.energy?'Your Energy battery is already full.':'Your health is already full.','#78dfff');return}
    state.cash-=quote.cashCost;const restored=treatment.energy?null:LOGIC.applyRecovery(state,treatment);initAudio();activeRecoverySession={treatment,cashCost:quote.cashCost,restored};openRecoverySession();
  }
  function openRecoverySession(){
    const session=activeRecoverySession;if(!session)return;const {treatment,cashCost}=session,modal=$('#recoverySessionModal'),meter=$('#recoverySessionMeter'),energyRecovery=treatment.energy>0,seconds=Math.max(0,Number(treatment.meterSeconds)||2),duration=seconds*1000;$('#recoverySessionKicker').textContent=treatment.freeRest?'RESTING':energyRecovery?'RECHARGING':'RECOVERY IN PROGRESS';$('#recoverySessionTitle').textContent=treatment.title.toUpperCase();$('#recoverySessionStatus').textContent=`${treatment.freeRest?'Rest':energyRecovery?'Energy boost':'Treatment'} in progress · ${seconds} seconds`;$('#recoverySessionResult').innerHTML=modalMeterSummary(treatment.freeRest?'RECOVERY':'COST',treatment.freeRest?`+${treatment.energy}% ENERGY`:`-$${cashCost}`,treatment.freeRest?'FULL BATTERY':energyRecovery?'ENERGY DRINK IN PROGRESS':'TREATMENT IN PROGRESS');$('#collectRecoveryResult').hidden=true;$('#exitRecoveryResult').hidden=true;$('#recoverySessionActions').classList.add('single-action');meter.classList.remove('working');meter.style.setProperty('--modal-meter-duration',`${seconds}s`);meter.setAttribute('aria-valuenow','0');modal.classList.add('open');modal.setAttribute('aria-hidden','false');updateUI();void meter.offsetWidth;requestAnimationFrame(()=>meter.classList.add('working'));recoverySessionTimer=setTimeout(resolveRecoverySession,duration);
  }
  function resolveRecoverySession(){
    recoverySessionTimer=null;if(!activeRecoverySession)return;const {treatment,cashCost}=activeRecoverySession,energyRecovery=treatment.energy>0,restored=activeRecoverySession.restored||LOGIC.applyRecovery(state,treatment);activeRecoverySession.restored=restored;trackEvent('recovery_completed',{treatment_id:treatment.id,cash_spent:cashCost,energy_restored:Math.round(restored.energy),health_restored:Math.round(restored.health)});const rewards=[restored.energy?`ENERGY +${formatGain(restored.energy)}%`:'',restored.health?`HEALTH +${formatGain(restored.health)}`:''].filter(Boolean),rewardText=rewards.join(' · ');$('#recoverySessionKicker').textContent=treatment.freeRest?'REST COMPLETE':energyRecovery?'RECHARGE COMPLETE':'RECOVERY COMPLETE';$('#recoverySessionTitle').textContent=`${treatment.title.toUpperCase()} FINISHED.`;$('#recoverySessionStatus').textContent=treatment.freeRest?'Battery fully charged.':energyRecovery?'One segment restored.':'Treatment collected.';$('#recoverySessionMeter').setAttribute('aria-valuenow','100');$('#recoverySessionResult').innerHTML=modalMeterSummary('REWARD',rewardText,energyRecovery?'BATTERY RECHARGED':'HEALTH RESTORED');const collect=$('#collectRecoveryResult'),exit=$('#exitRecoveryResult'),actions=$('#recoverySessionActions');collect.textContent='BACK TO TRAINING';collect.hidden=treatment.freeRest;exit.hidden=!treatment.freeRest;actions.classList.add('single-action');sfx.train();flashRecoveryResources(restored);toast(`RECOVERY REWARD · ${rewardText}`,'#78dcff','','',3200);updateUI();requestAnimationFrame(()=>(exit.hidden?collect:exit).focus());
  }
  ...
  function simulateRound(sim,round,planId,opening=null){
      const debutRule=LOGIC.debutFightRule({firstFightDone:state.firstFightDone,opponent:sim.o,fighterStyle:state.fighterStyle,pityCount:state.gearWinsSinceDrop});
      ...
      for(let ex=1;ex<=exchanges&&!stopped;ex++){
        ...
        if(landed){
          ...
          if(side==='opp'&&debutRule.active)damage=Math.min(damage,Math.max(0,sim.playerCondition-55));aStats.damage+=damage;if(side==='player')sim.oppCondition=clamp(sim.oppCondition-damage,0,100);else{sim.playerCondition=clamp(sim.playerCondition-damage,0,100);fightInjury=maybeRollLiveFightInjury(sim,true)}
        }
      ...
      if(debutRule.active&&round===1&&!sim.winner){sim.winner='player';sim.method=debutRule.method;sim.finishRound=debutRule.finishRound;sim.finishClock=debutRule.finishClock;sim.oppCondition=0;stopped=true;if(sim.method==='SUBMISSION'){rs.player.attempted++;rs.player.landed++;rs.player.takedowns++;rs.player.control+=34;sim.timeline.push({type:'action',round,clock:'0:51',text:`${P.name} changes levels, finishes the takedown, and settles into top control.`,className:'you',playerCondition:sim.playerCondition,oppCondition:18,landed:true,side:'player'});sim.timeline.push({type:'submission',round,clock:debutRule.finishClock,text:`TAP! ${P.name} locks the submission and ${O.name} has to concede!`,className:'ko',playerCondition:sim.playerCondition,oppCondition:0,big:true})}else{rs.player.attempted++;rs.player.landed++;rs.player.sig++;rs.player.damage+=18;rs.player.kd++;sim.timeline.push({type:'ko',round,clock:debutRule.finishClock,text:`IT'S OVER! ${P.name} pours on the pressure and the referee waves off the debut!`,className:'ko',playerCondition:sim.playerCondition,oppCondition:0,big:true})}}
      scoreRoundState(rs);
  ...
  function fillTape(f){
    ...
    $('#tapePurse').textContent='$'+fmt(payoutForOpponent(f.o));$('#tapeBoutLabel').textContent=titleBout?'WORLD CHAMPIONSHIP BOUT · 3 ROUNDS':rookieShowcase?ROOKIE_SHOWCASE.boutLabel:f.o.network?'RANKED FIGHT · 3 ROUNDS':'UNRANKED FIGHT · 3 ROUNDS';const xpTier=opponentXpTier(f.o),xpStatus=$('#tapeXpStatus'),needsEnergy=fightsLeft>0&&!hasActionEnergy(),fightButton=$('#tapeFightBtn'),recoveryButton=$('#tapeRecoveryBtn');xpStatus.textContent=xpTier.tapeLabel;xpStatus.className=`tape-xp-status ${xpTier.tier}`;fightButton.hidden=needsEnergy;fightButton.disabled=!cleared;recoveryButton.hidden=!needsEnergy;$('#tapeClearance').classList.toggle('recovery-needed',needsEnergy);$('#tapeClearance').textContent=!fightsLeft?'DAILY FIGHT LIMIT REACHED · NEW FIGHTS AT LOCAL MIDNIGHT':state.health<MINIMUM_FIGHT_HEALTH?`MEDICAL CLEARANCE REQUIRES ${MINIMUM_FIGHT_HEALTH} HEALTH`:needsEnergy?'0% ENERGY · VISIT THE RECOVERY ROOM TO REST BEFORE FIGHTING':startingCondition<100?`LIMITED CLEARANCE · ${startingCondition}% STARTING CONDITION`:`CLEARED · FIGHT USES UP TO ${FIGHT_ENERGY_COST}% ENERGY`;
  ...
  function openDebutFight(){
    if(!state.nameLocked||state.firstFightDone)return false;const opponent=ensureRookieShowcaseOpponent();if(!opponent||!opponentAvailable(opponent))return false;
    state.opponentFilter='recommended';saveState();navTo('fight');requestAnimationFrame(()=>document.querySelector(`[data-card-name="${ROOKIE_SHOWCASE.name}"]`)?.scrollIntoView({behavior:globalThis.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches?'auto':'smooth',block:'center'}));return true;
  }
  ...
  function visitRecoveryRoom(){closeFightPreview(true);navTo('train');requestAnimationFrame(()=>$(state.energy<state.maxEnergy?'#trainRestCard':'#recoveryRoomCard').scrollIntoView({behavior:'smooth',block:'start'}));toast('REST & RECHARGE · FREE FULL ENERGY','#78dcff')}
  ...
  function renderResultBonuses(notes=[]){
    const summary=$('#resultBonuses');if(!summary)return;summary.hidden=!notes.length;summary.innerHTML=notes.map(note=>{if(note.kind==='sponsor')return `<div class="result-bonus-row sponsor-bonus"><span>${escapeHtml(note.brand)} BONUS · +$${fmt(note.cash)} · +${fmt(note.followers)} FOLLOWERS</span>${note.contractDone?'<b class="contract-complete">CONTRACT COMPLETE</b>':''}</div>`;return `<div class="result-bonus-row${note.kind==='streak'?' win-streak-bonus':''}">${note.kind==='streak'?'🔥 ':''}${escapeHtml(note.text)}</div>`}).join('');
  }
  ...
  function finishFightSimulation(){
    ...
    const debutRule=LOGIC.debutFightRule({firstFightDone:state.firstFightDone,opponent:fight.o,fighterStyle:state.fighterStyle,pityCount:state.gearWinsSinceDrop}),debutCompleted=debutRule.active;fight.firstFight=debutCompleted;if(debutCompleted){if(!fight.forfeited){fight.winner='player';fight.method=debutRule.method;fight.finishRound=debutRule.finishRound;fight.finishClock=debutRule.finishClock}state.firstFightDone=true;state.debutFightId=''}
    ...
    if(win){
      ...
      if(debutCompleted)gearDrop=awardDebutCommonCollectible(o);else if(dropEligible)gearDrop=awardDeterministicGearDrop({opponent:o,upset,rivalry,titleWon,ko:fight.method.includes('KO'),guaranteed:firstCareerWin});ceoBonus=awardCeoPerformanceBonus({upset,ko:fight.method.includes('KO'),titleWon});cash+=ceoBonus;
    ...
    if(xp)gainXp(xp);xpResult.modifiers.forEach(text=>lootNotes.push({kind:'milestone',text}));if(win&&xpTier.hypeChange<0)lootNotes.push({kind:'milestone',text:`STALE MATCHUP · ${xpTier.hypeChange} HYPE · NO COLLECTIBLE DROP`});
    ...
    if(state.activeEndorsement&&!fight.forfeited){
      ...
      if(deal){
        ...
        lootNotes.push({kind:'sponsor',brand:deal.brand,cash:deal.perFight,followers:sponsorFollowers,contractDone});
  ...
  document.addEventListener('click',e=>{
    ...
    const recommendedStyle=e.target.closest('[data-recommended-style]');if(recommendedStyle){selectRecommendedStyle(recommendedStyle.dataset.recommendedStyle);return}
    const city=e.target.closest('[data-city]');if(city){chooseCity(city.dataset.city);return}
    const avatar=e.target.closest('[data-avatar]');if(avatar){stageRecommendedAvatar(avatar.dataset.avatar);return}
  ...
  $('#speedBtn').addEventListener('click',toggleFightSpeed);$('#detailsToggle').addEventListener('click',()=>{const details=$('#resultDetails'),open=!details.classList.contains('open');details.classList.toggle('open',open);$('#detailsToggle').textContent=open?'HIDE SCORECARD':'VIEW SCORECARD'});
  ...
  $('#collectRecoveryResult').addEventListener('click',closeRecoverySession);$('#exitRecoveryResult').addEventListener('click',closeRecoverySession);$('#recoverySessionModal').addEventListener('click',e=>{if(e.target===$('#recoverySessionModal'))closeRecoverySession()});$('#recoverySessionModal').addEventListener('keydown',e=>{if(e.key==='Escape')closeRecoverySession()});
  ...
  $('#recommendedFighterCard').addEventListener('click',acceptRecommendedFighter);$('#useRecommendedFighterBtn').addEventListener('click',acceptRecommendedFighter);$('#shuffleRecommendedFighterBtn').addEventListener('click',shuffleRecommendedFighter);$('#seeAllFightersBtn').addEventListener('click',showAllFighters);$('#backToRecommendedBtn').addEventListener('click',showRecommendedFighter);$('#debutFightBtn').addEventListener('click',openDebutFight);
