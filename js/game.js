(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.2';
  ...
  function refreshOpponents(){
    ...
    const showcase=state.roster.filter(item=>item.rookieShowcase&&state.rookieShowcasePending),fallback=state.roster.filter(item=>!item.network&&!item.rookieShowcase&&item.tier===state.level&&(item.lossesToPlayer||0)===0).sort(fighterLevelOrder);
    opponents=[...showcase,...(ranked.length?ranked:fallback.map((opponent,index)=>Object.assign({},opponent,{worldRank:index+1,circuitFallback:true})))];
  }
