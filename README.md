...
- **Home** — the fighter profile and portrait first, followed by identity, record, world rank, XP progress, Victory Pack progress, sponsor progress, and a dismissible CEO modal summarizing the past 24 hours in broad terms with a pro tip. It waits until other popups are closed. Dismissal hides the note for 24 hours across reloads; its next appearance fetches a fresh roundup.
...
The shared Supabase-backed championship remains server-authoritative and non-fatal when offline. World Rank weights are 25% career record, 45% quality of wins, 25% recent form, and 5% base attributes. A synced, visible undisputed champion locks rank 1; an interim or unsynced champion does not. Ties use raw win rate, career fight count, then immutable fighter ID. Hidden fighters retain their numerical positions. See `docs/ranking-scorer.md` for formulas, snapshot migration, fixtures, and debug tools. Live scores exclude perks, Aura, followers, and finish method. Each full-width row shows rank, portrait, name, record, level, win percentage, exact win XP, and the Attribute Points available for that matchup. Tap any available row to open the opponent profile modal: avatar, Fight Skin, stats, agent advice, rewards, and clearance. Go Back returns without booking. Start Fight books the bout and opens the existing promo poster with a single Set Fight Plan action. Once booked, leaving uses the existing forfeit rules.

Wins preserve the ranking performance already earned during the current winning streak. Beating a weaker opponent can leave the score unchanged instead of erasing an upset gain through lower averages or aging history. Stronger wins can still raise the score, and other fighters can still pass you. A loss or draw ends that protection. Existing streaks recover the best score reconstructable from their saved 30 bouts; new wins retain that protection across history rollover and reloads. Apply `supabase/migrations/20260908140000_protect_ranking_score_on_wins.sql` after the v2 scorer migration and before publishing this update.
