(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.38';
  ...
  function fillTape(f){
    ...
    const playerAccent=fighterAccent(state.fighterCity),opponentAccent=f.o.network?fighterAccent(f.o.networkCity):f.o.color||DEFAULT_FIGHTER_ACCENT,poster=$('#tapeStage .matchup-poster'),playerCard=$('#tapeStage .player-card'),opponentCard=$('#tapeStage .opponent-card');poster.style.setProperty('--player-accent',playerAccent);poster.style.setProperty('--opponent-accent',opponentAccent);playerCard.style.setProperty('--fighter-accent',playerAccent);opponentCard.style.setProperty('--fighter-accent',opponentAccent);applyPortraitStyle(playerCard,state.name);applyPortraitStyle(opponentCard,f.o.networkHandle||f.o.name);$('#tapePlayerCity').textContent=fighterCityCode(state.fighterCity);const tapeCountry=$('#tapeOppCity');if(f.o.network)tapeCountry.textContent=fighterCityCode(f.o.networkCity);else tapeCountry.innerHTML=opponentCountryBadge(f.o.country);
