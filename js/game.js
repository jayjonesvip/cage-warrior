(async () => {
  ...
  const ICON_ASSET_VERSION = '2.5.201';
  ...
  function updateUI(){
    ...
    $('#dailyBtn').disabled=false;$('#dailyBtn').textContent='CLAIM DAILY DROP';
  ...
  function renderTrain(){
    ensureDailyCounters();
    const left=sessionsLeft('train',DAILY_TRAINING_LIMIT),sparringLeft=sessionsLeft('sparring',DAILY_SPARRING_LIMIT),coach=state.trainerOn,fee=coachFee(),injury=currentTrainingInjury();
  ...
  function collectibleCardHtml(item,{dropStatus=''}={}){
    const equipped=state.equippedGear.includes(item.id),rarity=item.rarity||'COMMON',action=dropStatus?`<div class="gear-status">${escapeHtml(dropStatus)}</div>`:item.category==='Fight Gear'?`<button class="equip-btn" data-equip="${item.id}">${equipped?'UNEQUIP':'EQUIP'}</button>`:'<div class="gear-status">PERK ACTIVE</div>';
    return `<div class="gear collectible-card owned rarity-card-${rarity.toLowerCase()} ${equipped?'equipped':''} ${dropStatus?'drop-claim-card':''}" data-collectible-flip data-collectible-id="${item.id}" tabindex="0" aria-label="${escapeHtml(item.name)} collectible. Tap for details." aria-pressed="false"><div class="collectible-flip"><div class="collectible-side collectible-front" aria-hidden="false"><div class="gear-top"><span class="rarity-tag">${rarity}</span><span class="gear-count">×${gearCount(item.id)}</span></div><div class="gear-hero"><span class="gear-flair"></span><span class="equip-burst"></span><div class="gear-icon">${gameIcon(item.iconName||item.id,item.icon,item.assetExt)}</div></div><div class="gear-copy"><h3>${escapeHtml(item.name)}</h3><span class="collectible-flip-hint">TAP FOR DETAILS</span></div><div class="gear-footer"><span class="level-tag">MIN LVL ${item.minLevel||1}</span>${action}</div></div>${collectibleBackHtml(item,rarity)}</div></div>`;
  }
  function renderGear(){
