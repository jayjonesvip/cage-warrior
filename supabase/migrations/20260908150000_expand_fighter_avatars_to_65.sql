-- Add portraits 51-65 without changing existing identities or career rules.
-- Apply before publishing the avatar picker update. Safe to re-run.
begin;
...
check (fighter_avatar is null or fighter_avatar ~ '^fighter-(0[1-9]|[1-5][0-9]|6[0-5])$');
alter table public.cage_seed_fighters drop constraint if exists cage_seed_fighters_avatar;
alter table public.cage_seed_fighters add constraint cage_seed_fighters_avatar
check (fighter_avatar ~ '^fighter-(0[1-9]|[1-5][0-9]|6[0-5])$');
...
begin
  ...
  if p_fighter_avatar is null or p_fighter_avatar !~ '^fighter-(0[1-9]|[1-5][0-9]|6[0-5])$' then
...
'Public ID of the fighter portrait selected during career setup (fighter-01 through fighter-65).';
commit;
