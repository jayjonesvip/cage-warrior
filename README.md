...
CageReporter prefixes the existing result headline with one of five rotating win messages or five rotating loss messages about fighting hurt when the initiating fighter booked the bout Battered or Banged Up (below the Bruised health threshold, currently 70%). The condition is captured alongside the starting fight condition, so damage taken during that fight does not trigger the note. It accompanies wins, losses, streaks, and lower-tier backlash coverage in both the local and shared feed.
