(async () => {
  ...
  const ICON_ASSET_VERSION = '2.5.179';
  ...
  function renderTrain(){
    ensureDailyCounters();
    const left=sessionsLeft('train',DAILY_TRAINING_LIMIT),sparringLeft=sessionsLeft('sparring',DAILY_SPARRING_LIMIT),coach=state.trainerOn,fee=coachFee(),treatmentReady=state.treatmentAvailable,cooldown=trainingCooldownRemaining(),cooling=cooldown>0,cooldownLabel=trainingCooldownText(cooldown),injury=currentTrainingInjury();
    ...
    setLimitBadge('#recoveryLimitText',!treatmentReady?'FIGHT TO UNLOCK':usableTreatment?'TREATMENT READY':resourcesFull?'RESOURCES FULL':'NEED CASH');
  ...
  function handleTrain(i,cooldownConfirmed=false){
    ensureDailyCounters();const a=trainDefs[i],coach=state.trainerOn;if(!a)return;if(currentTrainingInjury()){toast('Training is closed until your injury heals at midnight.','#ff6875');return}if(trainingCooldownRemaining()>0&&!cooldownConfirmed){openTrainingCooldownWarning('training',i);return}const repeatCount=state.dailyCounters.train;const action={...a,cost:LOGIC.trainingCost(a,repeatCount)};const quote=LOGIC.trainingQuote(state,action,coach,coachFee(),sessionsLeft('train',DAILY_TRAINING_LIMIT));
  ...
  function handleSparring(i,cooldownConfirmed=false){
    ensureDailyCounters();const a=sparringDefs[i],coach=state.trainerOn;if(!a||activeSparringSession)return;if(currentTrainingInjury()){toast('Sparring is closed until your injury heals at midnight.','#ff6875');return}if(trainingCooldownRemaining()>0&&!cooldownConfirmed){openTrainingCooldownWarning('sparring',i);return}const repeatCount=state.dailyCounters.sparring;const action={...a,cost:LOGIC.trainingCost(a,repeatCount)};const quote=LOGIC.trainingQuote(state,action,coach,coachFee(),sessionsLeft('sparring',DAILY_SPARRING_LIMIT));
  ...
  function openRecoverySession(){
    const session=activeRecoverySession;if(!session)return;const {treatment,cashCost}=session,modal=$('#recoverySessionModal'),meter=$('#recoverySessionMeter'),seconds=Math.max(0,Number(treatment.meterSeconds)||2),duration=seconds*1000;$('#recoverySessionKicker').textContent='RECOVERY IN PROGRESS';$('#recoverySessionTitle').textContent=treatment.title.toUpperCase();$('#recoverySessionStatus').textContent=`Treatment in progress · ${seconds} seconds`;$('#recoverySessionResult').innerHTML=modalMeterSummary('COST',`-$${cashCost}`,'TREATMENT IN PROGRESS');$('#collectRecoveryResult').hidden=true;meter.classList.remove('working');meter.style.setProperty('--modal-meter-duration',`${seconds}s`);meter.setAttribute('aria-valuenow','0');modal.classList.add('open');modal.setAttribute('aria-hidden','false');updateUI();void meter.offsetWidth;requestAnimationFrame(()=>meter.classList.add('working'));recoverySessionTimer=setTimeout(resolveRecoverySession,duration);
  }
  function resolveRecoverySession(){
    recoverySessionTimer=null;if(!activeRecoverySession)return;const {treatment,restored}=activeRecoverySession,rewards=[restored.energy?`ENERGY +${formatGain(restored.energy)}`:'',restored.health?`HEALTH +${formatGain(restored.health)}`:''].filter(Boolean),rewardText=rewards.join(' · ');$('#recoverySessionKicker').textContent='RECOVERY COMPLETE';$('#recoverySessionTitle').textContent=`${treatment.title.toUpperCase()} FINISHED.`;$('#recoverySessionStatus').textContent='Treatment collected.';$('#recoverySessionMeter').setAttribute('aria-valuenow','100');$('#recoverySessionResult').innerHTML=modalMeterSummary('REWARD',rewardText,'RESOURCES RESTORED');$('#collectRecoveryResult').hidden=false;sfx.train();flashRecoveryResources(restored);toast(`RECOVERY REWARD · ${rewardText}`,'#78dcff','','',3200);requestAnimationFrame(()=>$('#collectRecoveryResult').focus());
  }
  ...
  function handlePublicity(i){
    ...
    trackEvent('publicity_completed',{publicity_id:a.id,outcome:viral?'viral':'success',cash_earned:cash,followers_gained:fans,energy_spent:a.cost});
  ...
  function runAutographSigning(){
    ...
    trackEvent('publicity_completed',{publicity_id:a.id,autograph_price:price,signatures,cash_earned:cash,followers_gained:fanChange,energy_spent:a.cost});
  ...
  function fillTape(f){
    const fightsLeft=sessionsLeft('fight',DAILY_FIGHT_LIMIT),roundCost=currentFightRoundCost(),clearance=roundCost*FIGHT_ROUNDS,startingCondition=LOGIC.startingFightCondition(state.health,state.maxHealth),cleared=fightsLeft>0&&state.energy>=clearance&&state.health>=MINIMUM_FIGHT_HEALTH;
