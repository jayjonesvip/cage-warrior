(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.4';
  ...
  function ensureRoster(){
    ...
    const active=state.roster.filter(o=>o.tier===state.level&&!o.network&&!o.rookieShowcase&&(o.lossesToPlayer||0)===0).length;for(let i=active;i<2;i++)state.roster.push(generateOpponent(state.level));ensureRookieShowcaseOpponent();
  ...
  function refreshOpponents(){
    ...
    opponents=[...showcase,...circuit,...ranked];
  }
  ...
  function fightWinRewardPreview(opponent){
    ...
    const xpNote=xpTier.tier==='lower_level'?'LOWER LEVEL':xpTier.tier==='exhausted'?'XP USED TODAY':xpTier.tier==='repeat'?'RUNBACK · 50% XP':'';
  ...
  function renderFightLadderRow(opponent,fightsLeft){
    ...
    return `<button class="fight-ranking-row${champion?' champion':''}${opponent.championDefense?' title-defense':''}${opponent.circuitFallback||opponent.rookieShowcase?' circuit':''}${locked?' locked':''}" type="button" ${locked?'disabled':`data-fight-key="${escapeHtml(opponent.key)}"`} aria-label="${escapeHtml(opponent.name)}, ${opponent.network?`rank ${rank}`:'unranked'}, level ${opponent.tier}, ${reward.xp} XP and ${reward.points} Attribute Points for a win${reward.xpNote?`, ${reward.xpNote.toLowerCase()}`:''}${locked?`, ${lockCopy.toLowerCase()}`:'. Open Tale of the Tape.'}"><strong class="fight-rank-position">${rank}</strong><span class="fight-rank-avatar"><img src="${escapeHtml(portrait)}" alt="" loading="lazy"></span><span class="fight-rank-identity"><b>${escapeHtml(opponent.networkHandle?`@${opponent.networkHandle}`:opponent.name)}</b><small>${titleLabel}</small><em>PRO ${opponent.wins}-${opponent.losses} · LVL ${opponent.tier} · ${winPercentage}% WIN</em></span><span class="fight-rank-record"><small>RECORD</small><b>${opponent.wins}-${opponent.losses}</b></span><span class="fight-rank-level"><small>LEVEL</small><b>${opponent.tier}</b></span><span class="fight-rank-win"><small>WIN %</small><b>${winPercentage}%</b></span><span class="fight-rank-rewards"><b>+${reward.xp} XP</b><em>+${reward.points} ATTR PT${reward.points===1?'':'S'}</em>${lockCopy?`<small>${lockCopy}</small>`:reward.xpNote?`<small class="xp-note">${reward.xpNote}</small>`:''}</span></button>`;
  }
  function renderOpponents(){
    refreshOpponents();const fightsLeft=sessionsLeft('fight',DAILY_FIGHT_LIMIT),ranking=currentRanking(),showcase=opponents.filter(item=>item.rookieShowcase),ranked=opponents.filter(item=>item.network),circuit=opponents.filter(item=>!item.network&&!item.rookieShowcase);$('#fightResetClock').hidden=fightsLeft>0;setLimitBadge('#fightLimitText',`${fightsLeft} ${fightsLeft===1?'FIGHT':'FIGHTS'} LEFT`);$('#rosterSummary').textContent=`${circuit.length} CIRCUIT · ${ranked.length} RANKED${ranking.position?` · YOUR RANK #${ranking.position}`:''}`;$('#fightInjuryWarning').hidden=state.health>=state.maxHealth;const showcaseRows=showcase.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join(''),circuitRows=circuit.length?`<div class="fight-ranking-group circuit-group"><b>ON-LEVEL CAGE CIRCUIT</b><span>${circuit.length} FRESH MATCHUPS · FULL XP</span></div>${circuit.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'',rankedRows=ranked.length?`<div class="fight-ranking-group ranked-group"><b>WORLD RANKINGS</b><span>${ranked.length} RANKED FIGHTERS</span></div>${ranked.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'';$('#opponentList').innerHTML=opponents.length?`<div class="fight-ranking-list">${showcaseRows}${circuitRows}${rankedRows}</div>`:`<div class="gear-empty"><b>BUILDING THE FIGHT CARD</b><span>New opponents will appear shortly.</span></div>`;
  }
