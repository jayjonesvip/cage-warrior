...
The shared Supabase-backed championship remains server-authoritative and non-fatal when offline. The Fight page uses the same ranking order as the Home rankings modal: the active champion is first, followed by a hybrid World Rank score weighted 50% résumé, 25% quality of defeated opposition, 15% recent form, and 10% permanent base attributes. Résumé win rate uses a neutral 2-2 prior so one-fight undefeated records do not outrank proven careers. Equipped gear and non-combat perks never inflate World Rank. Each full-width row shows rank, portrait, name, record, level, win percentage, exact win XP, and the Attribute Points available for that matchup. Tap any available row to open the existing Tale of the Tape.
