(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.123';
  ...
  const INSTALL_OFFER_DISMISS_KEY = 'cage-grind-install-offer-hidden';
  ...
  let installOfferDismissed = true;
  ...
  $('#installOfferHideBtn').addEventListener('click',()=>{installOfferDismissed=true;try{sessionStorage.setItem(INSTALL_OFFER_DISMISS_KEY,'1')}catch{/* session-only fallback remains in memory */}updateUI();sfx.tap()});
