(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.120';
  ...
  function gearCategorySlug(category){return String(category).toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}
  ...
  function collectionBlockHtml(category,availableItems,undiscoveredItems){
    ...
    return `<div class="gear-collection-block"><div class="gear-subhead gear-collection-subhead"><span><b>AVAILABLE + UNDISCOVERED</b><small>${availableItems.length} READY · ${undiscoveredItems.length} HIDDEN</small></span></div>${groups}</div>`;
  }
  function renderGear(){
