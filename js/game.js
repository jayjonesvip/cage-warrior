(async () => {
  ...
  const ICON_ASSET_VERSION = '2.5.212';
  ...
  function renderRankings(){
    const list=$('#rankingsList'),ranked=LOGIC.rankFighters(rankingProfiles(),sharedChampionship,25);
    if(ranked.length){list.innerHTML=ranked.map((profile,index)=>{const mine=profile.id===state.socialProfileId,winPercentage=Math.round(profile.winPercentage*100),champion=profile.isChampion?'<em class="rankings-champion">WORLD CHAMPION</em>':'',you=mine?'<em class="rankings-you">YOU</em>':'';return `<div class="rankings-row${profile.isChampion?' champion':''}${mine?' player':''}"><strong class="rankings-position">#${index+1}</strong><span class="rankings-fighter"><b>@${escapeHtml(profile.handle)}</b><small>${champion}${you}</small></span><b class="rankings-level">LVL ${profile.level}</b><span class="rankings-record">${profile.wins}-${profile.losses}</span><span class="rankings-percentage">${winPercentage}%</span></div>`}).join('');return}
