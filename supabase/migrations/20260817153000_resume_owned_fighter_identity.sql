-- Let an authenticated browser resume one of its own retired fighter identities.
-- A registry row owned by any other auth user remains permanently unavailable.
...
'Claims a unique fighter handle or resumes a retired handle already owned by the authenticated browser profile.';
