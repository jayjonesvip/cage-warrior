(() => {
  ...
  const ICON_ASSET_VERSION = '2.5.151';
  ...
  function resolveHustleShift(){
    hustleShiftTimer=null;if(!activeHustleShift)return;const {totalCash,payDetail,bonusText}=activeHustleShift,result=$('#hustleShiftResult'),jobsLeft=sessionsLeft('hustle',2);$('#hustleShiftTitle').textContent='JOB COMPLETE';$('#hustleShiftStatus').textContent=`${jobsLeft} job${jobsLeft===1?'':'s'} left today.`;$('#hustleShiftMeter').setAttribute('aria-valuenow','100');result.innerHTML=`<b>+$${totalCash} CASH</b>${payDetail?`<small>${payDetail}</small>`:''}<span>${bonusText||'JOB COMPLETE · NO BONUS THIS TIME'}</span>`;result.hidden=false;$('#collectHustleShift').hidden=false;sfx.coin();requestAnimationFrame(()=>$('#collectHustleShift').focus());
  }
