(async () => {
  ...
  const ICON_ASSET_VERSION = '2.5.189';
  ...
  const defaultState = {
    ...
    pendingFight:null,pendingChampionshipResult:null,fightPlanPreference:{pace:'slow',offense:'conservative',tactics:'stick'},focusTextDeck:[],lastFocusTextId:'',trainingInjury:null,
  ...
  function applyRepeatTrainingRisk(repeatCount=0,coach=false){
    const repeated=repeatCount>0;let injury=null;
    if(repeated&&!state.trainingInjury&&Math.random()<LOGIC.trainingInjuryChance(repeated,coach)){injury=trainingInjuryDefs[rint(0,trainingInjuryDefs.length-1)];state.trainingInjury={id:injury.id,date:todayKey()}}
    return {repeated,injury};
  }
  ...
  function spendEnergy(n){const energySpent=LOGIC.spendEnergy(state,n);if(!energySpent){toast('Energy is empty. Rest in the Recovery Room to recharge.','#ff766d');return 0}return energySpent}
  ...
  function updateUI(){
    ...
    const today=todayKey();$('#dailyBtn').disabled=state.lastDaily===today;$('#dailyBtn').textContent=state.lastDaily===today?'DROP CLAIMED':'CLAIM DAILY DROP';
  ...
  function createSocialAccount(){
    ...
    const previousCeoEvents=[...state.ceoEvents],accountPosts=copyPosts([STRINGS.social.account[0]],{name:state.name});addSocialPosts(accountPosts);publishCeoEvent('debut');if(firstAccount&&previousCeoEvents.length)queueSharedPosts(previousCeoEvents.map(eventKey=>({kind:'ceo',eventKey:ceoRemoteEventKey(eventKey)})));
  ...
  function renderTrain(){
    ensureDailyCounters();
    const left=sessionsLeft('train',DAILY_TRAINING_LIMIT),sparringLeft=sessionsLeft('sparring',DAILY_SPARRING_LIMIT),coach=state.trainerOn,fee=coachFee(),treatmentReady=state.treatmentAvailable,injury=currentTrainingInjury();
    ...
    $('#trainActions').innerHTML=trainDefs.map((a,i)=>{const trainRepeat=state.dailyCounters.train,coachCost=coach?fee:0,cost=LOGIC.trainingCost(a,trainRepeat),gain=LOGIC.trainingGain(a.gain,coach,false,trainRepeat),locked=!!injury||state.energy<=0||state.cash<coachCost||left<1,status=injury?'<b>INJURED</b><small>TRAINING CLOSED UNTIL MIDNIGHT</small>':`<b>+${gain} ${a.stat.toUpperCase()}</b><small>UP TO ${cost}% ENERGY${coach?` &middot; COACH $${coachCost}`:''}</small>`;return `<button class="action${injury?' injury-locked':''}" data-train="${i}" ${locked?'disabled':''}><div class="ico">${gameIcon(a.id,a.icon)}</div><div><h3>${a.title}</h3><p>${a.text}</p></div><div class="cost">${status}</div></button>`}).join('');
    setLimitBadge('#sparringLimitText',`${sparringLeft} SESSION${sparringLeft===1?'':'S'} LEFT`);
    $('#sparringActions').innerHTML=sparringDefs.map((a,i)=>{const sparringRepeat=state.dailyCounters.sparring,coachCost=coach?fee:0,cost=LOGIC.trainingCost(a,sparringRepeat),gain=LOGIC.trainingGain(a.gain,coach,false,sparringRepeat),risk=a.damage?` &middot; ${LOGIC.sparringDamage(a.damage[0],sparringRepeat)}&ndash;${LOGIC.sparringDamage(a.damage[1],sparringRepeat)} health risk`:' &middot; no health damage',locked=!!injury||state.energy<=0||state.cash<coachCost||sparringLeft<1,skillLabel=a.skills===1?`+${gain} RANDOM SKILL`:`+${gain} TO 2 RANDOM SKILLS`,status=injury?'<b>INJURED</b><small>SPARRING CLOSED UNTIL MIDNIGHT</small>':`<b>${skillLabel}</b><small>UP TO ${cost}% ENERGY${risk}${coach?` &middot; COACH $${coachCost}`:''}</small>`;return `<button class="action sparring-${a.tier}${injury?' injury-locked':''}" data-sparring="${i}" ${locked?'disabled':''}><div class="ico">${gameIcon(a.asset||a.id,a.icon)}</div><div><h3>${a.title}</h3><p>${a.text}</p></div><div class="cost">${status}</div></button>`}).join('');
    const recoveryOptions=recoveryDefs.map((a,i)=>{const treatmentFee=recoveryFee(a),quote=LOGIC.recoveryQuote(state,a,treatmentFee,a.freeRest?false:!treatmentReady);return {a,i,treatmentFee,quote}}),usableTreatment=recoveryOptions.some(({a,quote})=>!a.freeRest&&quote.ok),restAvailable=state.energy<state.maxEnergy;
    setLimitBadge('#recoveryLimitText',restAvailable?'REST AVAILABLE':!treatmentReady?'FIGHT TO UNLOCK':usableTreatment?'TREATMENT READY':'HEALTH FULL');
    $('#recoveryActions').innerHTML=recoveryOptions.map(({a,i,treatmentFee,quote})=>{const gains=[a.energy?`+${a.energy}% ENERGY`:'',a.health?`+${a.health} HEALTH`:''].filter(Boolean).map(gain=>`<span class="recovery-gain">${gain}</span>`).join(''),status=a.freeRest?(quote.ok?`FREE &middot; ${REST_DURATION_SECONDS} SECONDS`:'FULL POWER'):quote.reason==='limit'?'COMPLETE A FIGHT TO UNLOCK':quote.reason==='cash'?`NEED $${treatmentFee}`:quote.reason==='full'?'HEALTH FULL':`-$${treatmentFee}`;return `<button class="action${a.freeRest?' rest-action':''}" data-recovery="${i}" ${quote.ok?'':'disabled'}><div class="ico">${gameIcon(a.id,a.icon)}</div><div><h3>${a.title}</h3><p>${a.text}</p></div><div class="cost"><b class="recovery-gains">${gains}</b><small>${status}</small></div></button>`}).join('');
  ...
  function handleRecovery(i){
    ensureDailyCounters();const treatment=recoveryDefs[i];if(!treatment||activeRecoverySession)return;const fee=recoveryFee(treatment),quote=LOGIC.recoveryQuote(state,treatment,fee,treatment.freeRest?false:!state.treatmentAvailable);
    ...
    state.cash-=quote.cashCost;if(!treatment.freeRest)state.treatmentAvailable=false;const restored=treatment.freeRest?null:LOGIC.applyRecovery(state,treatment);initAudio();activeRecoverySession={treatment,cashCost:quote.cashCost,restored};openRecoverySession();
  }
  ...
  function visitRecoveryRoom(){closeFightPreview(true);navTo('train');requestAnimationFrame(()=>$('#recoveryRoomCard').scrollIntoView({behavior:'smooth',block:'start'}));toast('RECOVERY ROOM · REST FREE FOR +25% ENERGY','#78dcff')}
  ...
  function revealGearDrop(){
    ...
    const drop=Object.assign({},normalized,{item}),status=drop.isNew?'NEW ITEM':`OWNED ×${drop.count}`,dropLabel=`${drop.rarity} COLLECTIBLE DROP`,lootBox=$('#lootBox'),resultCard=$('#resultModal .result-card');
