(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.101';
  ...
  function renderGearLoadoutDock(){
    ...
    const group=category=>{const loadout=equippedForCategory(category),slug=gearCategorySlug(category),filter=LOADOUT_FILTER_BY_CATEGORY[category],available=state.gear.some(id=>itemCategory(id)===category&&!loadout.includes(id)&&state.level>=(gearItems.find(item=>item.id===id)?.minLevel||1)),slots=Array.from({length:LOGIC.loadoutCategoryLimit()},(_,index)=>{const item=gearItems.find(entry=>entry.id===loadout[index]),rarity=String(item?.rarity||'').toLowerCase(),slotLabel=item?`${category} slot ${index+1}: ${item.name}, ${item.rarity||'common'}`:`Empty ${category} slot ${index+1}`;return item?`<span class="gear-loadout-thumb filled rarity-slot-${rarity}" aria-label="${escapeHtml(slotLabel)}" title="${escapeHtml(slotLabel)}">${gameIcon(item.iconName||item.id,item.icon,item.assetExt)}<i class="loadout-rarity-dot" aria-hidden="true"></i></span>`:available?`<button class="gear-loadout-thumb add-slot" type="button" data-loadout-category="${filter}" aria-label="Choose an item for ${escapeHtml(category)} slot ${index+1}">+</button>`:`<span class="gear-loadout-thumb empty" aria-label="${escapeHtml(slotLabel)}"></span>`}).join('');return `<div class="gear-loadout-group loadout-group-${slug}" role="group" aria-label="${escapeHtml(category)} loadout"><span class="gear-loadout-category">${category==='Property & Rides'?'PROPERTY':category==='Fight Gear'?'COMBAT':category.toUpperCase()}</span><div class="gear-loadout-pair">${slots}</div></div>`};
    slotRoot.innerHTML=LOADOUT_CATEGORIES.map(group).join('');
  }
  function renderGear(){
    ...
    const shop=$('#gearShop'),headerHost=$('#gearCardHeader');shop.innerHTML=sections;const header=shop.querySelector('.shop-head');headerHost.replaceChildren(...(header?[header]:[]));shop.querySelector('.gear-loadout-shop-block')?.remove();
  }
