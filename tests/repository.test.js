...
test('the branded landing page gates the game and offers the correct career entry paths', () => {
  ...
  assert.match(page, /<main class="landing-card">[\s\S]*class="landing-watermark" src="assets\/app-icon-512\.png\?v=2\.5\.168" alt="" aria-hidden="true"/);
...
test('career identity keeps hometown informational and uses one shared real-player championship', () => {
  ...
  assert.match(serviceWorker, /\.\/assets\/icons\/title-world\.png\?v=2\.5\.168/);
...
test('the simplified World Championship is a daily server-authoritative flow', () => {
  ...
  assert.match(page, /id="worldTitleCard"[\s\S]*Career Opponents/);
...
test('career opponent roster uses proportional two-across collectible fighter cards', () => {
  ...
  assert.match(script, /<article class="opponent \$\{status\} \$\{o\.network\?'network':''\} \$\{rivalFight\?'rematch':''\} \$\{dailyExhausted\?'daily-exhausted':''\}"\$\{o\.network\?` style="--hometown-accent:\$\{fighterAccent\(o\.networkCity\)\}"`:''\} data-card-flip="true"/);
...
test('career fights use a reversible tale-of-the-tape preview before locker-room planning', () => {
  ...
  assert.match(css, /\.live-card\{[^}]*background-image:url\("\.\.\/assets\/cage-grind-octagon-transparent\.png\?v=2\.5\.168"\)[^}]*background-position:center 62%/);
  assert.match(css, /\.live-card\.decision-active\{[^}]*background-image:linear-gradient\(#030914f6,#030914f6\),url\("\.\.\/assets\/cage-grind-octagon-transparent\.png\?v=2\.5\.168"\)[^}]*background-position:center,center 62%/);
...
test('booked fights resolve a 50-50 locker-room Focus encounter after planning', () => {
  ...
  assert.match(page, /id="focusStage"[\s\S]*class="fight-plan-locker-room"[\s\S]*class="fight-plan-locker-art" src="assets\/focus-locker-room\.jpg\?v=2\.5\.168"/);
  ...
  assert.match(serviceWorker, /\.\/assets\/focus-locker-room\.jpg\?v=2\.5\.168/);
  assert.match(serviceWorker, /\.\/assets\/contact-mom\.jpg\?v=2\.5\.168/);
  assert.match(serviceWorker, /\.\/assets\/contact-wife\.jpg\?v=2\.5\.168/);
  assert.match(serviceWorker, /\.\/assets\/contact-brother-tommy\.png\?v=2\.5\.168/);
  assert.match(serviceWorker, /\.\/assets\/contact-agent-carl\.png\?v=2\.5\.168/);
  assert.match(serviceWorker, /\.\/assets\/contact-grandma\.jpg\?v=2\.5\.168/);
...
test('the landing page preserves Cage Grind history without treating the old game as a technical blueprint', () => {
  ...
  assert.match(originsPage, /css\/origins\.css\?v=2\.5\.168/);
  ...
  assert.match(serviceWorker, /'\.\/css\/origins\.css\?v=2\.5\.168'/);
});
...
test('rendered icons support stable per-file image overrides with fallbacks', () => {
  ...
  assert.match(script, /const ICON_ASSET_VERSION = '2\.5\.168'/);
...
test('Cage Feed combines career reports with avatar-driven fighter interactions', () => {
  ...
  assert.match(script, /mentions=posts\.filter\(feedPostMentionsPlayer\),visiblePosts=feedFilter==='mentions'\?mentions:posts/);
...
test('Cage Grind CEO is verified while championship announcements stay database-owned', () => {
  ...
  assert.match(page, /assets\/cage-grind-ceo\.jpg\?v=2\.5\.168/g);
  ...
  assert.match(serviceWorker, /\.\/assets\/cage-grind-ceo\.jpg\?v=2\.5\.168/);
});
...
test('hometown colors persist with fighters across the game presentation', () => {
  ...
  assert.match(script, /style="--hometown-accent:\$\{fighterAccent\(o\.networkCity\)\}"/);
  assert.match(script, /championship-matchup" style="\$\{fighterThemeStyle\(opponent\.networkCity\)\}"/);
  assert.match(script, /\$\('#tapeStage \.player-card'\)\.style\.setProperty\('--fighter-accent',playerAccent\)/);
  assert.match(script, /\$\('#tapeStage \.opponent-card'\)\.style\.setProperty\('--fighter-accent',opponentAccent\)/);
...
test('CageReporter has a verified clickable read-only profile', () => {
  ...
  assert.match(strings, /author:'CageReporter',handle:'@CageReporter',tone:'media',avatar:'assets\/cage-reporter\.jpg\?v=2\.5\.168',verified:true,bio:/);
  ...
  assert.match(script, /text:'@\{name\} has joined the global fight conversation\.'/);
  assert.match(script, /reporterValues=Object\.assign\(\{\},values,\{name:`@\$\{String\(values\.name\|\|name\)\.replace\(\/\^@\//);
  assert.match(script, /kind:'reporter',body:post\.text,targetProfileId:state\.socialProfileId/);
  ...
  assert.match(serviceWorker, /\.\/assets\/cage-reporter\.jpg\?v=2\.5\.168/);
  assert.match(readme, /CageReporter uses a dedicated newsroom portrait and verified profile/);
});
...
test('Hype bonuses are visible on the matchup and publicity is limited to one daily gig', () => {
  ...
  assert.match(script, /activePublicitySession=\{action:a,rewardText:/);
  assert.match(script, /modalMeterSummary\('COST',`-\$\{action\.cost\} ENERGY`/);
...
test('Odd Jobs are limited to two daily and retire when a fighter reaches Level 5', () => {
  ...
  assert.match(page, /id="hustleShiftModal"[\s\S]*id="hustleShiftMeter"[\s\S]*id="hustleShiftIcon"[\s\S]*COLLECT PAY/);
...
test('training separates daily sparring from post-fight recovery opportunities', () => {
  ...
  assert.match(css, /\.modal-meter>span\{[^}]*left:50%[^}]*transform:translateX\(-50%\)/);
  assert.doesNotMatch(css, /\.modal-meter\.working>span/);
  ...
  assert.match(script, /MIN_TRAINING_COOLDOWN_MS=60000/);
  assert.match(script, /function applyTrainingCooldown\(durationMs,coach=false\)/);
  assert.match(script, /state\.trainingCooldownUntil=\(overtraining\?Math\.max\(now,state\.trainingCooldownUntil\):now\)\+duration/);
  assert.match(script, /LOGIC\.trainingCooldownDuration\(\{type:'training',gain\}\)/);
  assert.match(script, /LOGIC\.trainingCooldownDuration\(\{type:'sparring',skills:a\.skills\}\)/);
  assert.match(script, /LOGIC\.trainingInjuryChance\(overtraining,coach\)/);
  ...
  assert.match(script, /modalMeterSummary\('COST',`-\$\$\{cashCost\}`,'TREATMENT IN PROGRESS'\)/);
  assert.match(script, /modalMeterSummary\('REWARD',rewardText,'RESOURCES RESTORED'\)/);
  ...
  assert.match(page, /Gym cooldowns last one minute for a \+1 gain and two minutes for a \+2 gain/);
  assert.match(page, /id="trainingCooldownModal"[\s\S]*NEVERMIND[\s\S]*NO PAIN, NO GAIN/);
  assert.match(script, /trainingCooldownRemaining\(\)>0&&!cooldownConfirmed/);
  assert.match(script, /LOGIC\.trainingRiskBonus\(recovery\.overtraining,!!recovery\.injury\)/);
...
test('Underground Buzz keeps persistent once-daily blackjack, Cage Dice, and horse racing', () => {
  ...
  assert.match(page, /assets\/cage-dice\.jpg\?v=2\.5\.168/);
  ...
  assert.match(serviceWorker, /\.\/assets\/cage-dice\.jpg\?v=2\.5\.168/);
...
test('the collectible drop pool includes early-career and status cards', () => {
  ...
  assert.match(script, /id:'energy-drink'.*category:'Lifestyle'.*assetExt:'jpg'.*rarity:'COMMON'.*minLevel:1.*energyRegen:\.02/);
...
test('opponent cards, matchup preview, and results disclose daily repeat XP', () => {
  ...
  assert.match(script, /state\.dailyOpponentWins\.wins\[o\.key\]=Math\.min\(2,winsToday\+1\)/);
