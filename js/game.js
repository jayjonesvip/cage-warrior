(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.3';
  ...
  const defaultState = {
    version:STATE_VERSION,name:'ROOKIE',nameLocked:false,rookieShowcasePending:false,postFightTutorialSeen:false,fans:0,level:1,xp:0,wins:0,losses:0,winStreak:0,bestStreak:0,attributePoints:0,
    ...
    pendingFight:null,pendingChampionshipResult:null,fightPlanPreference:{pace:'slow',offense:'conservative',tactics:'stick'},focusTextDeck:[],lastFocusTextId:'',fightInjury:null,
  ...
  function generatedOpponentRatings(tier,serial,seed,arch){
  ...
  function generateOpponent(tier){
    const serial=++state.rosterSerial,seed=serial*7919+tier*104729,arch=rosterPick(opponentArchetypes,seed),identity=generatedOpponentIdentity(seed),ratings=generatedOpponentRatings(tier,serial,seed,arch);
  ...
  function rebalanceGeneratedOpponent(opponent){
    const match=/^cw-(\d+)-(\d+)$/.exec(String(opponent?.key||''));if(!match||opponent.network||opponent.rookieShowcase)return;
    const tier=Math.max(1,Number(match[1])||opponent.tier||1),serial=Math.max(1,Number(match[2])||1),seed=serial*7919+tier*104729,arch=rosterPick(opponentArchetypes,seed);Object.assign(opponent,generatedOpponentRatings(tier,serial,seed,arch));
  }
  ...
  function ensureRoster(){
    ...
    const active=state.roster.filter(o=>o.tier===state.level&&!o.network&&!o.rookieShowcase&&(o.lossesToPlayer||0)===0).length;for(let i=active;i<4;i++)state.roster.push(generateOpponent(state.level));ensureRookieShowcaseOpponent();
  ...
  function rankedTitleContext(opponent,worldRank){
    const playerIsChampion=sharedChampionship?.is_champion===true,opponentIsChampion=opponent.sourceProfileId===sharedChampionship?.champion_id;
    if(!playerIsChampion&&!opponentIsChampion)return Object.assign({},opponent,{worldRank});
  ...
  function refreshOpponents(){
    ...
    ranking.forEach((profile,index)=>{if(profile.id===state.socialProfileId||String(profile.handle||'').toLowerCase()===String(state.name||'').toLowerCase()||(!profile.isChampion&&profile.fights<1))return;let opponent=state.roster.find(item=>item.network&&item.sourceProfileId===profile.id);if(!opponent)opponent=networkOpponentFromProfile(profile,profile.level);if(opponent)ranked.push(rankedTitleContext(opponent,index+1))});
    const showcase=state.roster.filter(item=>item.rookieShowcase&&state.rookieShowcasePending),circuit=state.roster.filter(item=>!item.network&&!item.rookieShowcase&&item.tier===state.level&&(item.lossesToPlayer||0)===0).sort(fighterLevelOrder).slice(0,4).map(opponent=>Object.assign({},opponent,{worldRank:null,circuitFallback:true}));
    opponents=[...ranked,...showcase,...circuit];
  }
  ...
  function applyOfflineRecovery(){
    const now=Date.now(),recovered=LOGIC.passiveRecovery(state,now,OFFLINE_RECOVERY_CAP,{energy:energyRecoveryInterval(),health:HEALTH_RECOVERY_INTERVAL});
  ...
  landingFeature=globalThis.CAGE_LANDING.createLandingFeature({
    $,logic:LOGIC,getState:()=>state,getRank:()=>rankName(),getChampionship:()=>sharedChampionship,setChampionship:setSharedChampionship,sharedFeed:SHARED_FEED,sharedUi:SHARED_UI,trackEvent,tap:()=>sfx.tap(),onEntered:()=>setTimeout(()=>{showRecoveryReport();offerRookieShowcase()},180),onChampionshipChange:renderOpponents
  });
  ...
  function syncSponsorProgress(announce=true){const before=state.activeEndorsement?.id||'',progress=LOGIC.sponsorProgress(endorsementDefs,state.fans,state.endorsementHistory);state.activeEndorsement=progress.active?{id:progress.active.id}:null;state.endorsementHistory=progress.history;if(announce&&progress.active&&progress.active.id!==before){state.sponsorAnnouncementPending=progress.active.id;publishSponsorSigning(progress.active);trackEvent('sponsor_unlocked',{sponsor_id:progress.active.id,followers:state.fans,followers_required:progress.active.followersRequired})}return progress}
  ...
  function renderRecoveryClocks(){
    ...
    healthStatus.hidden=state.health>=state.maxHealth;healthStatus.textContent=`RECOVERING · FULL IN ${LOGIC.formatCountdown(LOGIC.recoveryTimeRemaining(state.health,state.maxHealth,state.healthRecoveryAt,HEALTH_RECOVERY_INTERVAL))}`;
  }
  function updatePassiveRecovery(){
    const recovered=LOGIC.passiveRecovery(state,Date.now(),OFFLINE_RECOVERY_CAP,{energy:energyRecoveryInterval(),health:HEALTH_RECOVERY_INTERVAL});
    if(recovered.energy>0)flashRecoveryResources({energy:recovered.energy,health:0});
    if(recovered.energy||recovered.health){saveState();if(state.nameLocked)updateUI()}else renderRecoveryClocks();
  }
  ...
  function updateUI(){
    ...
    $('#hudEnergyText').textContent=`${energyNow}%`;$('#energyBattery').setAttribute('aria-label',`Energy ${energyNow} percent`);$$('#energyBattery i').forEach((segment,index)=>segment.style.setProperty('--charge',`${clamp((state.energy-index*25)/25*100,0,100)}%`));const lowEnergy=state.energy<=25;$('#energyHud').classList.toggle('critical',lowEnergy);
  ...
  function renderAttributeAssignment(){
    ...
    areas.forEach(area=>{area.hidden=state.attributePoints<1;const count=area.querySelector('[data-attribute-points]');if(count)count.textContent=`${state.attributePoints} ATTRIBUTE POINT${state.attributePoints===1?'':'S'} AVAILABLE`;area.querySelectorAll('[data-assign-attribute]').forEach(button=>{const key=button.dataset.assignAttribute;button.disabled=state.attributePoints<1;const value=button.querySelector('b');if(value)value.textContent=state.stats[key]})});
  }

  function renderPostFightTutorial(win){
    const tutorial=$('#postFightTutorial');tutorial.hidden=state.postFightTutorialSeen;$('#postFightTutorialReward').textContent=win?'Assign your Attribute Point now, or save it for Home.':'Attribute Points are awarded only when you win.';
  }
  ...
  function fightWinRewardPreview(opponent){
    const points=LOGIC.victoryAttributePointReward(state.level,opponent.tier),playerRating=state.stats.power+state.stats.speed+state.stats.chin+state.stats.cardio,opponentRating=opponent.power+opponent.speed+opponent.chin+opponent.cardio,upset=opponentRating>=playerRating+4,titleWon=opponent.globalChampionship&&!opponent.championDefense;
    const result=LOGIC.fightXp({playerLevel:state.level,opponentLevel:opponent.tier,won:true,upset,ranked:!!opponent.network&&!opponent.globalChampionship,championship:!!opponent.globalChampionship,titleWon,rival:false,opponentWinsToday:opponentWinsToday(opponent)});
    return {points,xp:result.xp};
  }
  function renderFightLadderRow(opponent,fightsLeft){
    const reward=fightWinRewardPreview(opponent),fights=Math.max(0,Number(opponent.wins)||0)+Math.max(0,Number(opponent.losses)||0),winPercentage=fights?Math.round((Number(opponent.wins)||0)/fights*100):0,titleLocked=opponent.globalChampionship&&opponent.titleCooldown,dailyLocked=fightsLeft<1,locked=titleLocked||dailyLocked,champion=opponent.sourceProfileId===sharedChampionship?.champion_id,portrait=silhouetteForOpponent(opponent),rank=opponent.network?`#${opponent.worldRank||'—'}`:'N/A',titleLabel=champion?'WORLD CHAMPION':opponent.championDefense?'TITLE DEFENSE':opponent.rookieShowcase?'OPENING FIGHT':opponent.circuitFallback?'UNRANKED · CAGE CIRCUIT':'RANKED FIGHTER',lockCopy=dailyLocked?'DAILY LIMIT':titleLocked?(opponent.championDefense?'DEFENSE USED':'ATTEMPT USED'):'';
    return `<button class="fight-ranking-row${champion?' champion':''}${opponent.championDefense?' title-defense':''}${opponent.circuitFallback||opponent.rookieShowcase?' circuit':''}${locked?' locked':''}" type="button" ${locked?'disabled':`data-fight-key="${escapeHtml(opponent.key)}"`} aria-label="${escapeHtml(opponent.name)}, ${opponent.network?`rank ${rank}`:'unranked'}, level ${opponent.tier}, ${reward.xp} XP and ${reward.points} Attribute Points for a win${locked?`, ${lockCopy.toLowerCase()}`:'. Open Tale of the Tape.'}"><strong class="fight-rank-position">${rank}</strong><span class="fight-rank-avatar"><img src="${escapeHtml(portrait)}" alt="" loading="lazy"></span><span class="fight-rank-identity"><b>${escapeHtml(opponent.networkHandle?`@${opponent.networkHandle}`:opponent.name)}</b><small>${titleLabel}</small><em>PRO ${opponent.wins}-${opponent.losses} · LVL ${opponent.tier} · ${winPercentage}% WIN</em></span><span class="fight-rank-record"><small>RECORD</small><b>${opponent.wins}-${opponent.losses}</b></span><span class="fight-rank-level"><small>LEVEL</small><b>${opponent.tier}</b></span><span class="fight-rank-win"><small>WIN %</small><b>${winPercentage}%</b></span><span class="fight-rank-rewards"><b>+${reward.xp} XP</b><em>+${reward.points} ATTR PT${reward.points===1?'':'S'}</em>${lockCopy?`<small>${lockCopy}</small>`:''}</span></button>`;
  }
  function renderOpponents(){
    refreshOpponents();const fightsLeft=sessionsLeft('fight',DAILY_FIGHT_LIMIT),ranking=currentRanking(),ranked=opponents.filter(item=>item.network),circuit=opponents.filter(item=>!item.network);$('#fightResetClock').hidden=fightsLeft>0;setLimitBadge('#fightLimitText',`${fightsLeft} ${fightsLeft===1?'FIGHT':'FIGHTS'} LEFT`);$('#rosterSummary').textContent=`${ranked.length} RANKED · ${circuit.length} CIRCUIT${ranking.position?` · YOUR RANK #${ranking.position}`:''}`;$('#fightInjuryWarning').hidden=state.health>=state.maxHealth;const circuitRows=circuit.length?`<div class="fight-ranking-group"><b>ON-LEVEL CAGE CIRCUIT</b><span>UNRANKED · RANK N/A · LEVEL ${state.level}</span></div>${circuit.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'';$('#opponentList').innerHTML=opponents.length?`<div class="fight-ranking-list">${ranked.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}${circuitRows}</div>`:`<div class="gear-empty"><b>BUILDING THE FIGHT CARD</b><span>New opponents will appear shortly.</span></div>`;
  }
  ...
  function openTapeBreakdown(){if(!fight)return;renderTapeBreakdown();$('#tapeBreakdown').hidden=false;$('#tapeTermsToggle').setAttribute('aria-expanded','true');$('#tapeBreakdownClose').focus();sfx.tap()}
  ...
  function renderResultBonuses(notes=[]){
    const summary=$('#resultBonuses');if(!summary)return;summary.hidden=!notes.length;summary.innerHTML=notes.map(note=>`<div class="result-bonus-row${note.kind==='streak'?' win-streak-bonus':''}">${note.kind==='streak'?'🔥 ':''}${escapeHtml(note.text)}</div>`).join('');
  }
  ...
  function finishFightSimulation(){
    ...
    if(win){
      ...
      if(attributePoint)lootNotes.push({kind:'milestone',text:`+${attributePoint} ATTRIBUTE POINT${attributePoint===1?'':'S'} · ASSIGN NOW OR SAVE`});else lootNotes.push({kind:'milestone',text:'LOWER-LEVEL WIN · NO ATTRIBUTE POINTS'});const currentSponsor=endorsementDefs.find(item=>item.id===state.activeEndorsement?.id);if(currentSponsor)lootNotes.push({kind:'sponsor',text:`CURRENT SPONSOR · ${currentSponsor.brand}`});if(o.globalChampionship)lootNotes.push({kind:'milestone',text:'CONFIRMING TITLE RESULT'});if(ceoHype)lootNotes.push({kind:'milestone',text:'CEO NOTICED · +3 HYPE'});if(upset)lootNotes.push({kind:'milestone',text:'UPSET VICTORY'});if(rivalry)lootNotes.push({kind:'milestone',text:'RIVALRY WIN'});if(state.winStreak>1)lootNotes.push({kind:'streak',text:`${state.winStreak}-FIGHT WIN STREAK`});ensureRoster()
      ...
      o.wins=(o.wins||0)+1;o.winsVsPlayer=(o.winsVsPlayer||0)+1;o.rematchAccepted=true;state.losses++;state.winStreak=0;if(!fight.forfeited){fans=Math.round(o.fans*.15);fans=changeFollowers(fans)}state.hype=clamp(state.hype-7,0,100);sfx.lose();
  ...
  $('#tapeBackBtn').addEventListener('click',closeFightPreview);$('#tapeFightBtn').addEventListener('click',()=>commitFight());$('#fightPlanConfirm').addEventListener('click',confirmFightPlan);$('#tapeTermsToggle').addEventListener('click',openTapeBreakdown);$('#tapeBreakdownClose').addEventListener('click',()=>closeTapeBreakdown());$('#tapeBreakdownBackdrop').addEventListener('click',()=>closeTapeBreakdown());$('#tapeBreakdown').addEventListener('keydown',e=>{if(e.key==='Escape')closeTapeBreakdown()});
  $('#speedBtn').addEventListener('click',toggleFightSpeed);$('#detailsToggle').addEventListener('click',()=>{const details=$('#resultDetails'),open=!details.classList.contains('open');details.classList.toggle('open',open);$('#detailsToggle').textContent=open?'HIDE SCORECARD':'VIEW SCORECARD'});$('#shareWinBtn').addEventListener('click',shareFightWin);
