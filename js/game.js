(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.160';
  ...
  function collectibleCardHtml(item,{dropStatus='',loadoutCard=false}={}){
    ...
    return `<div class="gear collectible-card owned rarity-card-${rarityClass} ${levelLocked?'level-locked':''} ${dropStatus?'drop-claim-card':''} ${loadoutCard&&equipped?'equipped-full':''}" data-collectible-flip data-collectible-id="${item.id}" tabindex="0" aria-label="${cardLabel}" aria-pressed="false"><div class="collectible-flip"><div class="collectible-side collectible-front" aria-hidden="false">${lockIcon}<div class="gear-top"><span class="rarity-tag">${rarity}</span>${equippedMark}</div><div class="gear-hero"><span class="gear-flair"></span><span class="equip-burst"></span><div class="gear-icon">${gameIcon(item.iconName||item.id,item.icon,item.assetExt)}</div></div><div class="gear-copy"><h3>${escapeHtml(item.name)}</h3></div><div class="gear-footer"><span class="level-tag">MIN LVL ${item.minLevel||1}</span>${action}</div></div>${collectibleBackHtml(item,rarity)}</div></div>`;
  }
  ...
  function revealDropClaim(){
    ...
    const drop=Object.assign({},normalized,{item});$('#dropClaimStage').innerHTML=collectibleCardHtml(item,{dropStatus:drop.autoEquipped?'NEW · EQUIPPED':'NEW ITEM'});$('#dropRevealBtn').hidden=true;$('#dropCloseBtn').hidden=false;resultDropRevealed=true;$('#dropCloseBtn').focus();
