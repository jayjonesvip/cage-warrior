-- Give the Fight page one authoritative championship matchup at a time.
-- Existing belts, challenges, history, profiles, and career saves remain intact.
...
begin
  ...
  left join lateral (
    select candidate.* from public.cage_profiles as candidate
    where candidate.retired_at is null and candidate.id<>champion.id
      and not exists (
        select 1 from public.cage_championship_challenges as bout
        where bout.championship_key='world' and bout.challenger_id=candidate.id
          and bout.challenge_day=v_today
      )
    order by md5(candidate.id::text || '|' || v_today::text),candidate.id
    limit 1
  ) as challenger on champion.id=v_user_id
...
'Returns the global belt, one daily title-shot state, one daily champion defense state, one deterministic active challenger, and career-scoped title/rematch history.';
...
begin
  ...
  if v_player_is_champion then
    ...
    select * into v_challenger from public.cage_profiles as candidate
    where candidate.retired_at is null and candidate.id<>v_champion.id
      and not exists (
        select 1 from public.cage_championship_challenges as prior
        where prior.championship_key='world' and prior.challenger_id=candidate.id
...
'Starts only the displayed championship matchup, enforces one contender attempt or champion defense per UTC day, and consumes a former champion rematch after one use.';
