(() => {
  ...
  const ICON_ASSET_VERSION = '2.5.153';
  ...
  const defaultState = {
    ...
    gear:[],gearCounts:{},gearSeed:Math.floor(Math.random()*0xffffffff),gearWinsSinceDrop:0,trainerOn:false,treatmentAvailable:true,dailyCounters:{date:'',fight:0,train:0,sparring:0,hustle:0,blackjack:0,cageDice:0,publicity:0,recovery:0},dailyOpponentWins:{date:'',wins:{}},blackjackHand:null,cageDiceResult:null,
  ...
  state = loadState();
  const fighterCities = [
    {id:'phoenix',name:'PHOENIX',region:'SOUTHWEST'},
  ...
  function renderSocial(){
    ...
    if(activeBioProfileId&&$('#fighterBioModal').classList.contains('open'))activeBioProfileId==='official-ceo'?renderCeoBioDetails():renderFighterBioInteractions(sharedSocialProfiles.find(profile=>profile.id===activeBioProfileId));
  }
  ...
  function renderHustle(){
    ensureDailyCounters();
    const fullTimeFighter=state.level>=5,blackjackUnlocked=state.level>=2,diceUnlocked=state.level>=4,hustleLeft=sessionsLeft('hustle',2),blackjackLeft=sessionsLeft('blackjack',1),diceLeft=sessionsLeft('cageDice',1),publicityLeft=sessionsLeft('publicity',1),blackjackActive=state.blackjackHand?.status==='playing',maxBlackjackBet=LOGIC.blackjackBetLimit(state.cash),maxDiceBet=LOGIC.cageDiceBetLimit(state.cash);
    $('#makeEndsMeetCard').hidden=fullTimeFighter;$('#fullTimeFighterNote').hidden=!fullTimeFighter;
    $('#hustleLimitText').textContent=`${hustleLeft} JOB${hustleLeft===1?'':'S'} LEFT`;
  ...
  function modalMeterSummary(reward,cost,detail=''){
    return `<div><small>REWARD</small><b>${reward}</b></div><div><small>COST</small><b>${cost}</b></div>${detail?`<span>${detail}</span>`:''}`;
  }
  ...
  function openSparringSession(){
    const session=activeSparringSession;if(!session)return;const {action,energyCost,cashCost}=session,modal=$('#sparringSessionModal'),meter=$('#sparringSessionMeter'),seconds=Math.max(0,Number(action.meterSeconds)||2),duration=seconds*1000,costText=`-${energyCost} ENERGY${cashCost?` · -$${cashCost}`:''}`;$('#sparringSessionTitle').textContent=action.title.toUpperCase();$('#sparringSessionStatus').textContent=`Live rounds in progress · ${seconds} seconds`;$('#sparringSessionIcon').innerHTML=gameIcon(action.asset||action.id,action.icon);$('#sparringSessionResult').innerHTML=modalMeterSummary('REVEALS AFTER SPAR',costText,'SESSION IN PROGRESS');$('#collectSparringResult').hidden=true;meter.classList.remove('working');meter.style.setProperty('--modal-meter-duration',`${seconds}s`);meter.setAttribute('aria-valuenow','0');modal.classList.add('open');modal.setAttribute('aria-hidden','false');updateUI();void meter.offsetWidth;requestAnimationFrame(()=>meter.classList.add('working'));sparringSessionTimer=setTimeout(resolveSparringSession,duration);
  }
  function resolveSparringSession(){
    sparringSessionTimer=null;if(!activeSparringSession)return;const {skills,gainText,perfect,injury,damage,bonusText,energyCost,cashCost}=activeSparringSession,rewardLabel=skills.length===1?'RANDOM SKILL REWARD':'RANDOM SKILL REWARDS',rewards=skills.map(skill=>`${skill.toUpperCase()} +${gainText}`).join(' · '),details=[rewardLabel,perfect?'ELITE SPAR':damage?`${damage} HEALTH LOST`:'SPAR COMPLETE',bonusText,injury?`${injury.icon} ${injury.name.toUpperCase()} · INJURED UNTIL MIDNIGHT`:null].filter(Boolean).join(' · '),costText=`-${energyCost} ENERGY${cashCost?` · -$${cashCost}`:''}`;$('#sparringSessionTitle').textContent='SPARRING COMPLETE';$('#sparringSessionStatus').textContent=`${sessionsLeft('sparring',2)} session${sessionsLeft('sparring',2)===1?'':'s'} left today.`;$('#sparringSessionMeter').setAttribute('aria-valuenow','100');$('#sparringSessionResult').innerHTML=modalMeterSummary(rewards,costText,details);$('#collectSparringResult').hidden=false;if(injury){sfx.crit();shake(true)}else{sfx.train();if(skills.length>1)vibrate([24,18,35])}flashTrainingStats(skills);toast(`${rewardLabel} · ${rewards}`,injury?'#ff8b94':perfect?'#f4c34a':'#77d13e','','',3200);requestAnimationFrame(()=>$('#collectSparringResult').focus());
  }
  ...
  function openRecoverySession(){
    const session=activeRecoverySession;if(!session)return;const {treatment,cashCost}=session,modal=$('#recoverySessionModal'),meter=$('#recoverySessionMeter'),seconds=Math.max(0,Number(treatment.meterSeconds)||2),duration=seconds*1000;$('#recoverySessionTitle').textContent=treatment.title.toUpperCase();$('#recoverySessionStatus').textContent=`Treatment in progress · ${seconds} seconds`;$('#recoverySessionIcon').innerHTML=gameIcon(treatment.id,treatment.icon);$('#recoverySessionResult').innerHTML=modalMeterSummary('REVEALS AFTER TREATMENT',`-$${cashCost}`,'TREATMENT IN PROGRESS');$('#collectRecoveryResult').hidden=true;meter.classList.remove('working');meter.style.setProperty('--modal-meter-duration',`${seconds}s`);meter.setAttribute('aria-valuenow','0');modal.classList.add('open');modal.setAttribute('aria-hidden','false');updateUI();void meter.offsetWidth;requestAnimationFrame(()=>meter.classList.add('working'));recoverySessionTimer=setTimeout(resolveRecoverySession,duration);
  }
  function resolveRecoverySession(){
    recoverySessionTimer=null;if(!activeRecoverySession)return;const {treatment,restored,cashCost}=activeRecoverySession,rewards=[restored.energy?`ENERGY +${formatGain(restored.energy)}`:'',restored.health?`HEALTH +${formatGain(restored.health)}`:''].filter(Boolean),rewardText=rewards.join(' · ');$('#recoverySessionTitle').textContent='RECOVERY COMPLETE';$('#recoverySessionStatus').textContent=`${treatment.title} finished · treatment collected.`;$('#recoverySessionMeter').setAttribute('aria-valuenow','100');$('#recoverySessionResult').innerHTML=modalMeterSummary(rewardText,`-$${cashCost}`,'RESOURCES RESTORED');$('#collectRecoveryResult').hidden=false;sfx.train();flashRecoveryResources(restored);toast(`RECOVERY REWARD · ${rewardText}`,'#78dcff','','',3200);requestAnimationFrame(()=>$('#collectRecoveryResult').focus());
  }
  ...
  function resolveHustleShift(){
    hustleShiftTimer=null;if(!activeHustleShift)return;const {action,totalCash,payDetail,bonusText}=activeHustleShift,result=$('#hustleShiftResult'),jobsLeft=sessionsLeft('hustle',2),detail=[payDetail,bonusText||'JOB COMPLETE · NO BONUS THIS TIME'].filter(Boolean).join(' · ');$('#hustleShiftTitle').textContent='JOB COMPLETE';$('#hustleShiftStatus').textContent=`${jobsLeft} job${jobsLeft===1?'':'s'} left today.`;$('#hustleShiftMeter').setAttribute('aria-valuenow','100');result.innerHTML=modalMeterSummary(`+$${totalCash} CASH`,`-${action.cost} ENERGY`,detail);$('#collectHustleShift').hidden=false;sfx.coin();requestAnimationFrame(()=>$('#collectHustleShift').focus());
  }
  ...
  function simulateRound(sim,round,planId,opening=null){
      if(sim.winner||round>3)return;const plan=planDefs.find(p=>p.id===planId)||planDefs[0],gamePlan=sim.gamePlan||state.fightPlanPreference,adapting=gamePlan.tactics==='adapt'&&round>1,adaptScale=adapting?(round===2?.5:1):1,edge=strategyEdge(plan.id,sim.o.tendency)*adaptScale+adaptationModifier(sim,round),familiarity=planFamiliarity(state.fighterStyle,plan.id),focusMod=fightFocusModifier(sim),fastPace=gamePlan.pace==='fast',aggressiveOffense=gamePlan.offense==='aggressive';sim.lastPlan=plan.id;sim.plans.push(plan.id);
  ...
  function buildResultDetails(f){
    ...
    $('#resultTape').innerHTML=`<div class="rt-name">${f.player.name}</div><div class="rt-vs">VS</div><div class="rt-name">${f.opp.name}</div><div class="rt-stats"><div><b>${formatStat(f.player.power)}/${formatStat(f.player.speed)}/${formatStat(f.player.chin)}/${formatStat(f.player.cardio)}</b><br><small>PWR · SPD · CHN · CAR</small></div><div class="rt-vs">RATINGS</div><div><b>${formatStat(f.opp.power)}/${formatStat(f.opp.speed)}/${formatStat(f.opp.chin)}/${formatStat(f.opp.cardio)}</b><br><small>PWR · SPD · CHN · CAR</small></div></div><div class="rt-focus">FIGHT PLAN · <b>${fightPlanLabel(f.gamePlan)}</b><br>FOCUS · <b>${f.focus}% ${focusTier(f.focus)}</b></div>`;
