(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.46';
  ...
  function renderGear(){
    ...
    const loadoutLimit=LOGIC.gearLoadoutLimit(state.level),perkLimit=LOGIC.perkLoadoutLimit(state.level),loadoutProgress=loadoutLimit<4?' · 4 SLOTS AT LVL 8':'',followerBonus=activePerkBonus('prestige'),recoveryBonus=activePerkBestBonus('healthRecoverySpeed'),perkEffects=[followerBonus?`+${followerBonus}% FIGHT FOLLOWERS`:'',recoveryBonus?`HEALTH EVERY ${Math.round((HEALTH_RECOVERY_INTERVAL-recoveryBonus)/1000)} SEC`:''].filter(Boolean),perkNames=state.equippedPerks.map(id=>gearItems.find(item=>item.id===id)?.name).filter(Boolean),perkPanel=`<section class="shop-section perk-loadout-panel" aria-labelledby="perk-loadout-title"><div class="shop-head"><div><b id="perk-loadout-title">PERK LOADOUT</b><small>Choose the bonuses powering your career</small></div><span class="shop-status"><span>${state.equippedPerks.length}/${perkLimit} ACTIVE</span><small>${perkLimit<4?'4 SLOTS AT LVL 8':'MAX SLOTS'}</small></span></div><div class="perk-loadout-body"><div class="perk-loadout-slots">${Array.from({length:perkLimit},(_,index)=>`<span class="${perkNames[index]?'filled':''}">${escapeHtml(perkNames[index]||'EMPTY PERK SLOT')}</span>`).join('')}</div><strong>${perkEffects.join(' · ')||'EQUIP PERKS TO ACTIVATE BONUSES'}</strong><small>New drops can improve this build. Equip Bling, Lifestyle, or Property &amp; Rides below.</small></div></section>`;
    $('#gearShop').innerHTML=perkPanel+order.map(cat=>{const items=owned.filter(g=>g.category===cat);if(!items.length)return '';const categoryTotal=gearItems.filter(g=>g.category===cat).length,collectionStatus=`${items.length} / ${categoryTotal} COLLECTIBLES`,loadoutStatus=cat==='Fight Gear'?`${state.equippedGear.length}/${loadoutLimit} EQUIPPED${loadoutProgress}`:'',status=`<span>${collectionStatus}</span>${loadoutStatus?`<small>${loadoutStatus}</small>`:''}`;return `<section class="shop-section" aria-labelledby="gear-${cat.replace(/[^a-z]+/gi,'-').toLowerCase()}"><div class="shop-head"><div><b id="gear-${cat.replace(/[^a-z]+/gi,'-').toLowerCase()}">${cat}</b><small>${subtitles[cat]}</small></div><span class="shop-status">${status}</span></div>${cat==='Fight Gear'?'<div class="loadout-note">Fight Gear uses separate combat slots. Duplicates do not stack.</div>':''}<div class="gear-grid">${items.map(g=>collectibleCardHtml(g)).join('')}</div></section>`}).join('');
  }
