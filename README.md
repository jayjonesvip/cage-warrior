...
Same-day opponent XP rules remain: full XP for the first same-level win, half XP for one runback, then zero XP. Lower-level opponents award no XP. Roster opponents in your tier can still award an Attribute Point. Winning against one also costs five percent of current followers, rounded up, and the matchup preview and result screen both disclose that fan backlash.
...
Roster opponents are not level-locked. Attribute Point rewards depend on opponent tier: higher-tier wins award two, same-tier wins one, and lower-tier wins none. Circuit opponents retain their level-based point rules. Lower-level wins earn no XP, incur the existing follower and Aura penalties, and do not qualify for Daily Heat or Victory Pack progress. Same-day XP reductions and drop limits remain in place. Old rank metadata cannot override these rules.
