(async () => {
  ...
  const ICON_ASSET_VERSION = '2.5.220';
  ...
  function renderTrain(){
    ...
    const recoveryOptions=recoveryDefs.map((a,i)=>{const treatmentFee=recoveryFee(a),quote=LOGIC.recoveryQuote(state,a,treatmentFee);return {a,i,treatmentFee,quote}}),energyOptions=recoveryOptions.filter(({a})=>a.energy>0),treatmentOptions=recoveryOptions.filter(({a})=>a.health>0),restAvailable=state.energy<state.maxEnergy,recoveryActionHtml=({a,i,treatmentFee,quote})=>{const gains=[a.energy?`+${a.energy}% ENERGY`:'',a.health?`+${a.health} HEALTH`:''].filter(Boolean).map(gain=>`<span class="recovery-gain">${gain}</span>`).join(''),seconds=Math.max(0,Number(a.meterSeconds)||2),status=a.freeRest?`FREE &middot; ${seconds} SECONDS`:quote.reason==='cash'?`NEED $${treatmentFee}`:quote.reason==='full'?(a.energy?'ENERGY FULL':'HEALTH FULL'):`-$${treatmentFee} &middot; ${seconds} SECONDS`;return `<button class="action${a.energy?' rest-action':''}" data-recovery="${i}" ${quote.ok?'':'disabled'}><div class="ico">${gameIcon(a.id,a.icon)}</div><div><h3>${a.title}</h3><p>${a.text}</p></div><div class="cost"><b class="recovery-gains">${gains}</b><small>${status}</small></div></button>`};
