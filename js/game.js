...
const dailyNews=globalThis.CAGE_DAILY_NEWS.create({element:$('#dailyCageNews'),client:SHARED_FEED,storage:localStorage,onSyncError:error=>{console.warn('Fight result sync pending:',error);toast('FIGHT RESULT SYNC PENDING · YOUR RESULT IS KEPT ON THIS DEVICE','#ffcf78')}});
  ...
  const ICON_ASSET_VERSION = '2.7.184';
  ...
  let centerFightRankingPending=false;
...
function sharedProfilePayload(){return {combatStats:Object.fromEntries(['power','speed','chin','cardio'].map(key=>[key,effectiveStat(key)])),defendingPlan:LOGIC.normalizeFightPlan(state.fightPlanPreference),fightSkinAura:Math.floor(effectiveAura()),city:state.fighterCity,archetype:state.fighterStyle,fighterAvatar:state.fighterAvatar,level:state.level,wins:state.wins,losses:state.losses,attributeTotal:Object.values(state.stats).reduce((sum,value)=>sum+Math.max(0,Number(value)||0),0),rankingHistory:state.rankingHistory}}
  ...
  function rankingProfiles(){
    const player={id:state.socialProfileId||'local-player',handle:state.name,city:state.fighterCity,archetype:state.fighterStyle,fighter_avatar:state.fighterAvatar,level:state.level,wins:state.wins,losses:state.losses,attributeTotal:Object.values(state.stats).reduce((sum,value)=>sum+Math.max(0,Number(value)||0),0),rankingHistory:state.rankingHistory};
  ...
  function navTo(screen){
    ...
    if(screen==='fight'&&currentScreen!=='fight')centerFightRankingPending=true;
  ...
  function renderFightLadderRow(opponent,fightsLeft){
    ...
    return `<button class="fight-ranking-row${champion?' champion':''}${opponent.championDefense?' title-defense':''}${opponent.circuitFallback||opponent.rookieShowcase||opponent.firstContract?' circuit':''}${locked?' locked':''}" type="button" data-fight-key="${escapeHtml(opponent.key)}" aria-label="${escapeHtml(opponent.name)}, ${opponent.network?`rank ${rank}`:`unranked, fighting out of ${country.name}`}, level ${opponent.tier}, ${reward.xp} XP and ${reward.points} Attribute Points for a win${reward.xpNote?`, ${reward.xpNote.toLowerCase()}`:''}${locked?`, ${lockCopy.toLowerCase()}`:''}. Open fighter profile."><strong class="fight-rank-position">${rank}</strong><span class="fight-rank-avatar"><img src="${escapeHtml(portrait)}" alt="" loading="lazy">${countryBadge}</span><span class="fight-rank-identity"><b>${escapeHtml(opponent.networkHandle?`@${opponent.networkHandle}`:opponent.name)}</b><small><span>${titleLabel}</span></small><em>PRO ${opponent.wins}-${opponent.losses} · LVL ${opponent.tier} · ${winPercentage}% WIN</em></span><span class="fight-rank-record"><small>RECORD</small><b>${opponent.wins}-${opponent.losses}</b></span><span class="fight-rank-level"><small>LEVEL</small><b>${opponent.tier}</b></span><span class="fight-rank-win"><small>WIN %</small><b>${winPercentage}%</b></span><span class="fight-rank-rewards"><b${reward.xp===0?' class="zero"':''}>+${reward.xp} XP</b><em${reward.points===0?' class="zero"':''}>+${reward.points} ATTR PT${reward.points===1?'':'S'}</em>${lockCopy?`<small>${lockCopy}</small>`:reward.xpNote?`<small class="xp-note${reward.xpPenalty?' penalty':''}">${reward.xpNote}</small>`:''}</span></button>`;
  }
  ...
  function renderOpponents(){
    if(centerFightRankingPending&&currentScreen==='fight')visibleFightRankingCount=Math.max(visibleFightRankingCount,(currentRanking().position||0)+10);
refreshOpponents();const fightsLeft=sessionsLeft('fight',DAILY_FIGHT_LIMIT),ranking=currentRanking(),showcase=opponents.filter(item=>item.rookieShowcase),contract=opponents.filter(item=>item.firstContract),ranked=opponents.filter(item=>item.network&&!combatStatsPending(item)),rankedCount=ranked.length+(ranking.profile&&ranking.position?1:0),circuit=opponents.filter(item=>!item.network&&!item.rookieShowcase&&!item.firstContract),circuitRematches=circuit.filter(item=>(item.winsVsPlayer||0)>0).length;$('#fightResetClock').hidden=fightsLeft>0;setLimitBadge('#fightLimitText',`${fightsLeft} ${fightsLeft===1?'FIGHT':'FIGHTS'} LEFT`);$('#rosterSummary').textContent=`${contract.length?`${contract.length} CONTRACT · `:''}${circuit.length} CIRCUIT · ${rankedCount} RANKED${ranking.position?` · YOUR RANK #${ranking.position}`:''}`;const showcaseRows=showcase.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join(''),contractRows=contract.length?`<div class="page-subhead fight-ranking-group circuit-group"><b>YOUR FIRST CONTRACT</b><span>VASO WIN UNLOCKED THIS FIGHT</span></div>${contract.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'',circuitSummary=circuitRematches?`${circuit.length-circuitRematches} FRESH · ${circuitRematches} REMATCH · PRO RECORD`:`${circuit.length} FRESH MATCHUPS · FULL XP · PRO RECORD`,circuitRows=circuit.length?`<div class="page-subhead fight-ranking-group circuit-group"><b>ON-LEVEL CAGE CIRCUIT</b><span>${circuitSummary}</span></div>${circuit.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'',rankedEntries=ranked.map(opponent=>({rank:opponent.worldRank||Number.MAX_SAFE_INTEGER,html:renderFightLadderRow(opponent,fightsLeft)}));if(ranking.profile&&ranking.position)rankedEntries.push({rank:ranking.position,html:renderPlayerRankingRow(ranking.profile,ranking.position),player:true});rankedEntries.sort((a,b)=>a.rank-b.rank);const visibleRankedEntries=rankedEntries.slice(0,visibleFightRankingCount),playerEntry=rankedEntries.find(entry=>entry.player);if(playerEntry&&!visibleRankedEntries.includes(playerEntry))visibleRankedEntries.push(playerEntry);visibleRankedEntries.sort((a,b)=>a.rank-b.rank);const rankedHasMore=visibleFightRankingCount<rankedEntries.length,loadedRankedCount=Math.min(visibleFightRankingCount,rankedEntries.length),rankingLoadMore=rankedHasMore?`<div class="fight-ranking-load-more" id="fightRankingsLoadMore" role="status">SHOWING ${loadedRankedCount} OF ${rankedEntries.length} · SCROLL FOR MORE</div>`:'';const rankedRows=rankedEntries.length?`<div class="page-subhead fight-ranking-group ranked-group"><b>WORLD RANKINGS</b><span>${rankedCount} RANKED FIGHTERS</span></div>${visibleRankedEntries.map(entry=>entry.html).join('')}${rankingLoadMore}`:'';$('#opponentList').innerHTML=opponents.length||ranking.profile?`<div class="fight-ranking-list">${showcaseRows}${contractRows}${circuitRows}${rankedRows}</div>`:`<div class="gear-empty"><b>BUILDING THE FIGHT CARD</b><span>New opponents will appear shortly.</span></div>`;
    if(centerFightRankingPending)requestAnimationFrame(centerPlayerInRankings);
  }

  function centerPlayerInRankings(){
    if(!centerFightRankingPending||currentScreen!=='fight'||fight||sharedSocialStatus!=='ready')return;
    const scroller=$('#opponentList'),row=scroller?.querySelector('.fight-ranking-row.player');
    if(!row||!scroller.clientHeight)return;
    const rowRect=row.getBoundingClientRect(),listRect=scroller.getBoundingClientRect();
    scroller.scrollTop=Math.max(0,scroller.scrollTop+rowRect.top-listRect.top-(scroller.clientHeight-rowRect.height)/2);
    centerFightRankingPending=false;
  }
  ...
  function createFight(o){
    ...
    return {o,rewardRanks,opponentPlan:LOGIC.normalizeFightPlan(o.defendingPlan),player:P,opp:O,playerAura:Math.floor(effectiveAura()),opponentAura:opponentFightAura(o),playerCondition:LOGIC.startingFightCondition(state.health,state.maxHealth),oppCondition:100,healthLost:0,rounds:[],timeline:[],totals:{player:emptyFightStats(),opp:emptyFightStats()},winner:null,method:'DECISION',submissionMove:null,finishRound:FIGHT_ROUNDS,finishClock:'0:00',ended:false,mode:'planned',gamePlan:Object.assign({},state.fightPlanPreference),planAssessment:null,focus:80,plans:[],lastPlan:state.fighterStyle||'striker',openingApproach:null,tendencyRevealed:true,deepRead:false,crisisUsed:false,cornerTowel:false,haymakerMiss:false,finalDecisionPending:false,lastChanceResolved:false,pendingMoment:null,resolvedMoments:[],roundIntros:[],rocked:{player:0,opp:0}};
  }
...
let championshipBout=null;if(o.globalChampionship){$('#opponentProfileFight').textContent='BOOKING TITLE FIGHT…';try{if(!await connectSharedSocial(true))throw new Error(sharedSocialError||'Cage Network connection required.');championshipBout=await SHARED_FEED.beginChampionshipBout(o.sourceProfileId);if(!championshipBout?.challenge_id)throw new Error('The title fight could not be booked.')}catch(error){if(o.championDefense&&String(error?.message||error).includes('That ranked fighter is not available for a title defense')){o=nonTitleOpponent(o);championshipBout=null;fight.o=o;fillTape(fight);toast('NON-TITLE RANKED FIGHT · YOUR BELT IS NOT ON THE LINE','#78dfff')}else{toast(fighterSessionMessage(error),'#ff766d');fillTape(fight);return}}}
  ...
  function finishFightSimulation(){
    ...
    dailyNews.record({resultId:fight.resultId,bout:state.wins+state.losses,at:new Date().toISOString(),won:win,opponent:o.networkHandle||o.name,playerRank:ranks.playerRank||0,opponentRank:o.network?ranks.opponentRank||0:0,seedId:o.seeded?o.sourceProfileId:null,playerLevel:playerLevelBefore});
