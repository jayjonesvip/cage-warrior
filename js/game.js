...
const dailyNews=globalThis.CAGE_DAILY_NEWS.create({element:$('#dailyCageNews'),tips:STRINGS.ticker,canShow:()=>!document.hidden&&!postFightPresentationBusy(),onDismiss:()=>requestAnimationFrame(showPostFightFollowup),client:SHARED_FEED,storage:localStorage,onSyncError:error=>{console.warn('Fight result sync pending:',error);toast('FIGHT RESULT SYNC PENDING · YOUR RESULT IS KEPT ON THIS DEVICE','#ffcf78')}});
  ...
  const ICON_ASSET_VERSION = '2.7.203';
  ...
  function opponentAvailable(o){return !o?.selfProfile&&!combatStatsPending(o)&&LOGIC.opponentAvailable(o,opponentContext())}
...
let championshipBout=null;if(o.globalChampionship){$('#opponentProfileFight').textContent='BOOKING TITLE FIGHT…';try{if(!await connectSharedSocial(true))throw new Error(sharedSocialError||'Cage Network connection required.');championshipBout=await SHARED_FEED.beginChampionshipBout(o.sourceProfileId);if(!championshipBout?.challenge_id)throw new Error('The title fight could not be booked.')}catch(error){if(o.championDefense&&String(error?.message||error).includes('That ranked fighter is not available for a title defense')){o=nonTitleOpponent(o);championshipBout=null;fight.o=o;fillTape(fight);toast('NON-TITLE FIGHT · YOUR BELT IS NOT ON THE LINE','#78dfff')}else{toast(fighterSessionMessage(error),'#ff766d');fillTape(fight);return}}}
  ...
  function finishFightSimulation(){
    ...
    if(calloutFight)lootNotes.push({kind:win?'milestone':'penalty',text:`CALLOUT ${win?'ANSWERED':'LOST'} · ${auraAppliedChange>0?'+':''}${auraAppliedChange} AURA`});else if(auraAppliedChange)lootNotes.push({kind:auraAppliedChange>0?'milestone':'penalty',text:`AURA · ${auraAppliedChange>0?'+':''}${auraAppliedChange}`});if(xp)gainXp(xp);xpResult.modifiers.forEach(text=>lootNotes.push({kind:'milestone',text}));if(win&&xpTier.tier==='exhausted'&&!calloutFight)lootNotes.push({kind:'penalty',text:`STALE MATCHUP · ${auraAppliedChange} AURA · NO VICTORY PACK PROGRESS`});const auraTitleAfter=LOGIC.auraTitle(state.aura);if(auraTitleAfter.key!==auraTitleBefore.key&&state.aura>auraBefore)lootNotes.push({kind:'milestone',text:`AURA STATUS · ${auraTitleAfter.label}`});
    ...
    openSocialCycle('fight',{win,opponent:o.name,method:fightMethodLabel(fight),winStreak:state.winStreak,title:'',lowerLevelWin:lowerLevelWin&&!calloutFight,followersLost:Math.abs(fans),fighterLevel:state.level,opponentLevel:o.min});
