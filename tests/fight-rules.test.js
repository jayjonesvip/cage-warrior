...
test('fight rule validation accepts safe edits and rejects unsafe values', () => {
  ...
  edited.energyCosts.levelsOneThroughTwoPerStartedRound = 8;
  ...
  assert.equal(normalized.energyCosts.levelsOneThroughTwoPerStartedRound, 8);
...
test('the static app loads and caches the editable fight configuration', () => {
  ...
  assert.match(serviceWorker, /'\.\/js\/fight-rules\.js\?v=2\.5\.179'/);
