(function(root){
  ...
  function createFightPlanFeature(options){
    ...
    function render(){
      ...
      const tacticsText=adapt?(signatureAnswers?'Your signature style already answers this matchup. Adapting trades familiarity for an unnecessary switch.':'This matchup calls for a style response. You will adjust in Rounds 2 and 3, and Focus controls execution.'):(signatureAnswers?`Your ${currentStyle()?.name||'signature'} style already answers this opponent and keeps full familiarity.`:`Sticking with ${currentStyle()?.name||'your signature style'} preserves familiarity, but does not directly answer their style.`);
