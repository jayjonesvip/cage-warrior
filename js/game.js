...
const dailyNews=globalThis.CAGE_DAILY_NEWS.create({element:$('#dailyCageNews'),tips:STRINGS.ticker,canShow:()=>!document.hidden&&!postFightPresentationBusy(),onDismiss:()=>requestAnimationFrame(showPostFightFollowup),client:SHARED_FEED,storage:localStorage,onSyncError:error=>{console.warn('Fight result sync pending:',error);toast('FIGHT RESULT SYNC PENDING · YOUR RESULT IS KEPT ON THIS DEVICE','#ffcf78')}});
  ...
  const ICON_ASSET_VERSION = '2.7.195';
...
$,logic:LOGIC,getState:()=>state,getRank:()=>rankName(),getChampionship:()=>sharedChampionship,setChampionship:setSharedChampionship,sharedFeed:SHARED_FEED,sharedUi:SHARED_UI,trackEvent,tap:()=>sfx.tap(),onEntered:()=>setTimeout(()=>{showCareerRestoreNotice();showRecoveryReport();if(!offerRookieShowcase())offerFirstContractOpponent();dailyNews.visit(currentScreen==='home')},180),onChampionshipChange:renderOpponents
  ...
  function renderRankMovement(){
    if(sharedSocialStatus==='ready'&&!state.attributePoints){const rank=currentRanking().position;$('#progressText').textContent=rank?'RANK #'+rank:'UNRANKED';$('#progressText').classList.toggle('rank-status',!!rank)}
    const movement=state.rankSnapshot,delta=movement?.delta||0,visible=sharedSocialStatus==='ready'&&delta!==0&&Date.now()-movement.changedAt<86400000&&!state.attributePoints,el=$('#rankMovement');
    el.hidden=!visible;el.className='rank-movement '+(delta>0?'rank-up':'rank-down');el.textContent=(delta>0?'↑ ':'↓ ')+Math.abs(delta);
    const label=`${delta>0?'Up':'Down'} ${Math.abs(delta)} place${Math.abs(delta)===1?'':'s'} since the previous ranking change`;
...
function sharedProfilePayload(){return {combatStats:Object.fromEntries(['power','speed','chin','cardio'].map(key=>[key,effectiveStat(key)])),defendingPlan:LOGIC.normalizeFightPlan(state.fightPlanPreference),fightSkinAura:Math.floor(effectiveAura()),city:state.fighterCity,archetype:state.fighterStyle,fighterAvatar:state.fighterAvatar,level:state.level,wins:state.wins,losses:state.losses,draws:state.draws||0,attributeTotal:Object.values(state.stats).reduce((sum,value)=>sum+Math.max(0,Number(value)||0),0),rankingHistory:state.rankingHistory}}
  ...
  async function connectSharedSocial(force=false){
    ...
    sharedSocialSyncPromise=(async()=>{
      ...
      sharedSocialStatus='ready';if(profilesLoaded&&seedsLoaded&&championshipResult.status==='fulfilled'){state.rankSnapshot=globalThis.CAGE_CAREER_FEEDBACK.rankSnapshot(state.rankSnapshot,currentRanking().position);renderRankMovement()}sharedSocialError='';sharedSocialNoticeShown=false;saveState();renderSocial();renderLanding();renderCareer();renderOpponents();$('#cageStatus').textContent=cageStatus();scheduleSharedSocialRefresh();if(state.pendingChampionshipResult&&!championshipSettlementPromise)queueMicrotask(()=>settleChampionshipResult());queueMicrotask(syncReferralRewards);queueMicrotask(syncDailyHeatPosts);requestAnimationFrame(showPendingTitleLoss);return true;
    })().catch(error=>{
  ...
  globalThis.cageRankingDebug=id=>{const profile=rankingProfiles().find(item=>item.id===id);return profile?LOGIC.rankingDebug(profile):null};
  ...
  function fightRewardRanks(opponent){return fight?.o===opponent&&combatLocked?fight.rewardRanks:{playerRank:currentRanking().position,opponentRank:opponent?.network?opponent.worldRank||0:0}}
  ...
  function renderPlayerRankingRow(profile,position){
    ...
    return `<button class="fight-ranking-row player${champion?' champion':''}" type="button" data-own-fighter-profile aria-label="Your fighter, rank #${position}, level ${profile.level}, record ${profile.wins}-${profile.losses}, ${winPercentage}% win rate"><strong class="fight-rank-position">#${position}</strong><span class="fight-rank-avatar"><img src="${escapeHtml(avatar)}" alt="" loading="lazy"></span><span class="fight-rank-identity"><b>@${escapeHtml(profile.handle)}</b><small><span>${champion?'WORLD CHAMPION · ':''}YOUR FIGHTER</span></small><em>PRO ${profile.wins}-${profile.losses} · LVL ${profile.level} · ${winPercentage}% WIN</em></span><span class="fight-rank-record"><small>RECORD</small><b>${profile.wins}-${profile.losses}</b></span><span class="fight-rank-level"><small>LEVEL</small><b>${profile.level}</b></span><span class="fight-rank-win"><small>WIN %</small><b>${winPercentage}%</b></span><span class="fight-rank-rewards"><b>YOUR FIGHTER</b><em>VIEW PROFILE</em></span></button>`;
  }
  function renderOpponents(){
refreshOpponents();const fightsLeft=sessionsLeft('fight',DAILY_FIGHT_LIMIT),ranking=currentRanking(),showcase=opponents.filter(item=>item.rookieShowcase),contract=opponents.filter(item=>item.firstContract),ranked=opponents.filter(item=>item.network&&!combatStatsPending(item)),rankedCount=ranked.length+(ranking.profile&&ranking.position?1:0),circuit=opponents.filter(item=>!item.network&&!item.rookieShowcase&&!item.firstContract),circuitRematches=circuit.filter(item=>(item.winsVsPlayer||0)>0).length;$('#fightResetClock').hidden=fightsLeft>0;setLimitBadge('#fightLimitText',`${fightsLeft} ${fightsLeft===1?'FIGHT':'FIGHTS'} LEFT`);$('#rosterSummary').textContent=`${contract.length?`${contract.length} CONTRACT · `:''}${circuit.length} CIRCUIT · ${rankedCount} RANKED${ranking.position?` · YOUR RANK #${ranking.position}`:''}`;const showcaseRows=showcase.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join(''),contractRows=contract.length?`<div class="page-subhead fight-ranking-group circuit-group"><b>YOUR FIRST CONTRACT</b><span>VASO WIN UNLOCKED THIS FIGHT</span></div>${contract.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'',circuitSummary=circuitRematches?`${circuit.length-circuitRematches} FRESH · ${circuitRematches} REMATCH · PRO RECORD`:`${circuit.length} FRESH MATCHUPS · FULL XP · PRO RECORD`,circuitRows=circuit.length?`<div class="page-subhead fight-ranking-group circuit-group"><b>ON-LEVEL CAGE CIRCUIT</b><span>START HERE · ${circuitSummary}</span></div>${circuit.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'',rankedEntries=ranked.map(opponent=>({rank:opponent.worldRank||Number.MAX_SAFE_INTEGER,html:renderFightLadderRow(opponent,fightsLeft)}));if(ranking.profile&&ranking.position)rankedEntries.push({rank:ranking.position,html:renderPlayerRankingRow(ranking.profile,ranking.position),player:true});rankedEntries.sort((a,b)=>a.rank-b.rank);const visibleRankedEntries=rankedEntries.slice(0,visibleFightRankingCount),playerEntry=rankedEntries.find(entry=>entry.player);if(playerEntry&&!visibleRankedEntries.includes(playerEntry))visibleRankedEntries.push(playerEntry);visibleRankedEntries.sort((a,b)=>a.rank-b.rank);const rankedHasMore=visibleFightRankingCount<rankedEntries.length,loadedRankedCount=Math.min(visibleFightRankingCount,rankedEntries.length),rankingLoadMore=rankedHasMore?`<div class="fight-ranking-load-more" id="fightRankingsLoadMore" role="status">SHOWING ${loadedRankedCount} OF ${rankedEntries.length} · SCROLL FOR MORE</div>`:'';const rankedRows=rankedEntries.length?`<div class="page-subhead fight-ranking-group ranked-group"><b>WORLD RANKINGS</b><span>${rankedCount} RANKED FIGHTERS</span></div>${visibleRankedEntries.map(entry=>entry.html).join('')}${rankingLoadMore}`:'';$('#opponentList').innerHTML=opponents.length||ranking.profile?`<div class="fight-ranking-list">${showcaseRows}${contractRows}${circuitRows}${rankedRows}</div>`:`<div class="gear-empty"><b>BUILDING THE FIGHT CARD</b><span>New opponents will appear shortly.</span></div>`;
  ...
  function createFight(o){
    const rewardRanks={playerRank:currentRanking().position,opponentRank:o.network?o.worldRank||0:0};
  ...
  function renderOpenGym(){
    ...
    $('#sparTargetChoices').innerHTML=filtered.length?filtered.map(choice=>`<button class="fighter-post-result" type="button" data-spar-target="${escapeHtml(choice.id)}" aria-pressed="${choice.id===state.sparringTarget}"><img src="${escapeHtml(choice.opponent.networkPortrait)}" alt=""><span><b>@${escapeHtml(choice.label)}</b><small>RANK #${choice.opponent.worldRank} · LEVEL ${choice.opponent.tier} · ${escapeHtml(fighterCityCode(choice.opponent.networkCity))} · ${choice.opponent.wins}-${choice.opponent.losses} · ${escapeHtml(choice.opponent.tag)}</small></span><em>${choice.id===state.sparringTarget?'SELECTED':'SELECT'}</em></button>`).join(''):`<div class="fighter-post-empty">${choices.length?'NO FIGHTERS MATCH THAT SEARCH':'NO RANKED FIGHTERS AVAILABLE · CONNECT TO THE CAGE NETWORK'}</div>`;
...
let championshipBout=null;if(o.globalChampionship){$('#opponentProfileFight').textContent='BOOKING TITLE FIGHT…';try{if(!await connectSharedSocial(true))throw new Error(sharedSocialError||'Cage Network connection required.');championshipBout=await SHARED_FEED.beginChampionshipBout(o.sourceProfileId);if(!championshipBout?.challenge_id)throw new Error('The title fight could not be booked.')}catch(error){if(o.championDefense&&String(error?.message||error).includes('That ranked fighter is not available for a title defense')){o=nonTitleOpponent(o);championshipBout=null;fight.o=o;fillTape(fight);toast('NON-TITLE RANKED FIGHT · YOUR BELT IS NOT ON THE LINE','#78dfff')}else{toast(fighterSessionMessage(error),'#ff766d');fillTape(fight);return}}}
  ...
  function finishFightSimulation(){
    ...
    state.rankingHistory=LOGIC.appendRankingResult(state.rankingHistory,fight.rankingSnapshot,fight.resultId,win?'win':'loss',{wins:state.wins-(win?1:0),losses:state.losses-(win?0:1),draws:state.draws});
    ...
    renderResultBonuses(lootNotes,{auraInRewards:!win});renderDailyHeatResult({won:win,lowerLevelWin,rankRepeatExhausted:win&&o.min<playerLevelBefore&&winsToday>=2&&LOGIC.higherRankedOpponent(ranks),forfeited:!!fight.forfeited,dailyStreakBefore,dailyBonusAwarded});
