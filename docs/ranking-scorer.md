...
`npm test` includes the acceptance fixtures in `tests/ranking-scorer.test.js`. `scripts/verify-ranking-sql.cjs` applies the migration twice to an isolated PostgreSQL engine, compares both implementations over 82 scenarios, and checks snapshot sync, seed-result retry behavior and shadow isolation. Install `@electric-sql/pglite` outside the project and set `PGLITE_MODULE_PATH` to that package path to run it without adding a production dependency.
