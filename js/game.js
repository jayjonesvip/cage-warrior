(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.145';
  ...
  function statsFighterBio({city,country,style,wins=0,losses=0,favorite=false,even=false}){
    ...
    return `${archetype} from ${location} with a ${wins}-${losses} pro record. ${favorite?'Comes in as the favorite on attributes.':even?'An evenly matched test on attributes.':'Comes in as the underdog on attributes.'}`;
  }
  ...
  function fillTape(f){
    ...
    const edge=(f.player.power+f.player.speed+f.player.chin+f.player.cardio)-(f.opp.power+f.opp.speed+f.opp.chin+f.opp.cardio),playerFavorite=edge>=4,oppFavorite=edge<=-4;$('#tapeStatsPlayerMeta').textContent=statsFighterBio({city:state.fighterCity,style:state.fighterStyle,wins:state.wins,losses:state.losses,favorite:playerFavorite,even:!playerFavorite&&!oppFavorite});$('#tapeStatsOppMeta').textContent=statsFighterBio({city:f.o.networkCity,country:f.o.country,style:f.o.tendency,wins:f.o.wins,losses:f.o.losses,favorite:oppFavorite,even:!playerFavorite&&!oppFavorite});
