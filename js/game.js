(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.87';
  ...
  const gearUndiscoveredExpanded=new Set();
  let fight = null;
  let fightTimers = [];
  ...
  function renderCareer(){
    ...
    const aura=clamp(Math.floor(Number(effectiveAura())||0),0,100),auraTitle=LOGIC.auraTitle(aura),auraToken=$('#careerAuraToken'),auraGear=activePerkBonus('auraBonus');$('#careerAuraTitle').textContent=auraTitle.label;$('#careerAuraValue').textContent=`${aura} / 100${auraGear?` · +${auraGear} BLING`:''}`;auraToken.className=`career-token career-aura-token aura-tier-${auraTitle.key}`;
  ...
  function collectibleCardHtml(item,{dropStatus=''}={}){
    ...
    if(equipped&&!dropStatus)return `<button class="gear collectible-card equipped equipped-compact rarity-card-${rarityClass}" type="button" data-equip="${item.id}" aria-label="Unequip ${escapeHtml(item.name)}"><span class="equipped-card-thumb">${gameIcon(item.iconName||item.id,item.icon,item.assetExt)}</span><span class="equipped-card-copy"><h3>${escapeHtml(item.name)} <em>· ${rarity}</em></h3><small>MIN LVL ${item.minLevel||1}</small></span><span class="equipped-card-check" aria-hidden="true">✓</span></button>`;
    const action=dropStatus?`<div class="gear-status">${escapeHtml(dropStatus)}</div>`:levelLocked?`<button class="equip-btn" type="button" disabled>LOCKED · LVL ${item.minLevel}</button>`:`<button class="equip-btn" type="button" data-equip="${item.id}" aria-label="Equip ${escapeHtml(item.name)}">EQUIP</button>`;
    ...
    return `<div class="gear collectible-card owned rarity-card-${rarityClass} ${levelLocked?'level-locked':''} ${dropStatus?'drop-claim-card':''}" data-collectible-flip data-collectible-id="${item.id}" tabindex="0" aria-label="${cardLabel}" aria-pressed="false"><div class="collectible-flip"><div class="collectible-side collectible-front" aria-hidden="false">${lockIcon}<div class="gear-top"><span class="rarity-tag">${rarity}</span></div><div class="gear-hero"><span class="gear-flair"></span><span class="equip-burst"></span><div class="gear-icon">${gameIcon(item.iconName||item.id,item.icon,item.assetExt)}</div></div><div class="gear-copy"><h3>${escapeHtml(item.name)}</h3></div><div class="gear-footer"><span class="level-tag">MIN LVL ${item.minLevel||1}</span>${action}</div></div>${collectibleBackHtml(item,rarity)}</div></div>`;
  }
  ...
  function undiscoveredBlockHtml(category,items){
    if(!items.length)return '';
    const slug=gearCategorySlug(category),expanded=gearUndiscoveredExpanded.has(slug),rarities=['COMMON','RARE','EPIC','LEGENDARY'],groups=rarities.map(rarity=>{const rarityItems=items.filter(item=>(item.rarity||'COMMON')===rarity);return rarityItems.length?`<section class="undiscovered-rarity-group rarity-undiscovered-${rarity.toLowerCase()}" aria-label="${rarity.toLowerCase()} undiscovered collectibles"><div class="undiscovered-rarity-head"><b>${rarity}</b><span>${rarityItems.length}</span></div><div class="gear-grid gear-undiscovered-grid">${rarityItems.map((item,index)=>undiscoveredCardHtml(category,rarity,index)).join('')}</div></section>`:''}).join('');
    return `<div class="gear-undiscovered-block"><button class="gear-undiscovered-toggle" type="button" data-gear-undiscovered="${slug}" aria-expanded="${expanded}" aria-controls="undiscovered-${slug}"><span><b>UNDISCOVERED</b><small>${items.length} REMAINING</small></span><em>${expanded?'HIDE':'SHOW'} <i aria-hidden="true"></i></em></button><div class="gear-undiscovered-contents" id="undiscovered-${slug}"${expanded?'':' hidden'}>${groups}</div></div>`;
  }
  ...
  function toggleUndiscoveredGear(slug){if(!slug)return;if(gearUndiscoveredExpanded.has(slug))gearUndiscoveredExpanded.delete(slug);else gearUndiscoveredExpanded.add(slug);renderGear();sfx.tap();requestAnimationFrame(()=>$(`[data-gear-undiscovered="${slug}"]`)?.focus())}
  ...
  function showPendingPostFightText(){
    if(!pendingPostFightText)return false;const modal=$('#postFightMessageModal');if(modal.classList.contains('open'))return true;const {contact,messages}=pendingPostFightText,avatar=$('#postFightMessageAvatar');avatar.src=contact.avatar;avatar.alt=contact.name;$('#postFightMessageName').textContent=contact.name;$('#postFightMessageStatus').textContent=`${contact.relationship} · JUST NOW`;$('#postFightMessageList').innerHTML=`<div class="post-fight-message-time">JUST NOW</div>${messages.map(message=>`<div class="post-fight-message-row in"><img src="${escapeHtml(contact.avatar)}" alt=""><div class="post-fight-message-bubble">${escapeHtml(message)}</div></div>`).join('')}`;$('#postFightMessageInput').value='';modal.classList.add('open');modal.setAttribute('aria-hidden','false');trackEvent('post_fight_text_received',{contact_id:contact.id});sfx.tap();requestAnimationFrame(()=>$('#postFightMessageClose').focus());return true;
  }
  function closePostFightText(){const modal=$('#postFightMessageModal');if(!modal.classList.contains('open'))return;pendingPostFightText=null;modal.classList.remove('open');modal.setAttribute('aria-hidden','true');sfx.tap();requestAnimationFrame(showPostFightFollowup)}
  ...
  document.addEventListener('click',e=>{
    ...
    const undiscoveredToggle=e.target.closest('[data-gear-undiscovered]');if(undiscoveredToggle){toggleUndiscoveredGear(undiscoveredToggle.dataset.gearUndiscovered);return}
