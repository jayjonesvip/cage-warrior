-- Full rolling roundup: retain legacy upset for older clients and include every grouped event.
begin;
...
begin
  ...
  select jsonb_build_object(
    ...
    'titles',coalesce((select jsonb_agg(t) from (
      ...
      order by max(h.created_at),h.action,h.champion_id
    ) t),'[]'::jsonb),
    'upsets',coalesce((select jsonb_agg(u) from (
      select fighter_handle as winner,opponent_handle as opponent,count(*) as count
      ...
      group by fighter_handle,opponent_handle order by min(occurred_at),fighter_handle,opponent_handle
    ) u),'[]'::jsonb),
