...
test('fight rankings hide unsynced fighters and restore them after sync in their named tiers',()=>{
 ...
 const ctx={$,LOGIC,highestRosterLevel:()=>20,opponents:[waiting,ready],resetFightListPending:false,visibleFightRankingCount:30,DAILY_FIGHT_LIMIT:12,refreshOpponents(){},renderFirstWinNextFight(){},sessionsLeft:()=>12,currentFighterList:()=>({fighters:[waiting,ready],profile:null,position:0}),combatStatsPending:o=>!o.statsReady,setLimitBadge(){},renderFightLadderRow:o=>{shown.push(o.worldRank);return `<button>${o.key}</button>`}};
...
test('champion precedes Circuit and descending named tiers, including the player beyond the loaded window',()=>{
 ...
 const ctx={$,LOGIC,highestRosterLevel:()=>20,opponents:[fighter('weak',10,900),fighter('champ',3,30,{isChampion:true}),fighter('strong',10,30),fighter('highest',20,30),{key:'circuit',tier:2}],resetFightListPending:false,visibleFightRankingCount:2,DAILY_FIGHT_LIMIT:12,refreshOpponents(){},renderFirstWinNextFight(){},sessionsLeft:()=>12,currentFighterList:()=>({profile:own,position:5}),combatStatsPending:()=>false,setLimitBadge(){},renderFightLadderRow:(o,fights,position)=>{labels.set(o.key,position);return `<button>${o.key}</button>`},renderPlayerRankingRow:p=>{labels.set('self',p.levelPosition);return '<button>self</button>'}};
 vm.createContext(ctx);vm.runInContext(source.slice(source.indexOf('  function renderOpponents('),source.indexOf('  function resetFightListScroll(')),ctx);
 ctx.renderOpponents();
 ...
 assert.deepEqual([...html.matchAll(/<b>(WORLD CHAMPION|ON-LEVEL CAGE CIRCUIT|[A-Z ]+ · TIER \d+)<\/b>/g)].map(m=>m[1]),['WORLD CHAMPION','ON-LEVEL CAGE CIRCUIT','ELITE · TIER 5','CHALLENGERS · TIER 3']);
