...
test('silent and musical entrances both wait seven seconds and prevent double starts',()=>{
  for(const enabled of [true,false]){
    const h=harness(enabled);h.context.confirmFightPlan();h.context.confirmFightPlan();assert.equal(h.jobs.length,5);
    h.jobs.find(j=>j.ms===1750).fn();assert.match(h.$('#walkoutMessage').textContent,enabled?/music hits/:/silently/);
    h.jobs.filter(j=>j.ms<7000).forEach(j=>j.fn());assert.ok(!h.calls.includes('fight'));
    h.jobs.find(j=>j.ms===7000).fn();assert.equal(h.calls.at(-1),'fight');assert.equal(h.context.walkoutPending,false);
  }
});
test('a cancelled entrance cannot start combat and all crowd tiers have copy',()=>{
  const h=harness(false);h.context.confirmFightPlan();h.context.fight=null;h.context.walkoutPending=false;h.jobs.find(j=>j.ms===7000).fn();assert.ok(!h.calls.includes('fight'));
