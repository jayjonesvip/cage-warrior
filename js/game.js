(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.68';
  ...
  function sendPostFightTextReply(event){event.preventDefault();const input=$('#postFightMessageInput'),message=input.value.trim();if(!message)return;const row=document.createElement('div'),bubble=document.createElement('div');row.className='post-fight-message-row out';bubble.className='post-fight-message-bubble';bubble.textContent=message;row.append(bubble);$('#postFightMessageList').append(row);row.scrollIntoView({block:'nearest'});input.value='';input.disabled=true;$('#postFightMessageSend').disabled=true;trackEvent('post_fight_text_replied',{contact_id:pendingPostFightText?.contact?.id||'unknown'});sfx.tap()}
  ...
  $('#postFightMessageClose').addEventListener('click',closePostFightText);$('#postFightMessageComposer').addEventListener('submit',sendPostFightTextReply);$('#postFightMessageModal').addEventListener('keydown',event=>{if(event.key==='Escape')closePostFightText()});
