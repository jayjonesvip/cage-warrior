(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.78';
  ...
  let gearFilter = (()=>{try{const saved=sessionStorage.getItem(GEAR_FILTER_KEY);return ['combat','bling','property','lifestyle','all'].includes(saved)?saved:'all'}catch{return 'all'}})();
  let fight = null;
  let fightTimers = [];
  ...
  function applyOfflineRecovery(){
    const now=Date.now(),recovered=LOGIC.passiveRecovery(state,now,OFFLINE_RECOVERY_CAP,{energy:energyRecoveryInterval(),health:healthRecoveryInterval()}),followerGrowth=LOGIC.passiveFollowerGrowth(state,now,FOLLOWER_OFFLINE_CAP);
  ...
  function perkValue(id){const item=gearItems.find(entry=>entry.id===id);return (Number(item?.healthRecoverySpeed)||0)*100+(Number(item?.prestige)||0)}
  function ensureLoadout(){const gearLimit=LOGIC.gearLoadoutLimit(state.level),perkLimit=LOGIC.perkLoadoutLimit(state.level),ownedFight=state.gear.filter(id=>{const g=gearItems.find(x=>x.id===id);return g&&g.category==='Fight Gear'}),ownedPerks=state.gear.filter(id=>{const g=gearItems.find(x=>x.id===id);return g&&g.category!=='Fight Gear'});state.equippedGear=state.equippedGear.filter(id=>ownedFight.includes(id)).slice(0,gearLimit);state.equippedPerks=state.equippedPerks.filter(id=>ownedPerks.includes(id)).slice(0,perkLimit);if(!state.equippedGear.length&&ownedFight.length)state.equippedGear=ownedFight.slice(0,gearLimit)}
  ...
  function activePerkBestBonus(prop){return state.equippedPerks.reduce((best,id)=>{const item=gearItems.find(entry=>entry.id===id);return Math.max(best,Number(item?.[prop])||0)},0)}
  function energyRecoveryInterval(){return Math.max(4000,ENERGY_RECOVERY_INTERVAL-ownedBestBonus('energyRecoverySpeed'))}
  function healthRecoveryInterval(){return Math.max(30000,HEALTH_RECOVERY_INTERVAL-activePerkBestBonus('healthRecoverySpeed'))}
  ...
  function updateUI(){
    ...
    $('#auraText').textContent=Math.floor(state.aura);
  ...
  function renderCareer(){
    ...
    const aura=clamp(Math.floor(Number(state.aura)||0),0,100),auraTitle=LOGIC.auraTitle(aura),auraToken=$('#careerAuraToken');$('#careerAuraTitle').textContent=auraTitle.label;$('#careerAuraValue').textContent=`${aura} / 100`;auraToken.className=`career-token career-aura-token aura-tier-${auraTitle.key}`;
  ...
  function renderGearLoadoutDock(){
    ...
    const group=(loadout,limit,label)=>`<div class="gear-loadout-group" role="group" aria-label="${label}">${Array.from({length:4},(_,index)=>{const item=gearItems.find(entry=>entry.id===loadout[index]),locked=index>=limit,rarity=String(item?.rarity||'').toLowerCase(),slotLabel=locked?`${label} slot ${index+1} unlocks at Level 8`:item?`${label} slot ${index+1}: ${item.name}, ${item.rarity||'common'}`:`Empty ${label} slot ${index+1}`;return locked?`<button class="gear-loadout-thumb locked" type="button" aria-label="${escapeHtml(slotLabel)}" disabled><i class="slot-lock" aria-hidden="true"></i></button>`:`<span class="gear-loadout-thumb${item?` filled rarity-slot-${rarity}`:''}" aria-label="${escapeHtml(slotLabel)}" title="${escapeHtml(slotLabel)}">${item?`${gameIcon(item.iconName||item.id,item.icon,item.assetExt)}<i class="loadout-rarity-dot" aria-hidden="true"></i>`:''}</span>`}).join('')}</div>`;
  ...
  function renderGear(){
    ...
    const subtitles={'Fight Gear':'Equippable combat upgrades','Bling':'Follower perks','Lifestyle':'Follower and recovery perks','Property & Rides':'Career status perks'};
    renderGearLoadoutDock();
    $$('#gearFilterTabs [data-gear-filter]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.gearFilter===gearFilter)));
    ...
    const loadoutLimit=LOGIC.gearLoadoutLimit(state.level),perkLimit=LOGIC.perkLoadoutLimit(state.level),loadoutProgress=loadoutLimit<4?' · 4 SLOTS AT LVL 8':'',perkProgress=perkLimit<4?' · 4 SLOTS AT LVL 8':'';
    const visibleOrder=gearFilter==='combat'?['Fight Gear']:gearFilter==='bling'?['Bling']:gearFilter==='property'?['Property & Rides']:gearFilter==='lifestyle'?['Lifestyle']:order,sections=visibleOrder.map(cat=>{const items=owned.filter(g=>g.category===cat);if(!items.length)return '';const categoryTotal=gearItems.filter(g=>g.category===cat).length,collectionStatus=`${items.length} / ${categoryTotal} COLLECTIBLES`,fightGear=cat==='Fight Gear',loadoutStatus=fightGear?`${state.equippedGear.length}/${loadoutLimit} EQUIPPED${loadoutProgress}`:`${state.equippedPerks.length}/${perkLimit} PERKS EQUIPPED${perkProgress}`,status=`<span>${collectionStatus}</span><small>${loadoutStatus}</small>`,activeItems=fightGear?items.filter(item=>state.equippedGear.includes(item.id)):items.filter(item=>state.equippedPerks.includes(item.id)),availableItems=items.filter(item=>!activeItems.includes(item)),followerBonus=fightGear?0:activeItems.reduce((sum,item)=>sum+(Number(item.prestige)||0),0),recoveryBonus=fightGear?0:activeItems.reduce((best,item)=>Math.max(best,Number(item.healthRecoverySpeed)||0),0),activeEffects=[followerBonus?`+${followerBonus}% FIGHT FOLLOWERS`:'',recoveryBonus?`HEALTH EVERY ${Math.round((HEALTH_RECOVERY_INTERVAL-recoveryBonus)/1000)} SEC`:''].filter(Boolean),note=fightGear?'Fight Gear uses separate combat slots. Duplicates do not stack.':activeEffects.length?`ACTIVE · ${activeEffects.join(' · ')}`:'Equip a collectible below to activate its career perk.',equippedBlock=activeItems.length?`<div class="gear-equipped-block"><div class="gear-subhead">YOUR LOADOUT <span>${activeItems.length}</span></div><div class="gear-grid gear-equipped-grid">${activeItems.map(g=>collectibleCardHtml(g)).join('')}</div></div>`:'',availableHeader=activeItems.length&&availableItems.length?'<div class="gear-subhead gear-available-subhead">AVAILABLE</div>':'';return `<section class="shop-section" aria-labelledby="gear-${cat.replace(/[^a-z]+/gi,'-').toLowerCase()}"><div class="shop-head"><div><b id="gear-${cat.replace(/[^a-z]+/gi,'-').toLowerCase()}">${cat}</b><small>${subtitles[cat]}</small></div><span class="shop-status">${status}</span></div><div class="loadout-note${fightGear?'':' perk-category-note'}">${note}</div>${equippedBlock}${availableHeader}${availableItems.length?`<div class="gear-grid">${availableItems.map(g=>collectibleCardHtml(g)).join('')}</div>`:''}</section>`}).join('');
    $('#gearShop').innerHTML=sections||`<div class="gear-empty"><b>NO ${gearFilter.toUpperCase()} ITEMS YET</b><span>Choose All to view the rest of your collection.</span></div>`;
  }
  function setGearFilter(filter){if(!['combat','bling','property','lifestyle','all'].includes(filter)||gearFilter===filter)return;gearFilter=filter;try{sessionStorage.setItem(GEAR_FILTER_KEY,filter)}catch{/* session-only fallback remains in memory */}renderGear();sfx.tap()}
  ...
  function openLoadoutFullDialog(trigger,perk=false){const modal=$('#loadoutFullModal'),limit=perk?LOGIC.perkLoadoutLimit(state.level):LOGIC.gearLoadoutLimit(state.level),label=perk?'PERK LOADOUT':'FIGHT GEAR';loadoutDialogReturnFocus=trigger&&typeof trigger.focus==='function'?trigger:null;$('#loadoutFullKicker').textContent=`${label} · ${limit}/${limit} SLOTS`;$('#loadoutFullDescription').textContent=limit<4?`Your rookie ${perk?'perk':'Fight Gear'} loadout has two active slots. Unequip one item to make room, or reach Level 8 to unlock four slots.`:`Unequip one ${perk?'perk':'Fight Gear item'} before equipping another. Duplicate items still count as one active bonus.`;modal.classList.add('open');modal.setAttribute('aria-hidden','false');sfx.lose();requestAnimationFrame(()=>$('#loadoutFullOk').focus())}
  ...
  function toggleEquip(id,trigger){const g=gearItems.find(x=>x.id===id);if(!g||!state.gear.includes(id))return;const perk=g.category!=='Fight Gear',loadout=perk?state.equippedPerks:state.equippedGear,limit=perk?LOGIC.perkLoadoutLimit(state.level):LOGIC.gearLoadoutLimit(state.level),eventName=perk?'perk':'gear',at=loadout.indexOf(id);if(at>=0){loadout.splice(at,1);trackEvent(`${eventName}_unequipped`,{gear_id:id,gear_rarity:g.rarity.toLowerCase()});sfx.tap();updateUI();return}if(loadout.length>=limit){openLoadoutFullDialog(trigger,perk);return}loadout.push(id);trackEvent(`${eventName}_equipped`,{gear_id:id,gear_rarity:g.rarity.toLowerCase()});initAudio();sfx.crit();const card=trigger&&trigger.closest('.gear');if(card){card.classList.add('equip-bursting');trigger.textContent='EQUIPPED!';trigger.disabled=true;saveState();setTimeout(updateUI,680)}else updateUI()}
