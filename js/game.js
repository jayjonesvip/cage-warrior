(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.170';
  ...
  function confirmFightPlan(){
    ...
    message.textContent='Plan locked. You leave the locker room.';progress.setAttribute('aria-valuenow','0');
    ...
    scheduleFight(()=>{message.textContent=musicOn?'Your entrance music hits. You head toward the cage.':'You walk silently toward the cage.';progress.setAttribute('aria-valuenow','25')},2250);
    scheduleFight(()=>{message.textContent=walkoutCrowdMessage(effectiveAura());progress.setAttribute('aria-valuenow','50')},4500);
    scheduleFight(()=>{message.textContent='You step into the cage. Time to go to work.';progress.setAttribute('aria-valuenow','75')},6750);
    scheduleFight(()=>walkoutMusic.stop(),8800);
    scheduleFight(()=>{if(fight!==bout||!walkoutPending)return;progress.setAttribute('aria-valuenow','100');modal.classList.remove('open');modal.setAttribute('aria-hidden','true');walkoutPending=false;fightPlanFeature.confirm()},9000);
  }
