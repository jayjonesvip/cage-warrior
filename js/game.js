(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.21';
  ...
  function buildResultDetails(f){
    ...
    const resultTape=$('#resultTape'),playerPortrait=currentAvatar()?.asset||$('#heroFighterArt').src,opponentPortrait=silhouetteForOpponent(f.o);resultTape.style.setProperty('--player-accent',fighterAccent(state.fighterCity));resultTape.style.setProperty('--opponent-accent',f.o.network?fighterAccent(f.o.networkCity):f.o.color||DEFAULT_FIGHTER_ACCENT);resultTape.innerHTML=`<div class="rt-name player">${escapeHtml(f.player.name)}</div><div class="rt-vs">VS</div><div class="rt-name opponent">${escapeHtml(f.opp.name)}</div><div class="rt-portrait player"><img src="${escapeHtml(playerPortrait)}" alt="${escapeHtml(f.player.name)}"></div><div class="rt-vs rt-portrait-vs">VS</div><div class="rt-portrait opponent"><img src="${escapeHtml(opponentPortrait)}" alt="${escapeHtml(f.opp.name)} ${f.o.network||f.o.portraitAsset?'portrait':'silhouette'}"></div><div class="rt-stats"><div class="player"><b>${formatStat(f.player.power)}/${formatStat(f.player.speed)}/${formatStat(f.player.chin)}/${formatStat(f.player.cardio)}</b><br><small>PWR · SPD · CHN · CAR</small></div><div class="rt-vs">RATINGS</div><div class="opponent"><b>${formatStat(f.opp.power)}/${formatStat(f.opp.speed)}/${formatStat(f.opp.chin)}/${formatStat(f.opp.cardio)}</b><br><small>PWR · SPD · CHN · CAR</small></div></div><div class="rt-focus">FIGHT PLAN · <b>${fightPlanLabel(f.gamePlan)}</b><br>PLAN GRADE · <b class="plan-grade ${planAssessment.grade.toLowerCase()}">${planAssessment.grade}</b><br>FOCUS · <b>${f.focus}% ${focusTier(f.focus)}</b></div>`;
