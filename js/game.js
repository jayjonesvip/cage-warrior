(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.144';
  ...
  function renderStatsFightSkin(selector,aura){
    ...
    container.innerHTML=`<div class="tape-stats-kit-items">${[['gloves','gloves'],['wraps','hand wraps'],['mouthguard','mouthguard'],['shorts','shorts']].map(([key,label])=>`<img src="${skin[key+'Asset']}?v=${ICON_ASSET_VERSION}" alt="${skin.colorName} ${label}">`).join('')}</div><b>${skin.label}</b><small>FIGHT SKIN · COSMETIC</small>`;
  }
  function fillTape(f){
