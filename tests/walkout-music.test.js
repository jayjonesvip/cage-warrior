...
test('music is opt-in and stops scheduling on mute, exit, and background',async()=>{
...
test('locker music is wired before the game and walkout ends before combat',()=>{
  ...
  assert.match(game,/scheduleFight\(\(\)=>walkoutMusic.stop\(\),2500\)/);
  assert.match(game,/scheduleFight\(\(\)=>\{walkoutPending=false;fightPlanFeature.confirm\(\)\},3000\)/);
