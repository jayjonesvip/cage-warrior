(() => {
  ...
  const ICON_ASSET_VERSION = '2.5.168';
  ...
  function renderCareer(){
    ...
    $('#app').style.setProperty('--fighter-accent',city?.accent||DEFAULT_FIGHTER_ACCENT);
  ...
  function renderOpponents(){
    ...
    const renderCard=o=>{
      ...
      return `<article class="opponent ${status} ${o.network?'network':''} ${rivalFight?'rematch':''} ${dailyExhausted?'daily-exhausted':''}"${o.network?` style="--hometown-accent:${fighterAccent(o.networkCity)}"`:''} data-card-flip="true" data-card-name="${safeName}" tabindex="0" aria-label="${safeName} fighter card${dailyExhausted?', daily fight limit reached':rivalFight?', rival fight available':tauntable?', taunt available':''}. Tap for details." aria-pressed="false">
  ...
  function fillTape(f){
    ...
    const playerAccent=fighterAccent(state.fighterCity),opponentAccent=f.o.network?fighterAccent(f.o.networkCity):f.o.color||DEFAULT_FIGHTER_ACCENT;$('#tapeStage .player-card').style.setProperty('--fighter-accent',playerAccent);$('#tapeStage .opponent-card').style.setProperty('--fighter-accent',opponentAccent);
