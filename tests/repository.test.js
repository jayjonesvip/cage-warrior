...
test('the branded landing page gates the game and offers the correct career entry paths', () => {
  ...
  assert.match(page, /<main class="landing-card">[\s\S]*class="landing-watermark" src="assets\/app-icon-512\.png\?v=2\.5\.206" alt="" aria-hidden="true"/);
...
test('career identity keeps hometown informational and uses one shared real-player championship', () => {
  ...
  assert.match(serviceWorker, /\.\/assets\/icons\/title-world\.png\?v=2\.5\.206/);
...
test('the simplified World Championship is a daily server-authoritative flow', () => {
  ...
  assert.match(script, /message=`Reach Level \$\{championLevel\} to earn a shot/);
...
test('career fights use a reversible tale-of-the-tape preview before locker-room planning', () => {
  ...
  assert.match(css, /\.live-card\{[^}]*background-image:url\("\.\.\/assets\/cage-grind-octagon-transparent\.png\?v=2\.5\.206"\)[^}]*background-position:center 62%/);
  assert.match(css, /\.live-card\.decision-active\{[^}]*background-image:linear-gradient\(#030914f6,#030914f6\),url\("\.\.\/assets\/cage-grind-octagon-transparent\.png\?v=2\.5\.206"\)[^}]*background-position:center,center 62%/);
...
test('booked fights resolve a 50-50 locker-room Focus encounter after planning', () => {
  ...
  assert.match(page, /id="focusStage"[\s\S]*class="fight-plan-locker-room"[\s\S]*class="fight-plan-locker-art" src="assets\/focus-locker-room\.jpg\?v=2\.5\.206"/);
  ...
  assert.match(serviceWorker, /\.\/assets\/focus-locker-room\.jpg\?v=2\.5\.206/);
  assert.match(serviceWorker, /\.\/assets\/contact-mom\.jpg\?v=2\.5\.206/);
  assert.match(serviceWorker, /\.\/assets\/contact-wife\.jpg\?v=2\.5\.206/);
  assert.match(serviceWorker, /\.\/assets\/contact-brother-tommy\.png\?v=2\.5\.206/);
  assert.match(serviceWorker, /\.\/assets\/contact-agent-carl\.png\?v=2\.5\.206/);
  assert.match(serviceWorker, /\.\/assets\/contact-grandma\.jpg\?v=2\.5\.206/);
...
test('the landing page preserves Cage Grind history without treating the old game as a technical blueprint', () => {
  ...
  assert.match(originsPage, /css\/origins\.css\?v=2\.5\.206/);
  ...
  assert.match(serviceWorker, /'\.\/css\/origins\.css\?v=2\.5\.206'/);
});
...
test('rendered icons support stable per-file image overrides with fallbacks', () => {
  ...
  assert.match(script, /const ICON_ASSET_VERSION = '2\.5\.206'/);
...
test('home ticker teaches current mechanics in a shady promoter voice', () => {
  ...
  assert.match(html, /class="daily-drop-pack" src="assets\/cage-grind-drop-pack\.png\?v=2\.5\.206"/);
...
test('Cage Grind CEO is verified while championship announcements stay database-owned', () => {
  ...
  assert.match(page, /assets\/cage-grind-ceo\.jpg\?v=2\.5\.206/g);
  ...
  assert.match(serviceWorker, /\.\/assets\/cage-grind-ceo\.jpg\?v=2\.5\.206/);
});
...
test('CageReporter has a verified clickable read-only profile', () => {
  ...
  assert.match(strings, /author:'CageReporter',handle:'@CageReporter',tone:'media',avatar:'assets\/cage-reporter\.jpg\?v=2\.5\.206',verified:true,bio:/);
  ...
  assert.match(serviceWorker, /\.\/assets\/cage-reporter\.jpg\?v=2\.5\.206/);
...
test('training separates daily sparring from post-fight recovery opportunities', () => {
  ...
  assert.match(page, /class="trainer-portrait" src="assets\/icons\/coach-vega\.png\?v=2\.5\.206" alt="Coach Scrapps"/);
  assert.match(page, /<h3>Coach Scrapps<\/h3><p>Boosts gym gains and perfect-session odds\./);
  assert.match(css, /\.trainer-profile\{[^}]*grid-template-columns:56px minmax\(0,1fr\)/);
  assert.match(serviceWorker, /\.\/assets\/icons\/coach-vega\.png\?v=2\.5\.206/);
  assert.equal(fs.existsSync('assets/icons/coach-vega.png'), true, 'Coach Scrapps portrait should exist');
  ...
  assert.match(script, /'COACH UNAVAILABLE'/);
  assert.match(script, /'NO SESSIONS LEFT'/);
  assert.match(script, /if\(tt\)\{if\(tt\.disabled\)return;state\.trainerOn=!state\.trainerOn/);
  assert.match(css, /\.trainer-toggle:disabled\{[^}]*cursor:not-allowed[^}]*opacity:\.58/);
...
test('Underground Buzz keeps persistent once-daily blackjack, Cage Dice, and horse racing', () => {
  ...
  assert.match(page, /assets\/cage-dice\.jpg\?v=2\.5\.206/);
  ...
  assert.match(serviceWorker, /\.\/assets\/cage-dice\.jpg\?v=2\.5\.206/);
  ...
  assert.match(script, /class="horse-marker" src="assets\/racehorse-right\.png\?v=2\.5\.206"/);
  assert.match(serviceWorker, /\.\/assets\/racehorse-right\.png\?v=2\.5\.206/);
...
test('daily drop guarantees a deterministic collectible without resetting fight pity', () => {
  ...
  assert.match(serviceWorker, /\.\/assets\/cage-grind-drop-pack\.png\?v=2\.5\.206/);
...
test('fight result action celebrates wins without labeling losses as reward claims', () => {
  ...
  assert.match(page, /class="drop-claim-pack" src="assets\/cage-grind-drop-pack\.png\?v=2\.5\.206"/);
...
test('opponent cards, matchup preview, and results disclose daily repeat XP', () => {
  ...
  assert.match(script, /matchupAction=hasHistory\?'RUN IT BACK':'VIEW MATCHUP'/);
