...
Aura replaces the former Hype meter without replacing Followers. It starts at 0, runs from 0–100, and multiplies follower momentum without changing fight odds. Home presents the cosmetic tiers `UNKNOWN`, `GETTING NOTICED`, `MAGNETIC`, `SUPERSTAR`, and `ICONIC`. Positive Aura growth slows as status rises: Unknown receives 100% of a base gain, Getting Noticed 80%, Magnetic 60%, Superstar 40%, and Iconic 25%, with a minimum gain of one Aura. This scaling applies to positive fight, callout, social, and CEO awards; Aura penalties remain at full strength.
...
The shared Supabase-backed championship remains server-authoritative and non-fatal when offline. The Fight page uses the same ranking order as the Home rankings modal: the active champion is first, followed by a hybrid World Rank score weighted 50% résumé, 25% quality of defeated opposition, 15% recent form, and 10% permanent base attributes. Equipped gear and non-combat perks never inflate World Rank. Each full-width row shows rank, portrait, name, record, level, win percentage, exact win XP, and the Attribute Points available for that matchup. Tap any available row to open the existing Tale of the Tape.
