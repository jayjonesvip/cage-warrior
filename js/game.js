(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.6';
  ...
  function renderOpponents(){
    refreshOpponents();const fightsLeft=sessionsLeft('fight',DAILY_FIGHT_LIMIT),ranking=currentRanking(),showcase=opponents.filter(item=>item.rookieShowcase),ranked=opponents.filter(item=>item.network),circuit=opponents.filter(item=>!item.network&&!item.rookieShowcase),circuitRematches=circuit.filter(item=>(item.winsVsPlayer||0)>0).length;$('#fightResetClock').hidden=fightsLeft>0;setLimitBadge('#fightLimitText',`${fightsLeft} ${fightsLeft===1?'FIGHT':'FIGHTS'} LEFT`);$('#rosterSummary').textContent=`${circuit.length} CIRCUIT · ${ranked.length} RANKED${ranking.position?` · YOUR RANK #${ranking.position}`:''}`;$('#fightInjuryWarning').hidden=state.health>=state.maxHealth;const showcaseRows=showcase.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join(''),circuitSummary=circuitRematches?`${circuit.length-circuitRematches} FRESH · ${circuitRematches} REMATCH`:`${circuit.length} FRESH MATCHUPS · FULL XP`,circuitRows=circuit.length?`<div class="fight-ranking-group circuit-group"><b>ON-LEVEL CAGE CIRCUIT</b><span>${circuitSummary}</span></div>${circuit.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'',rankedRows=ranked.length?`<div class="fight-ranking-group ranked-group"><b>WORLD RANKINGS</b><span>${ranked.length} RANKED FIGHTERS</span></div>${ranked.map(opponent=>renderFightLadderRow(opponent,fightsLeft)).join('')}`:'';$('#opponentList').innerHTML=opponents.length?`<div class="fight-ranking-list">${showcaseRows}${circuitRows}${rankedRows}</div>`:`<div class="gear-empty"><b>BUILDING THE FIGHT CARD</b><span>New opponents will appear shortly.</span></div>`;
  }
  ...
  function finishFightSimulation(){
    ...
    ensureRoster();state.dailyOpponentWins.wins[o.key]=LOGIC.nextOpponentXpStage(winsToday,win);
