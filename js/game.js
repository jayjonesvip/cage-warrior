...
const dailyNews=globalThis.CAGE_DAILY_NEWS.create({element:$('#dailyCageNews'),tips:STRINGS.ticker,canShow:()=>!document.hidden&&!postFightPresentationBusy(),onDismiss:()=>requestAnimationFrame(showPostFightFollowup),client:SHARED_FEED,storage:localStorage,onSyncError:error=>{console.warn('Fight result sync pending:',error);toast('FIGHT RESULT SYNC PENDING · YOUR RESULT IS KEPT ON THIS DEVICE','#ffcf78')}});
  ...
  const ICON_ASSET_VERSION = '2.7.204';
...
function sharedProfilePayload(){return {combatStats:Object.fromEntries(['power','speed','chin','cardio'].map(key=>[key,effectiveStat(key)])),defendingPlan:LOGIC.normalizeFightPlan(state.fightPlanPreference),fightSkinAura:Math.floor(effectiveAura()),city:state.fighterCity,archetype:state.fighterStyle,fighterAvatar:state.fighterAvatar,level:state.level,wins:state.wins,losses:state.losses,draws:state.draws||0,attributeTotal:Object.values(state.stats).reduce((sum,value)=>sum+Math.max(0,Number(value)||0),0),rankingHistory:state.rankingHistory}}
  ...
  function confirmFightPlan(){
    ...
    const bout=fight,modal=$('#walkoutModal'),message=$('#walkoutMessage'),progress=$('#walkoutProgress'),musicOn=walkoutMusic.enabled&&walkoutMusic.supported;
    ...
    scheduleFight(()=>{$('#walkoutTitle').textContent='MAKE YOUR ENTRANCE';message.textContent=musicOn?'Your entrance music hits. You head toward the cage.':'You walk silently toward the cage.';progress.setAttribute('aria-valuenow','25')},1750);
    scheduleFight(()=>{$('#walkoutTitle').textContent='HEAR THE CROWD';message.textContent=walkoutCrowdMessage(effectiveAura());progress.setAttribute('aria-valuenow','50')},3500);
    scheduleFight(()=>{$('#walkoutTitle').textContent='ENTER THE CAGE';message.textContent='You step inside. Time to go to work.';progress.setAttribute('aria-valuenow','75')},5250);
    scheduleFight(()=>walkoutMusic.stop(),6800);
    scheduleFight(()=>{if(fight!==bout||!walkoutPending)return;progress.setAttribute('aria-valuenow','100');modal.classList.remove('open');modal.setAttribute('aria-hidden','true');walkoutPending=false;fightPlanFeature.confirm()},7000);
  }
