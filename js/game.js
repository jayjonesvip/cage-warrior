(() => {
  ...
  const ICON_ASSET_VERSION = '2.5.163';
  ...
  const fighterCities = [
    ...
    {id:'atlanta',name:'ATLANTA',region:'SOUTHEAST'},
    {id:'san-francisco',name:'SAN FRANCISCO',region:'WEST COAST'},
    {id:'denver',name:'DENVER',region:'MOUNTAIN WEST'}
  ];
  let opponents=[];
  ...
  function mapSharedPost(post){const reporter=post.post_kind==='reporter',ceo=post.post_kind==='ceo',mine=post.author_id===state.socialProfileId,profile=reporter||ceo?null:sharedSocialProfiles.find(item=>item.id===post.author_id)||null,avatar=fighterAvatars.find(item=>item.id===profile?.fighter_avatar),ceoProfile=STRINGS.social.profiles.ceo,targetProfileId=String(post.target_profile_id||''),mentioned=!!targetProfileId&&targetProfileId===state.socialProfileId;return {id:`shared-${post.id}`,author:ceo?ceoProfile.author:reporter?'CageReporter':post.author_handle,handle:ceo?ceoProfile.handle:`@${reporter?'CageReporter':post.author_handle}`,tone:ceo?'ceo':reporter?'media':mine?'player player-post':'fighter',text:String(post.body||''),createdAt:post.created_at,shared:true,profileId:profile?.id||'',targetProfileId,targetHandle:String(post.target_handle||''),mentioned,avatarAsset:ceo?ceoProfile.avatar:avatar?.asset||'',verified:ceo}}
  ...
  function renderHustle(){
    ...
    const undergroundLeft=(blackjackUnlocked?blackjackLeft:0)+(diceUnlocked?diceLeft:0);setLimitBadge('#undergroundLimitText',`${undergroundLeft} PLAY${undergroundLeft===1?'':'S'} LEFT`);
  ...
  function buildResultDetails(f){
    ...
    $('#resultTape').innerHTML=`<div class="rt-name">${f.player.name}</div><div class="rt-vs">VS</div><div class="rt-name">${f.opp.name}</div><div class="rt-stats"><div><b>${formatStat(f.player.power)}/${formatStat(f.player.speed)}/${formatStat(f.player.chin)}/${formatStat(f.player.cardio)}</b><br><small>PWR · SPD · CHN · CAR</small></div><div class="rt-vs">RATINGS</div><div><b>${formatStat(f.opp.power)}/${formatStat(f.opp.speed)}/${formatStat(f.opp.chin)}/${formatStat(f.opp.cardio)}</b><br><small>PWR · SPD · CHN · CAR</small></div></div><div class="rt-focus">FIGHT PLAN · <b>${fightPlanLabel(f.gamePlan)}</b><br>PLAN GRADE · <b class="plan-grade ${planAssessment.grade.toLowerCase()}">${planAssessment.grade}</b><br>FOCUS · <b>${f.focus}% ${focusTier(f.focus)}</b></div>`;
