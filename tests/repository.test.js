...
test('Open Gym reuses checked Fight Plan controls and the shared system palette',()=>{
  ...
  assert.match(game,/class="fighter-post-result" type="button" data-spar-target=/);
...
test('spar plans require selection and explicit rearming after a round',()=>{
  const settings=game.slice(game.indexOf('  function selectSparSetting('),game.indexOf('  function renderSparringProgress('));
  const context={state:{sparringPlan:{pace:'slow'},lastSparringReport:{target:'rival'}},saveState:()=>{},renderOpenGym:()=>{},sfx:{tap:()=>{}}};
  vm.createContext(context);
  vm.runInContext('let sparringSession=null,sparringSnapshot=null,sparPlanReady=false;'+settings,context);
  vm.runInContext('selectSparSetting("pace","fast")',context);
  assert.equal(context.state.sparringPlan.pace,'slow');
  vm.runInContext('sparringSnapshot={id:"rival"};selectSparSetting("pace","fast")',context);
  assert.equal(context.state.sparringPlan.pace,'slow');
  vm.runInContext('prepareSparRematch();selectSparSetting("pace","fast")',context);
  assert.equal(context.state.sparringPlan.pace,'fast');
  vm.runInContext('sparringSession={};selectSparSetting("pace","slow")',context);
  assert.equal(context.state.sparringPlan.pace,'fast');
  assert.match(game,/sparringSession=null;sparPlanReady=false;report.createdAt/);
  assert.match(game,/if\(sparringSession\|\|!state.nameLocked\|\|!sparPlanReady\)return/);
  assert.match(html,/id="sparRematchBtn"/);
});
...
test('occasional post-fight texts use the established contact portraits',()=>{
  ...
  assert.match(game,/function showPostFightFollowup\(\)\{if\(postFightPresentationBusy\(\)\)return false;if\(showPendingSponsor\(\)\)return true/);
