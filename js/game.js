(async () => {
  ...
  const ICON_ASSET_VERSION = '2.7.172';
  ...
  function networkOpponentFromProfile(profile,tier){
    ...
    const seed=hashSeed(`cage-network-v1|${id}|${tier}`),difficulty=((seed%3)-1)*.7,storedRatings=profile.seeded===true&&['power','speed','chin','cardio'].every(stat=>Number.isFinite(Number(profile[stat]))&&Number(profile[stat])>0)?Object.fromEntries(['power','speed','chin','cardio'].map(stat=>[stat,Math.floor(Number(profile[stat]))])):null,ratings=storedRatings||LOGIC.networkOpponentRatings(tier,avatar.stats,arch.mods,difficulty);
    return {key:`network-${id}`,network:true,seeded:profile.seeded===true,sourceProfileId:id,networkHandle:handle,networkCity:String(profile.city||''),networkPortrait:avatar.asset,fighterAvatar:avatar.id,fightSkinAura:clamp(Math.floor(Number(profile.fight_skin_aura))||0,0,100),name,tag:arch.tag,archetype:arch.id,tendency:arch.tendency,scout:arch.scout,tier,min:tier,max:99,...ratings,fans:Math.round(22*Math.pow(1.48,tier-1)),color:fighterAccent(profile.city),look:seed%10,wins:Math.max(0,Math.floor(Number(profile.wins))||0),losses:Math.max(0,Math.floor(Number(profile.losses))||0),winsVsPlayer:0,lossesToPlayer:0,meetings:0,rematchAccepted:false,recordInitialized:true,createdAt:Date.now()};
  }
...
$,logic:LOGIC,getState:()=>state,getRank:()=>rankName(),getChampionship:()=>sharedChampionship,setChampionship:setSharedChampionship,sharedFeed:SHARED_FEED,sharedUi:SHARED_UI,trackEvent,tap:()=>sfx.tap(),onEntered:()=>setTimeout(()=>{dailyNews.visit(currentScreen==='home');showCareerRestoreNotice();showRecoveryReport();if(!offerRookieShowcase())offerFirstContractOpponent()},180),onChampionshipChange:renderOpponents
  ...
  function createFight(o){
    ...
    return {o,rewardRanks,player:P,opp:O,playerAura:Math.floor(effectiveAura()),opponentAura:opponentFightAura(o),playerCondition:LOGIC.startingFightCondition(state.health,state.maxHealth),oppCondition:100,healthLost:0,rounds:[],timeline:[],totals:{player:emptyFightStats(),opp:emptyFightStats()},winner:null,method:'DECISION',submissionMove:null,finishRound:FIGHT_ROUNDS,finishClock:'0:00',ended:false,mode:'planned',gamePlan:Object.assign({},state.fightPlanPreference),planAssessment:null,focus:80,plans:[],lastPlan:state.fighterStyle||'striker',openingApproach:null,tendencyRevealed:true,deepRead:false,crisisUsed:false,cornerTowel:false,haymakerMiss:false,finalDecisionPending:false,lastChanceResolved:false,pendingMoment:null,resolvedMoments:[],roundIntros:[],rocked:{player:0,opp:0}};
  }
  ...
  function simulateRound(sim,round,planId,opening=null){
      ...
      for(let ex=1;ex<=exchanges&&!stopped;ex++){
        ...
        const rockedInitiative=sim.rocked.player? -fightRule('fightFinishes.rockedInitiativeSwing',.12):sim.rocked.opp?fightRule('fightFinishes.rockedInitiativeSwing',.12):0;
        const pInitiative=clamp(.5+(P.speed-O.speed)*.022+(P.cardio-O.cardio)*.008+initiativeMod+edge+tendencyInitiative+paceInitiative+focusMod*.7+rockedInitiative+crowdEdge,.14,.86);
        const side=Math.random()<pInitiative?'player':'opp',defenderSide=side==='player'?'opp':'player',A=side==='player'?P:O,D=side==='player'?O:P,aStats=rs[side],attackingStyle=side==='player'?plan.id:sim.o.tendency,attackerRocked=sim.rocked[side]>0,defenderRocked=sim.rocked[defenderSide]>0;
        ...
        if(side==='player'){
          chance+=edge*.72+focusMod+crowdEdge+(aggressiveOffense?-.045:.05);if(plan.id==='striker'&&type!=='takedown')chance+=.035;if(plan.id==='grappler'&&type==='takedown')chance+=.14;
        }else{
          chance-=edge*.45+focusMod*.35+crowdEdge;chance+=aggressiveOffense?.04:-.035;if(sim.o.tendency==='striker'&&type!=='takedown')chance+=.03;if(sim.o.tendency==='grappler'&&type==='takedown')chance+=.13;if(plan.id==='striker')chance+=.025;
        }
        ...
        if(landed){
          ...
          const intentKnockdown=side==='player'?(aggressiveOffense?.045:-.02):0,kdChance=clamp(.025+(A.power-D.chin)*.012+(damage-8)*.018+(attackingStyle==='striker'?.02:0)+intentKnockdown+(defenderRocked?fightRule('fightFinishes.rockedKnockdownBonus',.12):0),0,.48);
        ...
        if(landed&&type==='takedown'&&attackingStyle==='grappler'){
          const targetCondition=side==='player'?sim.oppCondition:sim.playerCondition,subChance=LOGIC.submissionFinishChance({speed:A.speed,opponentSpeed:D.speed,cardio:A.cardio,opponentCardio:D.cardio,targetCondition,signature:side==='player'&&state.fighterStyle==='grappler',rocked:defenderRocked});
        ...
        if(!stopped&&landed&&type!=='jab'&&type!=='takedown'){
          const rockedNow=kd||Math.random()<LOGIC.rockedChance({significant:true,power:A.power,chin:D.chin,damage,aggressive:side==='player'&&aggressiveOffense});
  ...
  function finishFightSimulation(){
    ...
    dailyNews.record({bout:state.wins+state.losses,at:new Date().toISOString(),won:win,opponent:o.networkHandle||o.name,playerRank:ranks.playerRank||0,opponentRank:o.network?ranks.opponentRank||0:0});
