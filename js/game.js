(async () => {
  ...
  globalThis.CAGE_PILOT_SPONSOR?.apply(endorsementDefs);
  function sponsorLogo(sponsor){return sponsor.logoAsset||`assets/icons/${sponsor.id}.png?v=${ICON_ASSET_VERSION}`}
...
const dailyNews=globalThis.CAGE_DAILY_NEWS.create({element:$('#dailyCageNews'),tips:STRINGS.ticker,canShow:()=>!document.hidden&&!postFightPresentationBusy(),onDismiss:()=>requestAnimationFrame(showPostFightFollowup),client:SHARED_FEED,storage:localStorage,onSyncError:error=>{console.warn('Fight result sync pending:',error);toast('FIGHT RESULT SYNC PENDING · YOUR RESULT IS KEPT ON THIS DEVICE','#ffcf78')}});
  ...
  const SUBMISSION_FINISHES=[
    {id:'rear_naked_choke',name:'Rear-Naked Choke',call:'takes the back and sinks in the rear-naked choke'},
    {id:'guillotine_choke',name:'Guillotine Choke',call:'snatches the neck and locks in the guillotine choke'},
    {id:'armbar',name:'Armbar',call:'isolates the arm and extends the armbar'},
    {id:'triangle_choke',name:'Triangle Choke',call:'traps the head and arm in a triangle choke'},
    {id:'kimura',name:'Kimura',call:'pins the wrist and cranks the kimura'}
  ];
  ...
  const ICON_ASSET_VERSION = '2.7.221';
  ...
  const GEAR_FILTER_KEY = 'cage-grind-gear-filter';
  const REFERRAL_INVITER_KEY = 'cage-grind-referral-inviter';
  ...
  const defaultState = {
    fightHistory:[],
    unseenGear:[],rankSnapshot:null,
    pendingHeatPosts:[],
    personalBests:{fastestFinish:0,biggestUpset:0,bestStreak:0},sponsorHighlightHistory:{},
    version:STATE_VERSION,name:'ROOKIE',nameLocked:false,rookieShowcasePending:false,firstContractPending:false,postFightTutorialSeen:false,fans:0,level:1,xp:0,wins:0,losses:0,draws:0,winStreak:0,bestStreak:0,attributePoints:0,rankingHistory:[],
    ...
    roster:[],rosterSerial:0,circuitLossStreak:0,fighterStyle:'',fighterCity:'',fighterAvatar:'',fighterBaseStats:null,fighterDraftAvatar:'',fighterDraftStats:null,fighterNameDraft:'',fighterNameDraftManual:false,equippedGear:[],equippedPerks:[],leagueInitialized:false
  };
  ...
  let sparPlanReady = false;
  const FIGHT_RANKING_BATCH_SIZE=50;
  let visibleFightRankingCount=FIGHT_RANKING_BATCH_SIZE;
  let resetFightListPending=false;
  let attributeAssignmentExpanded = false;
  let gearFilter = (()=>{try{const saved=sessionStorage.getItem(GEAR_FILTER_KEY);return ['combat','bling','property','lifestyle'].includes(saved)?saved:'combat'}catch{return 'combat'}})();
  let fight = null;
  let fightTimers = [];
  let pendingResultDrop = null;
  let resultDropRevealed = false;
  let pendingDropContext = null;
  let pendingPostFightText = null;
  let postFightMessageTimer = null;
  let lastPostFightTextContactId = '';
  ...
  let sharedHeadToHead=[];
  let sharedFinishStats={};
  let sharedSocialStatus='idle';
  let sharedSocialError='';
  let sharedSocialSyncPromise=null;
  let sharedSocialRefreshTimer=null;
  let cloudCareerSaveTimer=null;
  let cloudCareerSavePromise=null;
  let cloudCareerSaveDirty=false;
  let cloudCareerEnabled=true;
  let careerRestoreNotice='';
  ...
  let identityStatusMessage='';
  let identityRetryable=false;
  ...
  let pendingReferralInviterId='';
  let pendingReferralDrop=null;
  ...
  const LOADOUT_CATEGORIES=['Fight Gear','Bling','Property & Rides','Lifestyle'];
  const LOADOUT_FILTER_BY_CATEGORY={'Fight Gear':'combat',Bling:'bling','Property & Rides':'property',Lifestyle:'lifestyle'};
  ...
  function networkOpponentFromProfile(profile,tier){
    ...
    const seed=hashSeed(`cage-network-v1|${id}|${tier}`),ratings=LOGIC.validCombatStats(profile.seeded===true?profile:profile.combat_stats);
    return {key:`network-${id}`,network:true,statsReady:!!ratings,statsUpdatedAt:profile.combat_stats_updated_at||null,defendingPlan:LOGIC.normalizeFightPlan(profile.defending_plan),seeded:profile.seeded===true,sourceProfileId:id,networkHandle:handle,networkCity:String(profile.city||''),networkPortrait:avatar.asset,fighterAvatar:avatar.id,fightSkinAura:clamp(Math.floor(Number(profile.fight_skin_aura))||0,0,100),name,tag:arch.tag,archetype:arch.id,tendency:arch.tendency,scout:arch.scout,tier,min:tier,max:99,...ratings,fans:Math.round(22*Math.pow(1.48,tier-1)),color:fighterAccent(profile.city),look:seed%10,wins:Math.max(0,Math.floor(Number(profile.wins))||0),losses:Math.max(0,Math.floor(Number(profile.losses))||0),winsVsPlayer:0,lossesToPlayer:0,meetings:0,rematchAccepted:false,recordInitialized:true,createdAt:Date.now()};
  }
  ...
  function rankedTitleContext(opponent,worldRank){
    ...
    const rematch=!playerIsChampion&&sharedChampionship?.former_champion_rematch===true,titleCooldown=playerIsChampion?sharedChampionship?.defense_used_today===true:!rematch&&sharedChampionship?.daily_bout_used===true;
  ...
  function combatStatsPending(opponent){return !!opponent?.network&&!opponent.seeded&&(opponent.statsReady!==true||!LOGIC.validCombatStats(opponent))}
  function opponentState(o){return LOGIC.opponentState(o,opponentContext())}
  function opponentRematchRemaining(opponent){if(opponent?.globalChampionship&&opponent.titleMode==='rematch'&&sharedChampionship?.former_champion_rematch===true&&opponent.sourceProfileId===sharedChampionship.champion_id)return 0;return globalThis.CAGE_FIGHT_HISTORY.rematchRemaining(state.fightHistory,opponent)}
  function rematchTimeLabel(milliseconds){const minutes=Math.max(1,Math.ceil(milliseconds/60000));return `${Math.floor(minutes/60)}H ${minutes%60}M`}
  function updateRematchClocks(){
    if(combatLocked)return;let expired=false;const now=Date.now();
    $$('[data-rematch-until]').forEach(node=>{const remaining=Number(node.dataset.rematchUntil)-now;if(remaining<=0){delete node.dataset.rematchUntil;expired=true}else node.textContent=`REMATCH IN ${rematchTimeLabel(remaining)}`});
    if(expired){renderOpponents();if(fight&&!fightBooking)fillTape(fight)}
  }
  function opponentAvailable(o){return !o?.selfProfile&&!combatStatsPending(o)&&!opponentRematchRemaining(o)&&LOGIC.opponentAvailable(o,opponentContext())}
  ...
  function equippedForCategory(category,gearLoadout=state?.equippedGear||[],perkLoadout=state?.equippedPerks||[]){return (category==='Fight Gear'?gearLoadout:perkLoadout).filter(id=>itemCategory(id)===category)}
  function trimCategoryLoadout(ids,ownedIds,categories){const valid=[...new Set(Array.isArray(ids)?ids:[])].filter(id=>ownedIds.includes(id));return categories.flatMap(category=>valid.filter(id=>itemCategory(id)===category).slice(0,LOGIC.loadoutCategoryLimit()))}
  ...
  function normalizeState(parsed){
      ...
      s.personalBests=LOGIC.careerHighlights(source.personalBests||{}).records;s.personalBests.bestStreak=Math.max(s.personalBests.bestStreak,s.bestStreak||0);
      s.sponsorHighlightHistory={};for(const id of ENDORSEMENT_IDS){const entry=source.sponsorHighlightHistory?.[id];if(entry)s.sponsorHighlightHistory[id]={date:String(entry.date||'').slice(0,10),fight:Math.max(0,Number(entry.fight)||0),index:Math.max(0,Math.floor(Number(entry.index)||0))}}
      ...
      s.rankingHistory=LOGIC.normalizeRankingHistory(source.rankingHistory);
      s.fightHistory=globalThis.CAGE_FIGHT_HISTORY.normalize(source.fightHistory);
      s.unseenGear=[...new Set(Array.isArray(source.unseenGear)?source.unseenGear:[])].filter(id=>gearItems.some(item=>item.id===id)&&s.gear.includes(id));
      const savedRank=source.rankSnapshot;
      s.rankSnapshot=savedRank&&Number.isInteger(savedRank.rank)&&savedRank.rank>0?{rank:savedRank.rank,delta:Number.isInteger(savedRank.delta)?savedRank.delta:0,changedAt:Math.max(0,Number(savedRank.changedAt)||0)}:null;
      ...
      s.fighterNameDraft=s.nameLocked?'':normalizeIdentityName(source.fighterNameDraft);s.fighterNameDraftManual=!s.nameLocked&&source.fighterNameDraftManual===true;
  ...
  function normalizeReferralId(value){const id=String(value||'').trim();return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id)?id:''}
  function clearReferralInvite(){pendingReferralInviterId='';try{localStorage.removeItem(REFERRAL_INVITER_KEY)}catch{/* Referral storage is optional. */}}
  function captureReferralInvite(){
    let stored='';try{stored=localStorage.getItem(REFERRAL_INVITER_KEY)||''}catch{/* Referral storage is optional. */}
    const url=new URL(window.location.href),incoming=normalizeReferralId(url.searchParams.get('invite'));
    if(incoming){try{localStorage.setItem(REFERRAL_INVITER_KEY,incoming)}catch{/* The link still works for this session. */}url.searchParams.delete('invite');history.replaceState(history.state,'',`${url.pathname}${url.search}${url.hash}`)}
    pendingReferralInviterId=incoming||normalizeReferralId(stored);
    if(!LOGIC.isBlankCareer(state))clearReferralInvite();
  }
  function recoveredStateFromProfile(profile){
    const avatar=fighterAvatars.find(item=>item.id===String(profile?.fighter_avatar||'')),name=normalizeIdentityName(profile?.handle),city=String(profile?.city||''),style=normalizeMajorArchetype(profile?.archetype);if(!profile?.id||!avatar||!name||!city||!style)return null;
    const stats=Object.assign({},avatar.stats),keys=['power','speed','chin','cardio'],baseTotal=keys.reduce((sum,key)=>sum+stats[key],0),targetTotal=Math.max(baseTotal,Math.floor(Number(profile.attribute_total))||baseTotal);for(let point=0;point<targetTotal-baseTotal;point++)stats[keys[point%keys.length]]++;
    return normalizeState(Object.assign(structuredClone(defaultState),{name,nameLocked:true,fighterCity:city,fighterStyle:style,fighterAvatar:avatar.id,fighterBaseStats:Object.assign({},avatar.stats),stats,level:Math.max(1,Math.floor(Number(profile.level))||1),wins:Math.max(0,Math.floor(Number(profile.wins))||0),losses:Math.max(0,Math.floor(Number(profile.losses))||0),rankingHistory:Array.isArray(profile.ranking_history)?profile.ranking_history:[],socialProfileId:String(profile.id),socialAccountCreated:true,postFightTutorialSeen:(Number(profile.wins)||0)+(Number(profile.losses)||0)>0,rookieShowcasePending:false,firstContractPending:false}));
  }
  async function restoreRemoteCareer(){
    if(!SHARED_FEED?.configured?.())return false;
    try{
      const remote=await SHARED_FEED.loadCareer?.();
      if(remote&&typeof remote==='object'&&!Array.isArray(remote)){const restored=normalizeState(remote);if(!LOGIC.isBlankCareer(restored)){state=restored;careerRestoreNotice='CAREER RESTORED FROM CLOUD';saveState();return true}}
    }catch(error){if(error?.status===404||/load_cage_career|schema cache/i.test(String(error?.message||'')))cloudCareerEnabled=false}
    try{
      const profile=await SHARED_FEED.loadOwnProfile?.(),restored=recoveredStateFromProfile(profile);if(!restored)return false;state=restored;careerRestoreNotice='CAREER IDENTITY RECOVERED · FUTURE PROGRESS WILL SYNC';saveState();return true;
    }catch{return false}
  }
  function scheduleCloudCareerSave(){
    if(!cloudCareerEnabled||!state.nameLocked||!state.socialProfileId||!SHARED_FEED?.saveCareer)return;cloudCareerSaveDirty=true;clearTimeout(cloudCareerSaveTimer);cloudCareerSaveTimer=setTimeout(flushCloudCareerSave,900);
  }
  async function flushCloudCareerSave(){
    clearTimeout(cloudCareerSaveTimer);cloudCareerSaveTimer=null;if(cloudCareerSavePromise||!cloudCareerSaveDirty||!cloudCareerEnabled)return false;cloudCareerSaveDirty=false;const expectedProfileId=state.socialProfileId,snapshot=structuredClone(state);
    cloudCareerSavePromise=SHARED_FEED.saveCareer(snapshot,expectedProfileId).then(()=>true).catch(error=>{if(error?.status===404||/save_cage_career|schema cache/i.test(String(error?.message||'')))cloudCareerEnabled=false;else cloudCareerSaveDirty=true;return false}).finally(()=>{cloudCareerSavePromise=null;if(cloudCareerSaveDirty)scheduleCloudCareerSave()});return cloudCareerSavePromise;
  }
  if(LOGIC.isBlankCareer(state))await restoreRemoteCareer();
  captureReferralInvite();
  function saveState(){
    ...
    try{localStorage.setItem(SAVE_KEY,JSON.stringify(state));careerSaveKnown=true;saveWarningShown=false;scheduleCloudCareerSave()}
  ...
  function applyOfflineRecovery(){
    ...
    if(followerGrowth.followers){syncSponsorProgress();trackEvent('passive_followers_awarded',{followers:followerGrowth.followers,hours:followerGrowth.hours,aura_rate:followerGrowth.aura,followers_per_hour:followerGrowth.rate})}if(followerGrowth.changed)saveState();
    ...
    return recovered.energy||recovered.health||followerGrowth.followers||refunded?{...recovered,followers:followerGrowth.followers,refunded}:null;
  }
  ...
  function showRecoveryReport(){
    if(!recoveryReport)return;const parts=[];if(recoveryReport.followers)parts.push(`+${fmt(recoveryReport.followers)} FOLLOWERS`);if(recoveryReport.energy)parts.push(`+${recoveryReport.energy} ENERGY`);if(recoveryReport.health)parts.push(`+${recoveryReport.health} HEALTH`);if(recoveryReport.refunded)parts.push('FIGHT BOOKING REFUNDED');const away=!!recoveryReport.followers;recoveryReport=null;toast(`${away?'WHILE YOU WERE AWAY':'WELCOME BACK'} · ${parts.join(' · ')}`,'#78dfff');
  }
  function showCareerRestoreNotice(){if(!careerRestoreNotice)return;const message=careerRestoreNotice;careerRestoreNotice='';toast(message,'#78dfff')}
  landingFeature=globalThis.CAGE_LANDING.createLandingFeature({
$,logic:LOGIC,getState:()=>state,getRank:()=>rankName(),getChampionship:()=>sharedChampionship,setChampionship:setSharedChampionship,sharedFeed:SHARED_FEED,sharedUi:SHARED_UI,trackEvent,tap:()=>sfx.tap(),onEntered:()=>{prepareCareerBuilder();setTimeout(()=>{showCareerRestoreNotice();showRecoveryReport();if(!offerRookieShowcase())offerFirstContractOpponent();dailyNews.visit(currentScreen==='home')},180)},onChampionshipChange:renderOpponents
  });
  ...
  function effectiveStat(key){
    ...
    for(const id of state.equippedGear){const g=gearItems.find(x=>x.id===id);if(g&&g.stat===key)v+=g.bonus;if(g?.bonuses?.[key])v+=g.bonuses[key]}
    return Math.max(1,Math.round(Number(v)||1));
  }
  let combatStatsSyncTimer=null,combatStatsSyncing=false,lastCombatStatsSync='';
  function scheduleCombatStatsSync(){
    if(!state.nameLocked||!state.socialProfileId||!SHARED_FEED?.saveCombatStats)return;
    const stats=Object.fromEntries(['power','speed','chin','cardio'].map(key=>[key,effectiveStat(key)])),owner=state.socialProfileId,signature=owner+JSON.stringify(stats);
    if(signature===lastCombatStatsSync||combatStatsSyncing)return;
    clearTimeout(combatStatsSyncTimer);
    combatStatsSyncTimer=setTimeout(async()=>{if(state.socialProfileId!==owner)return;combatStatsSyncing=true;try{await SHARED_FEED.saveCombatStats(stats);lastCombatStatsSync=signature}catch{/* Retry on the next UI or career sync. */}finally{combatStatsSyncing=false}},500);
  }
  function perkValue(id){const item=gearItems.find(entry=>entry.id===id);return (Number(item?.healthRecoverySpeed)||0)+(Number(item?.energyRecoverySpeed)||0)*10+(Number(item?.auraBonus)||0)*10000}
  function ensureLoadout(){const ownedFight=state.gear.filter(id=>itemCategory(id)==='Fight Gear'),ownedPerks=state.gear.filter(id=>itemCategory(id)&&itemCategory(id)!=='Fight Gear');state.equippedGear=trimCategoryLoadout(state.equippedGear,ownedFight,['Fight Gear']);state.equippedPerks=trimCategoryLoadout(state.equippedPerks,ownedPerks,LOADOUT_CATEGORIES.slice(1));if(!state.equippedGear.length&&ownedFight.length)state.equippedGear=ownedFight.slice(0,LOGIC.loadoutCategoryLimit())}
  ...
  function awardCeoPerformanceRecognition({upset=false,ko=false,titleWon=false}={}){const date=todayKey(),qualifies=!titleWon&&(upset||ko),roll=hashSeed(`${state.name}|${state.wins}|${date}|ceo-recognition-v1`)%100;if(!qualifies||state.ceoBonusDate===date||roll>=10)return 0;const bonus=LOGIC.scaledAuraGain(fightRule('auraRewards.ceoRecognition',3),state.aura);state.ceoBonusDate=date;state.aura=clamp(state.aura+bonus,0,100);publishCeoEvent(`performance_bonus_${state.wins}`);trackEvent('ceo_performance_recognition',{aura_bonus:bonus,upset,ko});return bonus}
  function gearCount(id){return Math.max(0,Math.floor(Number(state.gearCounts&&state.gearCounts[id]))||0)}
  function activePerkBonus(prop){return state.equippedPerks.reduce((sum,id)=>{const g=gearItems.find(x=>x.id===id);return sum+(g&&g[prop]?g[prop]:0)},0)}
  function effectiveAura(){return clamp(state.aura+activePerkBonus('auraBonus'),0,100)}
  function currentAuraFightSkin(){const tier=LOGIC.auraTitle(effectiveAura());return auraFightSkins.find(skin=>skin.key===tier.key)||auraFightSkins[0]}
  function energyRecoveryInterval(){return Math.max(4000,ENERGY_RECOVERY_INTERVAL-activePerkBonus('energyRecoverySpeed'))}
  function healthRecoveryInterval(){return Math.max(30000,HEALTH_RECOVERY_INTERVAL-activePerkBonus('healthRecoverySpeed'))}
  ...
  function autoEquipNewDrop(item){
    if(item&&!state.unseenGear.includes(item.id))state.unseenGear.push(item.id);
    if(!item||equippedForCategory(item.category).length>=LOGIC.loadoutCategoryLimit())return false;
    const loadout=item.category==='Fight Gear'?state.equippedGear:state.equippedPerks;
    if(loadout.includes(item.id))return false;
    loadout.push(item.id);return true;
  }
  function awardDeterministicGearDrop({opponent,titleWon=false,guaranteed=false,progressSteps=1}){
    ...
    const item=chooseGear(pool,random);state.gear.push(item.id);state.gearCounts[item.id]=1;state.gearWinsSinceDrop=0;const autoEquipped=autoEquipNewDrop(item);ensureLoadout();
    return {item,rarity,count:1,isNew:true,autoEquipped,guaranteed:true,reason:guaranteed?'FIRST WIN DROP':titleWon?'CHAMPIONSHIP DROP':'VICTORY PACK'};
  }
  function awardDailyCollectible(date){
    ...
    for(let r=rank-1;!pool.length&&r>=minRank;r--){rarity=gearRarityOrder[r];pool=eligibleGearAtLevel(state.level,rarity)}
    for(let r=rank+1;!pool.length&&r<gearRarityOrder.length;r++){rarity=gearRarityOrder[r];pool=eligibleGearAtLevel(state.level,rarity)}
    if(!pool.length)return null;const item=chooseGear(pool,random);state.gear.push(item.id);state.gearCounts[item.id]=1;const autoEquipped=autoEquipNewDrop(item);ensureLoadout();return {item,rarity,count:1,isNew:true,autoEquipped,guaranteed:true,reason:'DAILY DROP'};
  }
  function awardReferralCollectible(referralId){
    const random=seededRandom(hashSeed(`${state.gearSeed}|${referralId}|${state.level}|referral-drop-v1`));let rarity=rollGearRarity(state.level,random()),rank=gearRarityOrder.indexOf(rarity),pool=eligibleGearAtLevel(state.level,rarity);
    for(let r=rank-1;!pool.length&&r>=0;r--){rarity=gearRarityOrder[r];pool=eligibleGearAtLevel(state.level,rarity)}
    for(let r=rank+1;!pool.length&&r<gearRarityOrder.length;r++){rarity=gearRarityOrder[r];pool=eligibleGearAtLevel(state.level,rarity)}
  ...
  function victoryPackResultHtml({earned=false,eligible=false,steps=0,lowerLevel=false,repeatEligible=true,titleWon=false,firstCareerWin=false}={}){
    ...
    else if(lowerLevel)detail='LOWER LEVEL + NO RANK ADVANTAGE · NO PROGRESS';
  ...
  function ensureDailyCounters(){
    const today=todayKey();state.dailyCounters=LOGIC.dailyCountersFor(state.dailyCounters,today);if(state.dailyOpponentWins?.date!==today)state.dailyOpponentWins={date:today,wins:{}};
  }
  function opponentWinsToday(o){ensureDailyCounters();return clamp(Math.floor(Number(state.dailyOpponentWins.wins[o?.key]))||0,0,2)}
  function opponentXpTier(o){return fightRewardTier(o)}
  ...
  function toast(msg,color='white',iconName='',iconFallback='',duration=1700,toastClass=''){
    const t=$('#toast');t.textContent=msg;if(iconName)t.insertAdjacentHTML('afterbegin',gameIcon(iconName,iconFallback)+' ');t.style.color=color;t.classList.remove('fight-rocked-toast','fight-finish-toast','fight-finish-win','fight-finish-loss');for(const name of String(toastClass).split(/\s+/).filter(Boolean))t.classList.add(name);t.classList.add('show-toast');
  ...
  function formatHudRecoveryCountdown(milliseconds){
    const totalSeconds=Math.max(0,Math.ceil((Number(milliseconds)||0)/1000));return `${Math.floor(totalSeconds/60)}:${String(totalSeconds%60).padStart(2,'0')}`;
  }
  function renderRecoveryClocks(){
    const healthTier=LOGIC.healthTierName(state.health,state.maxHealth);
    $('#hudHealthLabel').textContent=`HEALTH · ${healthTier}`;
    $('#healthHud').dataset.healthTier=healthTier.toLowerCase().replaceAll(' ','-');
    const energyText=$('#hudEnergyText'),healthText=$('#hudHealthText'),energyRecovering=state.energy<state.maxEnergy,healthRecovering=state.health<state.maxHealth;
    energyText.classList.toggle('recovering',energyRecovering);healthText.classList.toggle('recovering',healthRecovering);
    energyText.textContent=energyRecovering?`full in ${formatHudRecoveryCountdown(LOGIC.recoveryTimeRemaining(state.energy,state.maxEnergy,state.energyRecoveryAt,energyRecoveryInterval()))}`:`${Math.floor(state.energy)}%`;
    healthText.textContent=healthRecovering?`full in ${formatHudRecoveryCountdown(LOGIC.recoveryTimeRemaining(state.health,state.maxHealth,state.healthRecoveryAt,healthRecoveryInterval()))}`:`${Math.floor(state.health)}/${state.maxHealth}`;
  }
  function renderResourceHud(){
    ...
    $('#hudEnergyText').textContent=`${energyNow}%`;$('#energyBattery').setAttribute('aria-label',`Energy ${energyNow} percent`);const chargedEnergyPips=Math.ceil(clamp(state.energy,0,state.maxEnergy)/Math.max(1,state.maxEnergy)*8);$$('#energyBattery i').forEach((segment,index)=>segment.classList.toggle('charged',index<chargedEnergyPips));$('#energyHud').classList.toggle('critical',state.energy<=25);
  ...
  function updatePassiveRecovery(showFollowerAway=false){
    ...
    else if(followerGrowth.changed)saveState();
    if(recovered.energy||recovered.health){saveState();if(state.nameLocked){renderResourceHud();if(fight&&!combatLocked&&$('#opponentProfileModal').classList.contains('open'))fillTape(fight)}}else renderRecoveryClocks();
  }
  ...
  function showLevelUp(summary){if(!summary)return;const levels=summary.toLevel-summary.fromLevel,loadoutUnlocked=summary.fromLevel<8&&summary.toLevel>=8;$('#levelUpNumber').textContent=summary.toLevel;$('#levelUpTitle').textContent=rankName();$('#levelUpFrom').textContent=`LEVEL ${summary.fromLevel} → LEVEL ${summary.toLevel}`;$('#levelUpHealth').textContent=`+${levels*5}`;$('#levelUpUnlockValue').textContent=loadoutUnlocked?'4 + 4':'NEW';$('#levelUpUnlockLabel').textContent=loadoutUnlocked?'LOADOUT SLOTS':'COMPETITION';const modal=$('#levelUpModal');modal.classList.add('active');modal.setAttribute('aria-hidden','false');sfx.level();vibrate([35,35,65,35,90]);confettiBurst();clearTimeout(modal._burstTimer);modal._burstTimer=setTimeout(confettiBurst,620)}
  function closeLevelUp(){const modal=$('#levelUpModal');clearTimeout(modal._burstTimer);modal._burstTimer=null;stopConfetti();modal.classList.remove('active');modal.setAttribute('aria-hidden','true');levelUpSummary=null;sfx.tap();updateUI();requestAnimationFrame(showPostFightFollowup)}
  ...
  let renderedFightHistory='';
  function renderFightHistory(){
    ...
    $('#fightHistoryBasis').textContent=`Based on ${entries.length} saved fight${entries.length===1?'':'s'}`;
    $('#fightHistoryCharts').innerHTML=globalThis.CAGE_FIGHT_HISTORY.charts(entries);
    ...
    }).join(''):'<p class="fight-history-note">No saved fights yet. Your next completed fight will appear here.</p>';
  }
  function updateUI(){
    scheduleCombatStatsSync();
    renderFightHistory();
    ensureDailyCounters();
    ...
    const progressText=$('#progressText');progressText.textContent=`PRO ${state.wins}-${state.losses}`;$('#cageStatus').textContent=cageStatus();$('#heroLevel').textContent=`LVL ${state.level}`;
    ...
    $('.hud-attributes-row').setAttribute('aria-label','Fighter attributes');
    ['power','speed','chin','cardio'].forEach(k=>{const value=effectiveStat(k);$('#'+k+'Stat').textContent=formatStat(value);$('#'+k+'Mini').style.width=clamp(value/10*100,0,100)+'%'});
    $('#dailyBtn').disabled=false;$('#dailyBtnLabel').textContent='CLAIM DAILY DROP';$('#dailyDropResetClock').hidden=true;
    renderCareer();renderHomeInstallCta();renderAttributeAssignment();renderSocial();renderGear();renderOpponents();renderOpenGym();saveState();
  }

  const championshipResetCopy=SHARED_UI.championshipResetCopy;

  function renderCareer(){
    $('#entranceMusicTrackName').textContent=globalThis.CAGE_MUSIC.trackNameFor(effectiveAura());
    const style=currentStyle(),city=currentCity(),avatar=currentAvatar(),allocationValid=!!avatar&&validFighterAllocation(state.fighterBaseStats),coreReady=!!(style&&city&&avatar&&allocationValid),ready=coreReady&&state.nameLocked,completed=Number(!!city&&!!avatar&&allocationValid&&!!style)+Number(state.nameLocked),progress=Math.round(completed/2*100),pwa=globalThis.CAGE_PWA;
    $('#app').style.setProperty('--fighter-accent',city?.accent||DEFAULT_FIGHTER_ACCENT);applyPortraitStyle($('#app'),state.name);
    const aura=clamp(Math.floor(Number(effectiveAura())||0),0,100),auraTitle=LOGIC.auraTitle(aura),auraToken=$('#careerAuraToken'),auraGear=activePerkBonus('auraBonus'),fightSkin=currentAuraFightSkin(),fightSkinCard=$('#homeFightSkin'),auraSkinIndex=auraFightSkins.findIndex(skin=>skin.key===auraTitle.key),nextAuraSkin=auraFightSkins[auraSkinIndex+1]||null,auraGoal=nextAuraSkin?.minimum||100,auraFloor=nextAuraSkin?auraTitle.minimum:99,auraStage=Math.max(1,auraGoal-auraFloor),auraProgress=clamp((aura-auraFloor)/auraStage*100,0,100);$('#careerAttributeTotal').textContent=['power','speed','chin','cardio'].reduce((sum,key)=>sum+effectiveStat(key),0);$('#careerAuraTitle').textContent=nextAuraSkin?`${auraTitle.label} → ${nextAuraSkin.label}`:auraTitle.label;$('#careerAuraProgress').textContent=nextAuraSkin?`${Math.max(0,auraGoal-aura)} AURA TO ${nextAuraSkin.label}`:aura<100?`${100-aura} AURA TO MAX`:'MAXIMUM AURA';$('#careerAuraValue').textContent=`${aura} / ${auraGoal} AURA${auraGear?` · +${auraGear} BLING`:''}`;$('#careerAuraFill').style.width=`${auraProgress}%`;$('#careerAuraTrack').setAttribute('aria-label',nextAuraSkin?`Aura progress toward ${nextAuraSkin.label}`:'Aura progress to maximum');$('#careerAuraTrack').setAttribute('aria-valuemin',String(auraFloor));$('#careerAuraTrack').setAttribute('aria-valuemax',String(auraGoal));$('#careerAuraTrack').setAttribute('aria-valuenow',String(aura));auraToken.className=`career-token career-progression-goal career-aura-goal career-aura-token aura-tier-${auraTitle.key}`;fightSkinCard.style.setProperty('--fight-skin-accent',fightSkin.accent);$('#homeFightSkinGloves').src=`${fightSkin.glovesAsset}?v=${ICON_ASSET_VERSION}`;$('#homeFightSkinWraps').src=`${fightSkin.wrapsAsset}?v=${ICON_ASSET_VERSION}`;$('#homeFightSkinMouthguard').src=`${fightSkin.mouthguardAsset}?v=${ICON_ASSET_VERSION}`;$('#homeFightSkinShorts').src=`${fightSkin.shortsAsset}?v=${ICON_ASSET_VERSION}`;$('#homeFightSkinRange').textContent=`AURA ${fightSkin.minimum}–${fightSkin.maximum}`;$('#homeFightSkinTitle').textContent=fightSkin.label;$('#homeFightSkinColor').textContent=`${fightSkin.colorName} FIGHT KIT`;
    if(pwa?.isInstalled?.())state.installDetected=true;
    const dailyAvailable=ready&&state.lastDaily!==todayKey(),dailyClaimed=ready&&!dailyAvailable,gearDropOffer=$('#gearDropOffer'),dailyDropResetClock=$('#dailyDropResetClock'),dailyButton=$('#dailyBtn'),gearNav=$('.navbtn[data-nav="gear"]');gearDropOffer.hidden=!ready||dailyClaimed;dailyDropResetClock.hidden=!dailyClaimed;gearNav.classList.toggle('drop-ready',dailyAvailable);gearNav.setAttribute('aria-label',dailyAvailable?'Gear, Daily Drop ready':'Gear');dailyButton.hidden=false;dailyButton.disabled=!dailyAvailable;if(dailyAvailable){$('#gearDropEyebrow').textContent='ONE FREE PACK EVERY DAY';$('#gearDropTitle').textContent='DAILY DROP';$('#gearDropDescription').textContent='Guaranteed collectible';$('#dailyBtnLabel').textContent='CLAIM DAILY DROP'}$('#app').classList.toggle('career-setup',!ready);$('.resource-hud').hidden=!ready;$('.bottomnav').hidden=!ready;$('#fighterBuilderIntro').hidden=ready;$('#builderProgressStep').textContent=`${completed} OF 2 COMPLETE`;$('#builderProgressPercent').textContent=`${progress}%`;$('#builderProgressFill').style.width=`${progress}%`;$('#builderProgressTrack').setAttribute('aria-valuenow',String(progress));$('#citySetup').hidden=!!city;$('#fighterSetup').hidden=!city||!!avatar;$('#fighterNameSetup').hidden=!coreReady||state.nameLocked;if(coreReady&&!state.nameLocked&&!identitySuggestion)identitySuggestion=randomIdentitySuggestion();const identityDisplay=$('#fighterNameSuggestion'),manualInput=$('#manualFighterNameInput'),nameRule=$('#fighterNameRule'),manualValid=manualIdentityName(identityManualValue);identityDisplay.textContent=identitySuggestion||state.name;identityDisplay.hidden=identityManualMode;manualInput.hidden=!identityManualMode;nameRule.hidden=!identityManualMode;if(document.activeElement!==manualInput)manualInput.value=identityManualValue||identitySuggestion;manualInput.disabled=identityPending;manualInput.classList.toggle('invalid',identityManualMode&&!!identityManualValue&&!manualValid);nameRule.classList.toggle('invalid',identityManualMode&&!!identityManualValue&&!manualValid);$('#newFighterNameBtn').disabled=identityPending||identityShufflePending;$('#manualFighterNameBtn').disabled=identityPending||identityShufflePending;$('#manualFighterNameBtn').textContent=identityManualMode?'Use suggestion':'Manual entry';$('#lockFighterNameBtn').disabled=identityPending||identityShufflePending||(identityManualMode&&!manualValid);$('#lockFighterNameBtn').textContent=identityPending?'Checking name…':identityRetryable?'Retry name check':'Next';$('#fighterNameStatus').hidden=!identityStatusMessage;$('#fighterNameStatus').textContent=identityStatusMessage;$('#homeProfileName').textContent=state.name;$('#homeProfileName').title=state.name;$('#careerIdentityStatus').textContent=`${state.wins}-${state.losses}`;$('#homeCityText').textContent=city?city.name:'NOT SELECTED';$('#homeStyleText').textContent=style?style.name:'NOT SELECTED';$('#careerFollowersText').textContent=fmt(state.fans);
    const sponsorProgress=syncSponsorProgress(false),sponsor=sponsorProgress.active,nextSponsor=sponsorProgress.next,hero=$('.hero'),sponsorWallpaper=$('#heroSponsorWallpaper'),sponsorTrack=$('#careerSponsorProgressTrack'),sponsorFill=$('#careerSponsorProgressFill');
    ...
    renderProfileSponsor({wallpaper:sponsorWallpaper,surface:hero},sponsor);
    renderProfileAttributes({chart:$('#homeAttributeDonut'),totalLabel:$('#homeAttributeTotal'),legend:$('#homeProfileStats')},Object.fromEntries(['power','speed','chin','cardio'].map(key=>[key,effectiveStat(key)])));
  ...
  function renderHomeInstallCta(){
    const ready=!!(state.fighterStyle&&state.fighterCity&&state.fighterAvatar&&validFighterAllocation(state.fighterBaseStats)),installed=state.installDetected||!!globalThis.CAGE_PWA?.isInstalled?.(),cta=$('#homeInstallCta'),button=$('#installGameBtn');
    cta.hidden=!ready||installed;button.hidden=!ready||installed;
  }
  function renderAttributeAssignment(){
    ...
    $$('[data-attribute-assignment]').forEach(area=>{
      area.hidden=!available||(area.hasAttribute('data-first-win-assignment')&&!fight?.firstCareerWin);area.classList.toggle('expanded',attributeAssignmentExpanded);
  ...
  function renderPostFightTutorial(win){
    const tutorial=$('#postFightTutorial');tutorial.hidden=state.postFightTutorialSeen&&!fight?.firstCareerWin;$('#postFightTutorialReward').textContent=win?(firstWinUpgradePending()?'Choose your first permanent upgrade to continue.':'Your Attribute Points are available on the Fight screen.'):'Attribute Points are awarded only when you win.';
  }
  function assignAttribute(attribute){
    const resultUpgrade=firstWinUpgradePending();
    if(!LOGIC.assignAttributePoint(state,attribute))return;
    if(resultUpgrade){fight.firstWinUpgradeRequired=false;fight.firstWinUpgradeAttribute=attribute}
    saveState();flashAttributeStats([attribute],1);trackEvent('attribute_point_assigned',{attribute,points_remaining:state.attributePoints,new_value:state.stats[attribute]});sfx.level();toast(`+1 ${attribute.toUpperCase()} · ${state.attributePoints} POINT${state.attributePoints===1?'':'S'} SAVED`,'#6ed7ff');updateUI();if(resultUpgrade){$('#postFightTutorialReward').textContent=`+1 ${attribute.toUpperCase()} added. Your fighter is stronger for the next fight.`;$('#continueBtn').focus()}
  }
  function prepareCareerBuilder(){
    if(state.nameLocked)return;
    if(!state.fighterAvatar){if(!currentCity())state.fighterCity=fighterCities[rint(0,fighterCities.length-1)].id;ensureFighterDraft()}
    if(state.fighterNameDraft){identitySuggestion=state.fighterNameDraft;identityManualMode=state.fighterNameDraftManual===true;identityManualValue=identityManualMode?state.fighterNameDraft:''}
    updateUI();
    const target=state.fighterAvatar?$('#fighterNameSetup'):$('#fighterSetup');target.setAttribute('tabindex','-1');requestAnimationFrame(()=>target.focus({preventScroll:true}));
  }
  function ensureFighterDraft(){if(!state.fighterCity||state.fighterAvatar)return null;if(!fighterAvatars.some(avatar=>avatar.id===state.fighterDraftAvatar))state.fighterDraftAvatar=fighterAvatars[rint(0,fighterAvatars.length-1)].id;if(!validFighterAllocation(state.fighterDraftStats))state.fighterDraftStats=LOGIC.rollFighterAllocation();return {avatar:fighterAvatars.find(item=>item.id===state.fighterDraftAvatar),stats:state.fighterDraftStats}}
  function renderFighterBuilder(){const draft=ensureFighterDraft();if(!draft)return;const hometown=$('#fighterBuildCity');if(!hometown.options.length)hometown.innerHTML=fighterCities.map(city=>`<option value="${city.id}">${city.name}</option>`).join('');hometown.value=state.fighterCity;const index=fighterAvatars.indexOf(draft.avatar),archetype=LOGIC.fighterArchetypeFromStats(draft.stats),style=fighterStyles.find(item=>item.id===archetype);$('#fighterBuildPortrait').src=draft.avatar.asset;$('#fighterBuildPortrait').alt=`Fighter ${index+1}`;$('#fighterBuildArchetype').textContent=style?.name||'STRIKER';$('#fighterBuildArchetypeReason').textContent=archetype==='grappler'?'Chin and cardio lead this build.':'Power and speed lead this build.';renderProfileAttributes({chart:$('#fighterBuildDonut'),totalLabel:$('#fighterBuildTotal'),legend:$('#fighterBuildAttributes')},draft.stats);}
  function chooseCity(id){if(state.fighterAvatar||state.nameLocked)return;const city=fighterCities.find(c=>c.id===id);if(!city)return;state.fighterCity=id;ensureFighterDraft();trackEvent('career_setup_step',{step:'city',selection:id});sfx.tap();updateUI()}
  ...
  function clearIdentityRetry(){identityStatusMessage='';identityRetryable=false;state.fighterNameDraft='';state.fighterNameDraftManual=false}
  function rerollFighterIdentity(){
    if(state.nameLocked||identityPending||identityShufflePending)return;clearIdentityRetry();identityManualMode=false;identityManualValue='';identityShufflePending=true;const button=$('#newFighterNameBtn');button.classList.add('shuffling');renderCareer();sfx.tap();
  ...
  function toggleManualFighterIdentity(){
    if(state.nameLocked||identityPending||identityShufflePending)return;clearIdentityRetry();identityManualMode=!identityManualMode;identityManualValue=identityManualMode?(identityManualValue||identitySuggestion):'';renderCareer();sfx.tap();if(identityManualMode)setTimeout(()=>{$('#manualFighterNameInput').focus();$('#manualFighterNameInput').select()},0)
  }
  function updateManualFighterIdentity(event){clearIdentityRetry();identityManualValue=event.target.value;renderCareer()}
  async function claimFighterName(profile){
    let timer;try{return await Promise.race([SHARED_FEED.claimIdentity(profile),new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error('Name confirmation timed out')),15000)})])}finally{clearTimeout(timer)}
  }
  async function lockFighterIdentity(){
    ...
    state.fighterNameDraft=requested;state.fighterNameDraftManual=manualRequested;saveState();
    if(!SHARED_FEED?.configured?.()||!SHARED_FEED.claimIdentity){identityRetryable=true;identityStatusMessage='The fighter network is unavailable. Your build and name are saved. Retry when connected.';renderCareer();return}
    identityPending=true;identityRetryable=false;identityStatusMessage='Reserving your name on the shared roster…';renderCareer();
    try{
      const candidates=manualRequested?[requested]:identityClaimCandidates(requested),profile=await claimFighterName(Object.assign(sharedProfilePayload(),{candidates}));if(!profile?.id||!normalizeIdentityName(profile.handle))throw new Error('Unique fighter name was not returned.');
      state.name=normalizeIdentityName(profile.handle);state.nameLocked=true;state.rookieShowcasePending=true;state.firstContractPending=false;state.socialProfileId=profile.id;clearIdentityRetry();identitySuggestion='';identityManualMode=false;identityManualValue='';ensureRookieShowcaseOpponent();trackEvent('career_setup_step',{step:'name'});trackEvent('career_started',{archetype:state.fighterStyle,city:state.fighterCity,avatar:state.fighterAvatar});createSocialAccount();saveState();registerPendingReferral().catch(error=>console.warn('Referral registration pending:',error));sfx.win();confettiBurst();toast(`@${state.name} IS YOURS. YOU’RE ON THE ROSTER.${requested!==state.name?` @${requested} WAS TAKEN.`:''}`,'#76dcff');updateUI();offerRookieShowcase();connectSharedSocial(true);
    }catch(error){const unavailable=manualRequested&&/no unique cage grind name/i.test(String(error?.message||error));identityRetryable=!unavailable;identityStatusMessage=unavailable?`@${requested} is already on the roster. Choose another name.`:'Couldn’t confirm your name. Your build and name are saved. Retry when connected.';console.warn('Cage identity claim failed:',error)}finally{identityPending=false;renderCareer()}
  }
  ...
  function sponsorFeedProfile(sponsorId){const sponsor=endorsementDefs.find(item=>item.id===sponsorId);if(!sponsor)return null;return {id:sponsor.id,author:sponsor.brand,handle:sponsor.handle||`@${sponsor.brand.replace(/[^A-Za-z0-9]/g,'')}`,tone:'sponsor',avatar:sponsorLogo(sponsor),website:sponsor.website||'',pilot:sponsor.pilot===true,verified:true,bio:sponsor.bio||`${sponsor.brand} supports Cage Grind fighters through ${sponsor.product.toLowerCase()}.`}}
  ...
  function socialPostTime(post){const time=Date.parse(post?.createdAt||'');return Number.isFinite(time)?time:0}
  function mergedSocialPosts(localPosts,remotePosts){const remoteTextCounts=new Map();for(const post of remotePosts){const text=String(post?.text||'');remoteTextCounts.set(text,(remoteTextCounts.get(text)||0)+1)}const localOnly=localPosts.filter(post=>{const text=String(post?.text||''),matches=remoteTextCounts.get(text)||0;if(!matches)return true;remoteTextCounts.set(text,matches-1);return false});return [...localOnly,...remotePosts].sort((a,b)=>socialPostTime(b)-socialPostTime(a))}
...
function sharedProfilePayload(){return {combatStats:Object.fromEntries(['power','speed','chin','cardio'].map(key=>[key,effectiveStat(key)])),defendingPlan:LOGIC.normalizeFightPlan(state.fightPlanPreference),fightSkinAura:Math.floor(effectiveAura()),city:state.fighterCity,archetype:state.fighterStyle,fighterAvatar:state.fighterAvatar,level:state.level,wins:state.wins,losses:state.losses,draws:state.draws||0,attributeTotal:Object.values(state.stats).reduce((sum,value)=>sum+Math.max(0,Number(value)||0),0),rankingHistory:state.rankingHistory}}
  ...
  function referralRpcUnavailable(error){return error?.status===404||/cage_fighter_referral|schema cache/i.test(String(error?.message||error||''))}
  async function registerPendingReferral(){
    if(!pendingReferralInviterId||!SHARED_FEED?.registerReferral)return false;
    try{const registered=await SHARED_FEED.registerReferral(pendingReferralInviterId);trackEvent('fighter_referral_registered',{registered:registered===true});clearReferralInvite();return registered===true}
    catch(error){if(!referralRpcUnavailable(error)){clearReferralInvite();console.warn('Cage referral registration failed:',error)}return false}
  }
  async function syncReferralRewards(){
    if(!state.nameLocked||!SHARED_FEED?.qualifyReferral)return false;
    try{
      await registerPendingReferral();await SHARED_FEED.qualifyReferral();
      if(pendingReferralDrop||pendingResultDrop||!SHARED_FEED.claimReferralReward||!eligibleGearAtLevel(state.level,'').length)return false;
      const referral=await SHARED_FEED.claimReferralReward();if(!referral?.referralId)return false;
      const drop=awardReferralCollectible(referral.referralId);if(!drop)return false;
      pendingReferralDrop={drop,inviteeHandle:referral.inviteeHandle};saveState();updateUI();trackEvent('fighter_referral_reward_earned',{invitee_handle:referral.inviteeHandle,gear_id:drop.item.id});requestAnimationFrame(showPendingReferralDrop);return true;
    }catch(error){if(!referralRpcUnavailable(error))console.warn('Cage referral reward sync failed:',error);return false}
  }
  function scheduleSharedSocialRefresh(){
    clearTimeout(sharedSocialRefreshTimer);sharedSocialRefreshTimer=null;
    if(!['feed','fight'].includes(currentScreen)||sharedSocialStatus!=='ready')return;
    sharedSocialRefreshTimer=setTimeout(()=>{
      sharedSocialRefreshTimer=null;
      if(!['feed','fight'].includes(currentScreen))return;
      if(currentScreen==='fight'&&(fight||combatLocked)){scheduleSharedSocialRefresh();return}
      connectSharedSocial(true);
    },30000);
  }
  async function connectSharedSocial(force=false){
    ...
    sharedSocialSyncPromise=(async()=>{
      ...
      await dailyNews.profile(profile);
      ...
      const [postsResult,profilesResult,candidatesResult,seedsResult,interactionsResult,championshipResult]=await Promise.allSettled([SHARED_FEED.loadFeed(50),SHARED_FEED.loadProfiles(1000),SHARED_FEED.loadOpponentCandidates(state.level,20),SHARED_FEED.loadSeedFighterRoster(),SHARED_FEED.loadInteractionAllowance(),SHARED_FEED.loadChampionship()]),postsLoaded=postsResult.status==='fulfilled',profilesLoaded=profilesResult.status==='fulfilled',candidatesLoaded=candidatesResult.status==='fulfilled',seedsLoaded=seedsResult.status==='fulfilled';let posts=postsLoaded?postsResult.value:[],profiles=profilesLoaded?profilesResult.value:[],opponentCandidates=candidatesLoaded?candidatesResult.value:[],seedFighters=seedsLoaded?seedsResult.value:[];if(!profilesLoaded&&!candidatesLoaded&&!seedsLoaded)throw profilesResult.reason||candidatesResult.reason||seedsResult.reason||new Error('Fighter roster unavailable.');const interactionsRemaining=interactionsResult.status==='fulfilled'?interactionsResult.value:sharedSocialInteractionsRemaining,championship=championshipResult.status==='fulfilled'?championshipResult.value:sharedChampionship,rankedProfiles=[...(Array.isArray(profiles)?profiles:[]),...(Array.isArray(seedFighters)?seedFighters:[]),...(Array.isArray(opponentCandidates)?opponentCandidates:[])].filter((item,index,all)=>item?.id&&all.findIndex(candidate=>candidate?.id===item.id)===index);if(championshipResult.status==='fulfilled'){setSharedChampionship(championship);queueTitleLossPresentation(sharedChampionship);landingFeature.setAvailability(sharedChampionship,true,false)}else if(!landingFeature.status().championshipLoaded)landingFeature.setAvailability(null,true,true);syncRankedOpponents(rankedProfiles);const hasOwnRemotePost=Array.isArray(posts)&&posts.some(post=>post.author_id===profile.id);
      const establishedCareer=state.wins+state.losses>0;
      if(postsLoaded&&state.socialAccountCreated&&!establishedCareer&&!hasOwnRemotePost&&!state.socialRemoteInitialized){await SHARED_FEED.publishPost({kind:'player',body:'Hello, fight fans! Stay tuned—the climb starts now.'});state.socialRemoteInitialized=true;saveState();posts=await SHARED_FEED.loadFeed(50)}
      state.socialRemoteInitialized=state.socialRemoteInitialized||state.socialAccountCreated&&(establishedCareer||hasOwnRemotePost||Array.isArray(posts)&&posts.some(post=>post.author_id===profile.id));
      sharedSocialProfiles=[profile,...rankedProfiles.filter(item=>item.id!==profile.id)];const sharedIds=sharedSocialProfiles.map(item=>item.id).slice(0,2000),[headToHeadResult,finishStatsResult]=await Promise.allSettled([SHARED_FEED.loadHeadToHead(sharedIds),SHARED_FEED.loadFinishStats(sharedIds)]);if(headToHeadResult.status==='fulfilled')sharedHeadToHead=headToHeadResult.value;if(finishStatsResult.status==='fulfilled')sharedFinishStats=finishStatsResult.value||{};try{state.socialFollowingCount=await SHARED_FEED.loadProfileCount()}catch{state.socialFollowingCount=sharedSocialProfiles.length}sharedSocialInteractionsRemaining=Math.max(0,Math.min(5,Number(interactionsRemaining)||0));sharedSocialPosts=Array.isArray(posts)?posts.map(mapSharedPost):[];
  ...
  function publishSponsorHighlight(moment){
    const sponsor=state.activeEndorsement?.id,pool=endorsementDefs.find(item=>item.id===sponsor)?.highlightMessages||STRINGS.sponsorHighlights?.[sponsor];if(!moment||!pool||!state.socialAccountCreated)return;
    const previous=state.sponsorHighlightHistory[sponsor],completed=state.wins+state.losses;
    if(previous&&(previous.date===todayKey()||completed-previous.fight<3))return;
    const index=previous?(previous.index+1)%pool.length:Math.floor(Math.random()*pool.length);
    state.sponsorHighlightHistory[sponsor]={date:todayKey(),fight:completed,index};
    state.socialCycle++;addSocialPosts([{profile:'sponsor',sponsorId:sponsor,kind:'sponsor-highlight',text:copyText(pool[index],{name:state.name,moment}),targetProfileId:state.socialProfileId}]);
  }
  ...
  function showPendingTitleLoss(){
    if(!pendingTitleLossPresentation||postFightPresentationBusy())return false;
    const notice=pendingTitleLossPresentation,lostAt=new Date(notice.lostAt),dateCopy=Number.isFinite(lostAt.getTime())?lostAt.toLocaleString([],{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}).toUpperCase():'CAGE NETWORK RESULT';$('#titleLossOpponent').textContent=`@${notice.opponent}`;$('#titleLossMeta').textContent=`BELT CHANGED HANDS · ${dateCopy}`;$('#titleLossRematch').textContent=notice.rematch?'TITLE REMATCH AVAILABLE NOW':'STANDARD TITLE-SHOT RULES APPLY';$('#titleLossModal').classList.add('open');$('#titleLossModal').setAttribute('aria-hidden','false');sfx.lose();return true;
  }
  function closeTitleLoss(goToFight=false){
    const notice=pendingTitleLossPresentation;if(notice)state.lastTitleLossSeenId=Math.max(state.lastTitleLossSeenId,notice.id);pendingTitleLossPresentation=null;$('#titleLossModal').classList.remove('open');$('#titleLossModal').setAttribute('aria-hidden','true');saveState();sfx.tap();if(goToFight){resetFightListPending=true;navTo('fight');requestAnimationFrame(()=>{const heading=$('#opponentList .champion-group');heading?.scrollIntoView({block:'start'});heading?.focus({preventScroll:true});showPostFightFollowup()})}else requestAnimationFrame(showPostFightFollowup);
  }
  ...
  function closeCeoOffice(){$('#ceoOfficeModal').classList.remove('open');$('#ceoOfficeModal').setAttribute('aria-hidden','true');sfx.tap();requestAnimationFrame(showPostFightFollowup)}
  ...
  function openSocialCycle(type,data={}){
    ...
    if(type==='fight'){
      ...
      if(data.win&&data.lowerLevelWin)addHeadline('lowerLevelWin',cycles.lowerLevelWin,Object.assign(values,{followersLost:fmt(Math.abs(Number(data.followersLost)||0)),fighterLevel:Math.max(1,Number(data.fighterLevel)||state.level),opponentLevel:Math.max(1,Number(data.opponentLevel)||1)}));
      else if(data.win){const key=data.winStreak>=2?'fightStreak':'fightWin',entries=key==='fightStreak'?cycles.fightStreakHeadline:cycles[key];addHeadline(key,entries,values)}
      else addHeadline('fightLoss',cycles.fightLoss,values);
      if(['BATTERED','BANGED UP'].includes(data.startingHealthTier)){const hurtPrefix=drawSocialHeadline(data.win?'hurtWin':'hurtLoss',cycles.fightingHurt[data.win?'win':'loss']);if(hurtPrefix)for(const post of posts)if(post.profile==='media')post.text=hurtPrefix.text+post.text;}
    }
    const reporterPosts=posts.filter(post=>post.profile==='media');if(reporterPosts.length){addSocialPosts(reporterPosts);saveState();renderSocial();queueSharedPosts(reporterPosts.map(post=>({kind:'reporter',body:post.text})))}
  }
  ...
  function rankingProfiles(){
    ...
    if(sharedSocialProfiles.length){let found=false;const profiles=sharedSocialProfiles.map(profile=>{const mine=profile.id===state.socialProfileId||String(profile.handle||'').toLowerCase()===String(state.name||'').toLowerCase();if(!mine)return profile;found=true;return {...profile,...player,id:profile.id||player.id}});return found?profiles:[player,...profiles]}
    return [player,...state.roster.filter(opponent=>opponent.network).map(opponent=>({id:opponent.sourceProfileId,handle:opponent.networkHandle,city:opponent.networkCity,archetype:opponent.archetype,fighter_avatar:opponent.fighterAvatar,combat_stats:LOGIC.validCombatStats(opponent),level:opponent.tier,wins:opponent.wins,losses:opponent.losses}))];
  }
  globalThis.cageFighterListDebug=id=>{const profile=currentFighterList().fighters.find(item=>item.id===id);return profile?{id:profile.id,level:profile.level,attributeTotal:profile.attributeTotal,winPercentage:profile.winPercentage,wins:profile.wins,headToHeadDifferential:profile.headToHeadDifferential,isChampion:profile.isChampion}:null};
  function currentFighterList(){const fighters=LOGIC.orderFighters(rankingProfiles(),sharedChampionship,1000,sharedHeadToHead),name=String(state.name||'').toLowerCase(),index=fighters.findIndex(profile=>profile.id===state.socialProfileId||String(profile.handle||'').toLowerCase()===name);return {fighters,profile:index>=0?fighters[index]:null,position:index>=0?index+1:0}}
  ...
  function renderFighterPostResults(){const query=$('#fighterPostSearch').value.trim().replace(/^@/,'').toLowerCase(),ranked=LOGIC.orderFighters(sharedSocialProfiles,sharedChampionship,1000,sharedHeadToHead).map((profile,index)=>Object.assign(profile,{worldRank:index+1})),profiles=ranked.filter(profile=>profile.id!==state.socialProfileId&&(!query||String(profile.handle||'').toLowerCase().includes(query))).slice(0,60);$('#fighterPostResults').innerHTML=profiles.length?profiles.map(profile=>{const avatar=fighterAvatars.find(item=>item.id===profile.fighter_avatar);return `<button class="fighter-post-result" type="button" data-fighter-post-target="${escapeHtml(profile.id)}">${avatar?`<img src="${escapeHtml(avatar.asset)}" alt="">`:'<span class="fighter-post-result-avatar">CG</span>'}<span><b>@${escapeHtml(profile.handle)}</b><small>LEVEL ${profile.level} · ${profile.attributeTotal} ATTR · ${fighterCityCode(profile.city)} · ${profile.wins}-${profile.losses}</small></span><em>SELECT</em></button>`}).join(''):'<div class="fighter-post-empty">NO FIGHTERS MATCH THAT SEARCH</div>'}
  ...
  function cancelFighterPostComposer(){if(fighterInteractionPending)return;if(!fighterPostTarget){closeFighterPostComposer();return}fighterPostTarget=null;$('#fighterPostSearchStep').hidden=false;$('#fighterPostPreviewStep').hidden=true;$('#fighterPostTitle').textContent=STRINGS.social.interactions[fighterPostIntent]?.label||'CHOOSE A FIGHTER';requestAnimationFrame(()=>$('#fighterPostSearch').focus());sfx.tap()}
  async function handleFighterInteraction(choice,target){
    ...
    try{await SHARED_FEED.publishPost({kind:choice.kind,body:choice.text,targetProfileId:target.id});sharedSocialInteractionsRemaining=Math.max(0,sharedSocialInteractionsRemaining-1);const reward=LOGIC.socialInteractionReward(hashSeed(`${choice.id}|${target.id}|${state.socialCycle}|${sharedSocialInteractionsRemaining}`),state.aura),followersGained=changeFollowers(reward.followers),auraBefore=state.aura;state.aura=clamp(state.aura+reward.aura,0,100);const auraGained=state.aura-auraBefore;trackEvent('social_post',{post_type:'fighter_interaction',interaction_kind:choice.kind,posts_remaining:sharedSocialInteractionsRemaining,followers_gained:followersGained,aura_gained:auraGained});saveState();fighterInteractionPending=false;closeFighterPostComposer();await connectSharedSocial(true);updateUI();toast(`POST SENT · +${followersGained} FOLLOWERS · +${auraGained} AURA · ${sharedSocialInteractionsRemaining}/5 LEFT`,'#6ed7ff');sfx.win();requestAnimationFrame(()=>$('#socialTimeline').scrollTo({top:0,behavior:'smooth'}))}
  ...
  function navTo(screen){
    ...
    if(screen==='gym'&&currentScreen!=='gym'&&!sparringSession){sparringSnapshot=null;sparPlanReady=false}
    if(screen==='fight'&&currentScreen!=='fight')resetFightListPending=true;
  ...
  function handleHistoryNavigation(event){
    if(fightBooking){writeHistory('preview','push');return}
    if(fightExitGuarded()){writeHistory('fight','push');openForfeitFightDialog();return}
    if(fight?.ended&&combatLocked){writeHistory('result','push');toast('FINISH THE FIGHT RESULT TO CONTINUE','#ffcf78');return}
    if(fight&&!combatLocked&&$('#opponentProfileModal').classList.contains('open'))closeFightPreview(true);
  ...
  function collectibleBackHtml(item,rarity){
    ...
    return `<div class="collectible-side collectible-back" aria-hidden="true"><div class="gear-top"><span class="rarity-tag">${rarity}</span></div><div class="collectible-back-copy">${sponsorLabel}<h3>${sponsorName||escapeHtml(item.name)}</h3>${sponsorDescription}${perk}${qr}</div><div class="collectible-flip-hint">TAP TO RETURN</div></div>`;
  }
  function collectibleCardHtml(item,{dropStatus='',loadoutCard=false}={}){
    const equipped=(item.category==='Fight Gear'?state.equippedGear:state.equippedPerks).includes(item.id),rarity=item.rarity||'COMMON',rarityClass=rarity.toLowerCase(),levelLocked=state.level<(item.minLevel||1);
    const action=dropStatus?`<div class="gear-status">${escapeHtml(dropStatus)}</div>`:loadoutCard&&equipped?`<button class="equip-btn unequip-btn" type="button" data-equip="${item.id}" aria-label="Unequip ${escapeHtml(item.name)}">UNEQUIP</button>`:levelLocked?`<button class="equip-btn" type="button" disabled>LOCKED · LVL ${item.minLevel}</button>`:`<button class="equip-btn" type="button" data-equip="${item.id}" aria-label="Equip ${escapeHtml(item.name)}">EQUIP</button>`;
    const lockIcon=levelLocked?'<span class="gear-level-lock" aria-hidden="true"><i class="slot-lock"></i></span>':'',cardLabel=levelLocked?`${escapeHtml(item.name)} collectible. Locked until level ${item.minLevel}. Tap for details.`:`${escapeHtml(item.name)} collectible. Tap for details.`;
    const equippedMark=loadoutCard&&equipped?'<span class="gear-equipped-full-check" aria-label="Equipped">✓</span>':'';
    return `<div class="gear collectible-card owned rarity-card-${rarityClass} ${state.unseenGear.includes(item.id)?'new-gear-spotlight':''} ${levelLocked?'level-locked':''} ${dropStatus?'drop-claim-card':''} ${loadoutCard&&equipped?'equipped-full':''}" data-collectible-flip data-collectible-id="${item.id}" tabindex="0" aria-label="${cardLabel}" aria-pressed="false"><div class="collectible-flip"><div class="collectible-side collectible-front" aria-hidden="false">${lockIcon}<div class="gear-top"><span class="rarity-tag">${rarity}</span>${equippedMark}</div>${state.unseenGear.includes(item.id)?'<span class="new-gear-label">NEW</span>':''}<div class="gear-hero"><span class="gear-flair"></span><span class="equip-burst"></span><div class="gear-icon">${gameIcon(item.iconName||item.id,item.icon,item.assetExt)}</div></div><div class="gear-copy"><h3>${escapeHtml(item.name)}</h3></div><div class="gear-footer"><span class="level-tag">MIN LVL ${item.minLevel||1}</span>${action}</div></div>${collectibleBackHtml(item,rarity)}</div></div>`;
  }
  function undiscoveredCardHtml(category,rarity,index){return `<div class="gear collectible-card gear-undiscovered-card rarity-undiscovered-${rarity.toLowerCase()}" role="img" aria-label="Undiscovered ${rarity.toLowerCase()} ${escapeHtml(category)} collectible ${index+1}"><span class="undiscovered-card-mark" aria-hidden="true">?</span><b>UNDISCOVERED</b><small>FIND IN A DROP</small></div>`}
  function loadoutEmptyCardHtml(category,index,hasAvailable){return `<button class="gear collectible-card gear-loadout-empty-card" type="button"${hasAvailable?` data-loadout-category="${LOADOUT_FILTER_BY_CATEGORY[category]}"`:' disabled'} aria-label="Empty ${escapeHtml(category)} loadout slot ${index+1}${hasAvailable?'. Choose an item.':''}"><span class="undiscovered-card-mark" aria-hidden="true">+</span><b>EMPTY LOADOUT SLOT</b><small>${hasAvailable?'CHOOSE ITEM':'NO ITEM AVAILABLE'}</small></button>`}
  function collectionBlockHtml(category,availableItems,undiscoveredItems){
    const rarities=['COMMON','RARE','EPIC','LEGENDARY'],groups=rarities.map(rarity=>{const available=availableItems.filter(item=>(item.rarity||'COMMON')===rarity),hidden=undiscoveredItems.filter(item=>(item.rarity||'COMMON')===rarity),count=available.length+hidden.length;if(!count)return '';return `<section class="gear-rarity-group rarity-undiscovered-${rarity.toLowerCase()}" aria-label="${rarity.toLowerCase()} ${escapeHtml(category)} collection"><div class="gear-rarity-head"><b>${rarity}</b><span>${available.length} AVAILABLE · ${hidden.length} UNDISCOVERED</span></div><div class="gear-grid gear-collection-grid">${available.map(item=>collectibleCardHtml(item)).join('')}${hidden.map((item,index)=>undiscoveredCardHtml(category,rarity,index)).join('')}</div></section>`}).join('');
    return `<div class="gear-collection-block"><div class="page-subhead gear-subhead gear-collection-subhead"><span><b>AVAILABLE + UNDISCOVERED</b><small>${availableItems.length} READY · ${undiscoveredItems.length} HIDDEN</small></span></div>${groups}</div>`;
  }
  function combatLoadoutEffects(items){
    return ['power','speed','chin','cardio'].map(stat=>{
      const bonus=items.filter(item=>item.stat===stat).reduce((sum,item)=>sum+(Number(item.bonus)||0),0);
      return bonus>0?`+${bonus} ${stat.toUpperCase()}`:'';
    }).filter(Boolean);
  }
  function renderGear(){
    const owned=gearItems.filter(g=>gearCount(g.id)>0);
    ...
    const categoryByFilter={combat:'Fight Gear',bling:'Bling',property:'Property & Rides',lifestyle:'Lifestyle'},visibleOrder=[categoryByFilter[gearFilter]||'Fight Gear'],sections=visibleOrder.map(cat=>{const categoryItems=gearItems.filter(g=>g.category===cat),items=owned.filter(g=>g.category===cat),undiscoveredItems=categoryItems.filter(g=>gearCount(g.id)<1);const categoryTotal=categoryItems.length,collectionStatus=`${items.length} / ${categoryTotal} COLLECTIBLES`,fightGear=cat==='Fight Gear',activeItems=equippedForCategory(cat).map(id=>items.find(item=>item.id===id)).filter(Boolean),loadoutStatus=`${activeItems.length}/${loadoutLimit} EQUIPPED`,status=`<span>${collectionStatus}</span><small>${loadoutStatus}</small>`,availableItems=items.filter(item=>!activeItems.includes(item)),auraBonus=activeItems.reduce((sum,item)=>sum+(Number(item.auraBonus)||0),0),healthBonus=activeItems.reduce((sum,item)=>sum+(Number(item.healthRecoverySpeed)||0),0),energyBonus=activeItems.reduce((sum,item)=>sum+(Number(item.energyRecoverySpeed)||0),0),activeEffects=[...combatLoadoutEffects(activeItems),auraBonus?`+${auraBonus} EFFECTIVE AURA`:'',healthBonus?`HEALTH EVERY ${Math.round((HEALTH_RECOVERY_INTERVAL-healthBonus)/1000)} SEC`:'',energyBonus?`ENERGY EVERY ${((ENERGY_RECOVERY_INTERVAL-energyBonus)/1000).toFixed(1)} SEC`:''].filter(Boolean),activeEffect=activeEffects.length?`ACTIVE · ${activeEffects.join(' · ')}`:'',hasAvailable=availableItems.some(item=>state.level>=(item.minLevel||1)),loadoutCards=Array.from({length:loadoutLimit},(_,index)=>activeItems[index]?collectibleCardHtml(activeItems[index],{loadoutCard:true}):loadoutEmptyCardHtml(cat,index,hasAvailable)).join(''),loadoutBlock=`<div class="gear-loadout-shop-block"><div class="page-subhead gear-subhead"><div class="gear-loadout-heading"><b>YOUR LOADOUT</b>${activeEffect?`<small>${activeEffect}</small>`:''}</div><span>${activeItems.length}/${loadoutLimit}</span></div><div class="gear-grid gear-loadout-shop-grid">${loadoutCards}</div></div>`;return `<section class="shop-section" id="gear-panel-${gearFilter}" role="tabpanel" aria-labelledby="gear-tab-${gearFilter}">${loadoutBlock}${collectionBlockHtml(cat,availableItems,undiscoveredItems)}</section>`}).join('');
    const shop=$('#gearShop');shop.innerHTML=sections;const headerCat=visibleOrder[0];if(headerCat){$('#gearCategoryTitle').textContent=headerCat;$('#gearCategorySubtitle').textContent=subtitles[headerCat]||'';const headerOwned=owned.filter(g=>g.category===headerCat);$('#gearCategoryStatus').innerHTML=`<span>${headerOwned.length} / ${gearItems.filter(g=>g.category===headerCat).length} COLLECTIBLES</span><small>${equippedForCategory(headerCat).length}/${loadoutLimit} EQUIPPED</small>`}
  }
  ...
  function moveGearFilter(key){const filters=['combat','bling','property','lifestyle'],current=Math.max(0,filters.indexOf(gearFilter)),next=key==='Home'?0:key==='End'?filters.length-1:(current+(key==='ArrowLeft'?-1:1)+filters.length)%filters.length;setGearFilter(filters[next]);requestAnimationFrame(()=>$(`[data-gear-filter="${filters[next]}"]`)?.focus())}
  function toggleCollectibleCard(card){if(!card)return;const flipped=!card.classList.contains('flipped'),name=card.querySelector('.collectible-front h3')?.textContent||'Collectible',front=card.querySelector('.collectible-front'),back=card.querySelector('.collectible-back'),button=front?.querySelector('button'),item=gearItems.find(entry=>entry.id===card.dataset.collectibleId);card.classList.toggle('flipped',flipped);card.setAttribute('aria-pressed',String(flipped));card.setAttribute('aria-label',`${name} collectible. ${flipped?'Details shown. Tap to return.':'Tap for details.'}`);front?.setAttribute('aria-hidden',String(flipped));back?.setAttribute('aria-hidden',String(!flipped));if(button)button.tabIndex=flipped?-1:0;if(flipped&&item){state.unseenGear=state.unseenGear.filter(id=>id!==item.id);$('[data-collectible-id]').filter(el=>el.dataset.collectibleId===item.id).forEach(el=>{el.classList.remove('new-gear-spotlight');el.querySelector('.new-gear-label')?.remove()});saveState()}if(flipped&&item)trackEvent('collectible_details_viewed',{gear_id:item.id,sponsored:item.sponsored===true,has_qr:!!item.qrAsset});sfx.tap()}
  function openLoadoutFullDialog(trigger,category){const modal=$('#loadoutFullModal'),limit=LOGIC.loadoutCategoryLimit();loadoutDialogReturnFocus=trigger&&typeof trigger.focus==='function'?trigger:null;$('#loadoutFullKicker').textContent=`${category.toUpperCase()} · ${limit}/${limit} SLOTS`;$('#loadoutFullDescription').textContent=`Unequip one ${category} item before equipping another. Each category has two active slots.`;modal.classList.add('open');modal.setAttribute('aria-hidden','false');sfx.lose();requestAnimationFrame(()=>$('#loadoutFullOk').focus())}
  ...
  function toggleEquip(id,trigger){const g=gearItems.find(x=>x.id===id);if(!g||!state.gear.includes(id))return;const perk=g.category!=='Fight Gear',loadout=perk?state.equippedPerks:state.equippedGear,categoryLoadout=equippedForCategory(g.category),eventName=perk?'perk':'gear',at=loadout.indexOf(id);if(at>=0){loadout.splice(at,1);trackEvent(`${eventName}_unequipped`,{gear_id:id,gear_rarity:g.rarity.toLowerCase()});sfx.tap();updateUI();return}if(categoryLoadout.length>=LOGIC.loadoutCategoryLimit()){openLoadoutFullDialog(trigger,g.category);return}loadout.push(id);trackEvent(`${eventName}_equipped`,{gear_id:id,gear_rarity:g.rarity.toLowerCase()});initAudio();sfx.crit();const card=trigger&&trigger.closest('.gear');if(card){card.classList.add('equip-bursting');trigger.textContent='EQUIPPED!';trigger.disabled=true;saveState();setTimeout(updateUI,680)}else updateUI()}
  function opponentHasHistory(o){return (Number(o?.meetings)||0)>0||(Number(o?.winsVsPlayer)||0)>0||(Number(o?.lossesToPlayer)||0)>0}
  function fightRewardRanks(){return {playerRank:0,opponentRank:0}}
  function fightRewardTier(opponent){const ranks=fightRewardRanks(opponent),level=LOGIC.rewardMatchup({playerLevel:state.level,opponentLevel:opponent.min||opponent.tier,...ranks}).xpLevel;return LOGIC.opponentXpTier(opponentWinsToday(opponent),level,state.level)}
  function highestRosterLevel(){
    return rankingProfiles().reduce((highest,profile)=>profile&&!profile.retired_at&&profile.active!==false?Math.max(highest,Math.floor(Number(profile.level)||1)):highest,Math.max(1,state.level));
  }
  function attributeRewardContext(opponent,tierHighestLevel=highestRosterLevel()){return {tierHighestLevel:Math.max(tierHighestLevel,opponent.network?Number(opponent.tier)||1:1),circuit:!opponent.network,playerLevel:state.level,opponentLevel:opponent.tier||opponent.min}}
  function fightWinRewardPreview(opponent,attributeContext=attributeRewardContext(opponent)){
    const ranks=fightRewardRanks(opponent);
    const points=LOGIC.victoryAttributePointReward(attributeContext.playerLevel,attributeContext.opponentLevel,{...attributeContext,opponentWinsToday:opponentWinsToday(opponent)}),playerRating=state.stats.power+state.stats.speed+state.stats.chin+state.stats.cardio,opponentRating=opponent.power+opponent.speed+opponent.chin+opponent.cardio,upset=opponentRating>=playerRating+4,titleWon=opponent.globalChampionship&&!opponent.championDefense,winsToday=opponentWinsToday(opponent),xpTier=fightRewardTier(opponent);
    const result=LOGIC.fightXp({...ranks,playerLevel:state.level,opponentLevel:opponent.tier,won:true,upset,ranked:!!opponent.network&&!opponent.globalChampionship,championship:!!opponent.globalChampionship,titleWon,rival:false,opponentWinsToday:winsToday});
    const backlash=xpTier.tier==='lower_level'&&!LOGIC.sameFighterTier(attributeContext),lowerAura=Math.abs(LOGIC.auraFightChange({...ranks,won:true,...attributeContext})),xpNote=backlash?`-${fightRule('experienceRewards.lowerLevelOpponentFollowerLossPercent',5)}% FOLLOWERS · -${lowerAura} AURA`:xpTier.tier==='lower_level'?'LOWER LEVEL · NO XP':xpTier.tier==='exhausted'?'XP USED TODAY':xpTier.tier==='repeat'?'RUNBACK · 50% XP':'';
    return {points,xp:result.xp,xpNote,xpPenalty:backlash};
  }
  function renderFightLadderRow(opponent,fightsLeft,levelPosition=0,tierHighestLevel){
    const attributeContext=attributeRewardContext(opponent,tierHighestLevel),sameLevel=opponent.network?LOGIC.fighterCompetitiveTier(opponent.tier,attributeContext.tierHighestLevel)===LOGIC.fighterCompetitiveTier(state.level,attributeContext.tierHighestLevel):Number(opponent.tier)===Number(state.level),reward=fightWinRewardPreview(opponent,attributeContext),fights=Math.max(0,Number(opponent.wins)||0)+Math.max(0,Number(opponent.losses)||0)+Math.max(0,Number(opponent.draws)||0),winPercentage=fights?Math.round((Number(opponent.wins)||0)/fights*100):0,titleLocked=opponent.globalChampionship&&opponent.titleCooldown,dailyLocked=fightsLeft<1,rematchRemaining=opponentRematchRemaining(opponent),statsPending=combatStatsPending(opponent),locked=titleLocked||dailyLocked||statsPending||rematchRemaining>0,champion=opponent.isChampion===true,portrait=silhouetteForOpponent(opponent),levelBadge=opponent.network?(levelPosition?`${opponent.tier}.${levelPosition}`:`L${opponent.tier}`):'N/A',attributeTotal=opponent.attributeTotal??LOGIC.fighterAttributeTotal({combat_stats:opponent}),titleLabel=champion?'WORLD CHAMPION':opponent.championDefense?'TITLE DEFENSE':opponent.rookieShowcase?'OPENING FIGHT':opponent.firstContract?'FIRST CONTRACT':opponent.circuitFallback?(opponent.winsVsPlayer>0?'CAGE CIRCUIT REMATCH':'FRESH CAGE CIRCUIT'):'CAGE FIGHTER',country=!opponent.network?opponentCountry(opponent.country):null,countryBadge=country?opponentCountryBadge(opponent.country):'',lockCopy=rematchRemaining>0?`REMATCH IN ${rematchTimeLabel(rematchRemaining)}`:statsPending?'AWAITING STATS SYNC':dailyLocked?'DAILY LIMIT':titleLocked?(opponent.championDefense?'DEFENSE USED':'ATTEMPT USED'):'';
    return `<button class="fight-ranking-row${sameLevel?' same-level':''}${champion?' champion':''}${opponent.championDefense?' title-defense':''}${opponent.circuitFallback||opponent.rookieShowcase||opponent.firstContract?' circuit':''}${locked?' locked':''}" type="button" data-fight-key="${escapeHtml(opponent.key)}" aria-label="${escapeHtml(opponent.name)}, ${opponent.network?`${attributeTotal} total attributes`:`fighting out of ${country.name}`}, level ${opponent.tier}${levelPosition?`, position ${levelPosition} in this level`:""}, ${reward.xp} XP and ${reward.points} Attribute Points for a win${reward.xpNote?`, ${reward.xpNote.toLowerCase()}`:''}${locked?`, ${lockCopy.toLowerCase()}`:''}. Open fighter profile."><strong class="fight-rank-position${levelPosition?' level-position':''}">${levelBadge}</strong><span class="fight-rank-avatar"><img src="${escapeHtml(portrait)}" alt="" loading="lazy">${countryBadge}</span><span class="fight-rank-identity"><b>${escapeHtml(opponent.networkHandle?`@${opponent.networkHandle}`:opponent.name)}</b><small><span>${titleLabel}</span>${sameLevel?`<span class="same-level-badge">${opponent.network?'YOUR TIER':'YOUR LEVEL'}</span>`:''}</small><em>PRO ${opponent.wins}-${opponent.losses} · LVL ${opponent.tier}${opponent.network?` · ${attributeTotal} ATTR`:""} · ${winPercentage}% WIN</em></span><span class="fight-rank-record"><small>RECORD</small><b>${opponent.wins}-${opponent.losses}</b></span><span class="fight-rank-level"><small>${opponent.network?"ATTR":"LEVEL"}</small><b>${opponent.network?attributeTotal:opponent.tier}</b></span><span class="fight-rank-win"><small>WIN %</small><b>${winPercentage}%</b></span><span class="fight-rank-rewards"><b${reward.xp===0?' class="zero"':''}>+${reward.xp} XP</b><em${reward.points===0?' class="zero"':''}>+${reward.points} ATTR PT${reward.points===1?'':'S'}</em>${lockCopy?`<small${rematchRemaining>0?` data-rematch-until="${Date.now()+rematchRemaining}"`:""}>${lockCopy}</small>`:reward.xpNote?`<small class="xp-note${reward.xpPenalty?' penalty':''}">${reward.xpNote}</small>`:''}</span></button>`;
  }
  function renderPlayerRankingRow(profile,position){
    if(!profile||!position)return '';const fights=Math.max(0,Number(profile.wins)||0)+Math.max(0,Number(profile.losses)||0)+Math.max(0,Number(profile.draws)||0),winPercentage=fights?Math.round((Number(profile.wins)||0)/fights*100):0,avatar=fighterAvatars.find(item=>item.id===profile.fighter_avatar)?.asset||currentAvatar()?.asset||fighterSilhouettes[0],champion=profile.isChampion===true;
    return `<button class="fight-ranking-row player${champion?' champion':''}" type="button" data-own-fighter-profile aria-label="Your fighter, ${profile.attributeTotal} total attributes, level ${profile.level}${profile.levelPosition?`, position ${profile.levelPosition} in this level`:""}, record ${profile.wins}-${profile.losses}, ${winPercentage}% win rate"><strong class="fight-rank-position${profile.levelPosition?' level-position':''}">${profile.levelPosition?`${profile.level}.${profile.levelPosition}`:`L${profile.level}`}</strong><span class="fight-rank-avatar"><img src="${escapeHtml(avatar)}" alt="" loading="lazy"></span><span class="fight-rank-identity"><b>@${escapeHtml(profile.handle)}</b><small><span>${champion?'WORLD CHAMPION · ':''}YOUR FIGHTER</span></small><em>PRO ${profile.wins}-${profile.losses} · LVL ${profile.level} · ${profile.attributeTotal} ATTR · ${winPercentage}% WIN</em></span><span class="fight-rank-record"><small>RECORD</small><b>${profile.wins}-${profile.losses}</b></span><span class="fight-rank-level"><small>ATTR</small><b>${profile.attributeTotal}</b></span><span class="fight-rank-win"><small>WIN %</small><b>${winPercentage}%</b></span><span class="fight-rank-rewards"><b>YOUR FIGHTER</b><em>VIEW PROFILE</em></span></button>`;
  }
  function renderFirstWinNextFight(){
    const panel=$('#firstWinNextFight'),opponent=LOGIC.firstWinCircuitOpponent(state,opponents.filter(opponentAvailable));
    panel.hidden=!opponent;if(!opponent)return;
    $('#firstWinNextName').textContent=opponent.name;
    $('#firstWinNextDetail').textContent=`LEVEL ${opponent.tier} · CAGE CIRCUIT · A real fight. Make your plan count.`;
    const button=$('#firstWinNextButton');button.dataset.fightKey=opponent.key;button.disabled=false;
  }
  function renderOpponents(){
    refreshOpponents();
    renderFirstWinNextFight();
    const fightsLeft=sessionsLeft('fight',DAILY_FIGHT_LIMIT),ranking=currentFighterList(),tierHighestLevel=highestRosterLevel(),tierBands=LOGIC.fighterTierBands(tierHighestLevel),showcase=opponents.filter(item=>item.rookieShowcase),contract=opponents.filter(item=>item.firstContract),ranked=opponents.filter(item=>item.network&&!combatStatsPending(item)),rankedCount=ranked.length+(ranking.profile&&ranking.position?1:0),circuit=opponents.filter(item=>!item.network&&!item.rookieShowcase&&!item.firstContract),circuitRematches=circuit.filter(item=>(item.winsVsPlayer||0)>0).length;
    $('#fightResetClock').hidden=fightsLeft>0;
    setLimitBadge('#fightLimitText',`${fightsLeft} ${fightsLeft===1?'FIGHT':'FIGHTS'} LEFT`);
    $('#rosterSummary').textContent=`${contract.length?`${contract.length} CONTRACT · `:''}${circuit.length} CIRCUIT · ${rankedCount} FIGHTERS · 5 TIERS`;
    const showcaseRows=showcase.map(opponent=>renderFightLadderRow(opponent,fightsLeft,0,tierHighestLevel)).join(''),contractRows=contract.length?`<div class="page-subhead fight-ranking-group circuit-group"><b>YOUR FIRST CONTRACT</b><span>VASO WIN UNLOCKED THIS FIGHT</span></div>${contract.map(opponent=>renderFightLadderRow(opponent,fightsLeft,0,tierHighestLevel)).join('')}`:'',circuitSummary=circuitRematches?`${circuit.length-circuitRematches} FRESH · ${circuitRematches} REMATCH · PRO RECORD`:`${circuit.length} FRESH MATCHUPS · FULL XP · PRO RECORD`,circuitRows=circuit.length?`<div class="page-subhead fight-ranking-group circuit-group"><b>ON-LEVEL CAGE CIRCUIT</b><span>START HERE · ${circuitSummary}</span></div>${circuit.map(opponent=>renderFightLadderRow(opponent,fightsLeft,0,tierHighestLevel)).join('')}`:'';
    const entries=ranked.map(opponent=>({id:opponent.sourceProfileId,level:opponent.tier,attributeTotal:opponent.attributeTotal,order:opponent.fighterListOrder,champion:opponent.isChampion===true,render:levelPosition=>renderFightLadderRow(opponent,fightsLeft,levelPosition,tierHighestLevel)}));
    if(ranking.profile&&ranking.position)entries.push({id:ranking.profile.id,level:ranking.profile.level,attributeTotal:ranking.profile.attributeTotal,order:ranking.position-1,champion:ranking.profile.isChampion===true,render:levelPosition=>renderPlayerRankingRow({...ranking.profile,levelPosition},ranking.position),player:true});
    const byLevel=(a,b)=>a.order-b.order,championEntries=entries.filter(entry=>entry.champion),levelEntries=entries.filter(entry=>!entry.champion).sort(byLevel),visibleEntries=levelEntries.slice(0,visibleFightRankingCount),playerEntry=levelEntries.find(entry=>entry.player);
    const levelPositions=new Map();
    for(const entry of levelEntries){const position=(levelPositions.get(entry.level)||0)+1;levelPositions.set(entry.level,position);entry.html=entry.render(position)}
    for(const entry of championEntries)entry.html=entry.render(0);
    if(playerEntry&&!visibleEntries.includes(playerEntry))visibleEntries.push(playerEntry);
    visibleEntries.sort(byLevel);
    const championRows=championEntries.length?`<div class="page-subhead fight-ranking-group champion-group" tabindex="-1"><b>WORLD CHAMPION</b><span>REIGNING CHAMPION</span></div>${championEntries.map(entry=>entry.html).join('')}`:'',hasMore=visibleFightRankingCount<levelEntries.length,loadMore=hasMore?`<div class="fight-ranking-load-more" id="fightRankingsLoadMore" role="status">SHOWING ${Math.min(visibleFightRankingCount,levelEntries.length)} OF ${levelEntries.length} · SCROLL FOR MORE</div>`:'';
    const wrapGroup=html=>html?`<div class="fight-ranking-section">${html}</div>`:'',tierGroups=[];
    for(const entry of visibleEntries){const number=LOGIC.fighterCompetitiveTier(entry.level,tierHighestLevel);let group=tierGroups.at(-1);if(!group||group.number!==number){group={...tierBands[number-1],rows:[]};tierGroups.push(group)}group.rows.push(entry.html)}
    const rankedRows=tierGroups.map(group=>wrapGroup(`<div class="page-subhead fight-ranking-group ranked-group"><b>${group.name.toUpperCase()} · TIER ${group.number}</b><span>LEVELS ${group.minLevel}–${group.maxLevel}</span></div>${group.rows.join('')}`)).join('')+loadMore;
    $('#opponentList').innerHTML=opponents.length||ranking.profile?`<div class="fight-ranking-list">${wrapGroup(championRows)}${showcaseRows}${wrapGroup(contractRows)}${wrapGroup(circuitRows)}${rankedRows}</div>`:`<div class="gear-empty"><b>BUILDING THE FIGHT CARD</b><span>New opponents will appear shortly.</span></div>`;
    if(resetFightListPending)resetFightListScroll();
  }

  function resetFightListScroll(){
    if(!resetFightListPending||currentScreen!=='fight'||fight)return;
    $('#opponentList').scrollTop=0;resetFightListPending=false;
  }

  function maybeLoadMoreFightRankings(){
    const trigger=$('#fightRankingsLoadMore'),scroller=$('#opponentList');if(currentScreen!=='fight'||!trigger||!scroller)return;if(scroller.scrollTop+scroller.clientHeight<scroller.scrollHeight-240)return;visibleFightRankingCount+=FIGHT_RANKING_BATCH_SIZE;renderOpponents();
  }
  ...
  async function requestGameInstall(){
    ...
    if(result.status==='accepted'){toast('INSTALLING CAGE GRIND','#77d13e');return}
    if(result.status==='dismissed'){toast('INSTALL CANCELLED','#ffcf78');return}
  ...
  function clearFightTimers(){fightTimers.forEach(clearTimeout);fightTimers=[];walkoutPending=false;$('#walkoutModal').classList.remove('open');$('#walkoutModal').setAttribute('aria-hidden','true');stopHomeMusicPreview();walkoutMusic.stop()}
  ...
  function opponentFightAura(o){const explicit=Number(o?.aura);return Number.isFinite(explicit)?clamp(Math.round(explicit),0,100):hashSeed(`${o?.sourceProfileId||o?.key||o?.name||'opponent'}|crowd-aura-v1`)%101}

  function createFight(o){
    const rewardRanks={playerRank:0,opponentRank:0};
    ...
    return {o,rewardRanks,attributeRewardContext:attributeRewardContext(o),playerLevelAtBooking:state.level,seedRankingSnapshot:LOGIC.rankingFightSnapshot({playerLevel:o.tier,opponentLevel:state.level,ranked:true,opponentRank:rewardRanks.playerRank}),rankingSnapshot:LOGIC.rankingFightSnapshot({playerLevel:state.level,opponentLevel:o.tier,ranked:!!o.network,championship:!!o.globalChampionship,opponentRank:rewardRanks.opponentRank}),opponentPlan:LOGIC.normalizeFightPlan(o.defendingPlan),player:P,opp:O,playerAura:Math.floor(effectiveAura()),opponentAura:opponentFightAura(o),playerCondition:LOGIC.startingFightCondition(state.health,state.maxHealth),startingHealthTier:LOGIC.healthTierName(state.health,state.maxHealth),oppCondition:100,healthLost:0,rounds:[],timeline:[],totals:{player:emptyFightStats(),opp:emptyFightStats()},winner:null,method:'DECISION',submissionMove:null,finishRound:FIGHT_ROUNDS,finishClock:'0:00',ended:false,mode:'planned',gamePlan:Object.assign({},state.fightPlanPreference),planAssessment:null,focus:80,plans:[],lastPlan:state.fighterStyle||'striker',openingApproach:null,tendencyRevealed:true,deepRead:false,crisisUsed:false,cornerTowel:false,haymakerMiss:false,finalDecisionPending:false,lastChanceResolved:false,pendingMoment:null,resolvedMoments:[],roundIntros:[],rocked:{player:0,opp:0}};
  }
  ...
  function sparringTargets(){
    return currentFighterList().fighters.flatMap((profile,index)=>{
      if(profile.id===state.socialProfileId||String(profile.handle||'').toLowerCase()===String(state.name||'').toLowerCase())return [];
      const opponent=state.roster.find(item=>item.network&&item.sourceProfileId===profile.id)||networkOpponentFromProfile(profile,Number(profile.level));if(!opponent||combatStatsPending(opponent))return [];
      return [{id:opponent.sourceProfileId,label:opponent.networkHandle||opponent.name,meta:`LVL ${opponent.tier} · ${profile.attributeTotal} ATTR · ${opponent.tag}`,opponent:{...opponent,worldRank:index+1}}];
    });
  }
  function sparPlanAdvice(assessment,plan){
    const axis=assessment.axis,value=assessment.components[axis],copy={pace:value<0?(plan.pace==='fast'?'Dial back the output and protect your gas tank.':'Raise the tempo before the round slips away.'):(plan.pace==='fast'?'Your cardio supports the higher output.':'The measured pace keeps your best weapons available.'),offense:value<0?(plan.offense==='aggressive'?'Use disciplined entries; the pressure is creating counters.':'Add pressure when the opening appears; safety is surrendering initiative.'):(plan.offense==='aggressive'?'The pressure creates useful finishing windows.':'Disciplined offense is limiting their counters.'),tactics:value<0?(plan.tactics==='adapt'?'Trust your base style longer before making the switch.':'This matchup calls for an in-round style adjustment.'):(plan.tactics==='adapt'?'The adjustment matches what this opponent is showing.':'Your signature style fits this test.')} ;
    return copy[axis];
  }
  ...
  function renderOpenGym(){
    ...
    if(sparringSnapshot&&sparringSnapshot.id!==state.sparringTarget){sparringSnapshot=null;sparPlanReady=false}
    ...
    const query=$('#sparScoutSearch').value.trim().replace(/^@/,'').toLowerCase(),filtered=choices.filter(choice=>choice.label.toLowerCase().includes(query));
    $('#sparTargetChoices').innerHTML=filtered.length?filtered.map(choice=>`<button class="fighter-post-result" type="button" data-spar-target="${escapeHtml(choice.id)}" aria-pressed="${choice.id===state.sparringTarget}"><img src="${escapeHtml(choice.opponent.networkPortrait)}" alt=""><span><b>@${escapeHtml(choice.label)}</b><small>LEVEL ${choice.opponent.tier} · ${LOGIC.fighterAttributeTotal({combat_stats:choice.opponent})} ATTR · ${escapeHtml(fighterCityCode(choice.opponent.networkCity))} · ${choice.opponent.wins}-${choice.opponent.losses} · ${escapeHtml(choice.opponent.tag)}</small></span><em>${choice.id===state.sparringTarget?'SELECTED':'SELECT'}</em></button>`).join(''):`<div class="fighter-post-empty">${choices.length?'NO FIGHTERS MATCH THAT SEARCH':'NO FIGHTERS AVAILABLE · CONNECT TO THE CAGE NETWORK'}</div>`;
    $('#sparSelectedSummary').textContent=sparringSnapshot?`Sparring clone of @${sparringSnapshot.label}`:'Choose a fighter by level and record';
    $('#sparSelectedMeta').textContent=sparringSnapshot?`${sparringSnapshot.meta} · MATCHUP SNAPSHOT`:'No official fight. No career impact.';
    $('#sparFighterPicker').disabled=!!sparringSession;
    ...
    const report=state.lastSparringReport,date=$('#sparReportDate'),output=$('#sparReport');if(!report){date.textContent='AWAITING SESSION';output.innerHTML='<div class="spar-report-empty">CHOOSE A FIGHTER AND RUN YOUR FIRST SCOUT SPAR</div>';return}date.textContent=new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}).format(new Date(report.createdAt)).toUpperCase();const gradeClass=report.grade==='D'?'exposed':report.grade==='C'?'even':'';output.innerHTML=`<article class="spar-report-card"><header class="spar-report-head"><img class="spar-coach-avatar" src="assets/icons/coach-vega.png?v=${ICON_ASSET_VERSION}" alt="Coach Vega"><span><small class="spar-coach-label">COACH VEGA</small><b>${escapeHtml(report.headline)}</b><small>VS ${escapeHtml(report.opponent)} · ${escapeHtml(report.opponentMeta)}</small></span><strong class="spar-grade ${gradeClass}" aria-label="Scout grade ${report.grade}">${report.grade}</strong></header><div class="spar-report-body">${report.recognition?`<p class="spar-coach-recognition">${escapeHtml(report.recognition)}</p>`:''}<p>This is a coaching read, not an official result. The tape stays here until your next session.</p><div class="spar-report-grid"><div class="spar-report-note"><small>ATTRIBUTE PRIORITY · ${escapeHtml(report.attribute)}</small><b>${escapeHtml(report.attributeAdvice)}</b></div><div class="spar-report-note"><small>PLAN ADVICE</small><b>${escapeHtml(report.planAdvice)}</b></div></div></div></article>`;
  }
  function selectSparTarget(id){if(sparringSession)return;const target=sparringTargets().find(choice=>choice.id===id);if(!target)return;if(state.lastSparringReport?.target===id&&!sparPlanReady){sparringSnapshot=structuredClone(target);state.sparringTarget=id;prepareSparRematch();closeSparScout();saveState();return}sparringSnapshot=structuredClone(target);sparPlanReady=true;state.sparringTarget=id;closeSparScout();saveState();renderOpenGym();sfx.tap()}
  function selectSparSetting(setting,value){const allowed={pace:['slow','fast'],offense:['conservative','aggressive'],tactics:['stick','adapt']};if(sparringSession||!sparPlanReady||!sparringSnapshot||!allowed[setting]?.includes(value))return;state.sparringPlan[setting]=value;saveState();renderOpenGym();sfx.tap()}
  function prepareSparRematch(){if(sparringSession||!sparringSnapshot||state.lastSparringReport?.target!==sparringSnapshot.id)return;sparPlanReady=true;renderOpenGym();sfx.tap()}
  function renderSparringProgress(){
    const session=sparringSession,active=!!session;
    $('#sparSession').hidden=!active;$('#startSparBtn').disabled=active||!sparringSnapshot||!sparPlanReady;$('#startSparBtn').textContent=active?'SPARRING IN PROGRESS…':'START ONE-ROUND SPAR';
    $$('[data-spar-target], #sparFighterPicker').forEach(button=>button.disabled=active);
    $$('[data-spar-setting]').forEach(button=>button.disabled=active||!sparringSnapshot||!sparPlanReady);
    $('#sparRematchBtn').hidden=!sparringSnapshot||sparPlanReady||state.lastSparringReport?.target!==sparringSnapshot.id;
    $('#sparRematchBtn').disabled=active;
    $('#sparPlanStatus').textContent=active?'SPARRING IN PROGRESS':sparPlanReady?'MAKE YOUR THREE CALLS':state.lastSparringReport?.target===sparringSnapshot?.id&&sparringSnapshot?'CHOOSE A NEW FIGHTER OR REMATCH':'SELECT A SCOUT TO SET YOUR PLAN';
    if(!session)return;
    const percent=Math.min(100,Math.floor((performance.now()-session.startedAt)/10000*100));
    $('#sparSessionPercent').textContent=`${percent}%`;$('#sparSessionFill').style.width=`${percent}%`;$('#sparSessionTrack').setAttribute('aria-valuenow',String(percent));
    const phases=['Touch gloves. Find your range.','Settle into the pace. Work your entries.','Keep your shape. Test the openings.','Final exchanges. Finish the round strong.','Round complete. Your coach is reviewing the tape.'];
    const phase=Math.min(4,Math.floor(percent/22));if(session.phase!==phase){session.phase=phase;$('#sparSessionStatus').textContent=phases[phase]}
  }
  function startSparringSession(){
    if(sparringSession||!state.nameLocked||!sparPlanReady)return;
    const target=sparringSnapshot,career=state;if(!target){toast('Choose a fighter to scout.','#91b7a0');return}
    ...
    if(currentScreen==='gym')$('#sparSession').scrollIntoView({behavior:globalThis.matchMedia?.('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'nearest'});
    const timer=setInterval(()=>{
      if(state!==career){clearInterval(timer);sparringSession=null;renderSparringProgress();return}
      renderSparringProgress();if(performance.now()-sparringSession.startedAt<10000)return;
      clearInterval(timer);sparringSession=null;sparPlanReady=false;report.createdAt=Date.now();state.lastSparringReport=report;saveState();renderOpenGym();
      trackEvent('open_gym_spar_completed',{target:target.id,grade:report.grade});sfx.win();toast(`SCOUTING REPORT READY · GRADE ${report.grade}`,'#6ed7ff');
      if(currentScreen==='gym')$('#sparReport').scrollIntoView({behavior:globalThis.matchMedia?.('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'nearest'});
    },100);
  }
  ...
  function scoreRoundState(rs){
    ...
    if(pMetric>oMetric||(pMetric===oMetric&&Math.random()<.5)){rs.scoreP=10;rs.scoreO=(pMetric-oMetric>dominantMargin||rs.player.kd>rs.opp.kd)?8:9}else{rs.scoreO=10;rs.scoreP=(oMetric-pMetric>dominantMargin||rs.opp.kd>rs.player.kd)?8:9}
  }

  function simulateRound(sim,round,planId,opening=null){
      if(sim.winner||round>FIGHT_ROUNDS)return;
      const playerStyle=state.fighterStyle||'striker',opponentStyle=sim.o.tendency||'striker';
      const playerRound=LOGIC.combatPlanRound({fighter:sim.player,opponent:sim.opp,plan:sim.gamePlan||state.fightPlanPreference,style:playerStyle,opponentStyle,round});
      const opponentRound=LOGIC.combatPlanRound({fighter:sim.opp,opponent:sim.player,plan:sim.opponentPlan,style:opponentStyle,opponentStyle:playerStyle,round});
      const plan=planDefs.find(p=>p.id===playerRound.style)||planDefs[0],gamePlan=playerRound.plan,adapting=gamePlan.tactics==='adapt'&&round>1,edge=(playerRound.edge-opponentRound.edge)/2,familiarity=planFamiliarity(playerStyle,plan.id),fastPace=playerRound.fast,crowdEdge=LOGIC.auraComebackEdge({round,rounds:sim.rounds,playerAura:sim.playerAura,opponentAura:sim.opponentAura});
      if(round===1)sim.planAssessment=playerRound.assessment;sim.lastPlan=plan.id;sim.plans.push(plan.id);
      const P=sim.player,O=sim.opp,rs={round,plan:plan.id,player:emptyFightStats(),opp:emptyFightStats(),scoreP:0,scoreO:0};let stopped=false;sim.rocked={player:0,opp:0};
      sim.timeline.push({type:'roundStart',round,clock:'5:00'});
      if(crowdEdge){const favorite=crowdEdge>0?P:O;sim.timeline.push({type:'action',round,clock:'4:59',text:`THE FANS ARE TRYING TO WILL ${favorite.name.toUpperCase()}'S COMEBACK INTO EXISTENCE.`,className:'crowd-surge',playerCondition:sim.playerCondition,oppCondition:sim.oppCondition,side:crowdEdge>0?'player':'opp'})}
      if(opening?.damage){const damage=opening.damage,knockdown=damage>=26||sim.oppCondition-damage<32;rs.player.attempted++;rs.player.landed++;rs.player.sig++;rs.player.damage+=damage;if(knockdown)rs.player.kd++;sim.oppCondition=clamp(sim.oppCondition-damage,0,100);sim.timeline.push({type:'action',round,clock:'4:56',text:`HAYMAKER LANDS! ${P.name} detonates a desperate right hand${knockdown?` and drops ${O.name}`:''}!`,className:'big',playerCondition:sim.playerCondition,oppCondition:sim.oppCondition,big:true,landed:true,side:'player'});const finishChance=sim.oppCondition<=0?1:sim.oppCondition<18?.42:0;if(finishChance&&Math.random()<finishChance){sim.winner='player';sim.method='KO';sim.finishRound=round;sim.finishClock='4:54';stopped=true;sim.timeline.push({type:'ko',round,clock:'4:54',text:`IT'S OVER! ${P.name} came back from the brink!`,className:'ko',playerCondition:sim.playerCondition,oppCondition:sim.oppCondition})}else if(knockdown){sim.rocked.opp=Math.round(fightRule('fightFinishes.rockedExchanges',2));sim.timeline.push({type:'rocked',round,clock:'4:53',text:`${O.name} is rocked! ${P.name} surges forward looking for the finish.`,className:'rocked',fighterName:O.name,side:'opp',playerCondition:sim.playerCondition,oppCondition:sim.oppCondition,big:true})}}
      ...
      const fastCount=rint(fightRule('exchangeCounts.fastPaceMinimum',9),fightRule('exchangeCounts.fastPaceMaximum',11)),slowCount=rint(fightRule('exchangeCounts.slowPaceMinimum',6),fightRule('exchangeCounts.slowPaceMaximum',7));
      const exchanges=Math.round(((playerRound.fast?fastCount:slowCount)+(opponentRound.fast?fastCount:slowCount))/2);
      for(let ex=1;ex<=exchanges&&!stopped;ex++){
        const initiativeMod=({striker:.06,grappler:-.01}[playerRound.style]||0)-({striker:.06,grappler:-.01}[opponentRound.style]||0);
        const tendencyInitiative=0;
        const paceInitiative=((playerRound.fast?clamp((P.cardio-O.cardio)*.018+(P.cardio-8)*.008,-.12,.14):0)-(opponentRound.fast?clamp((O.cardio-P.cardio)*.018+(O.cardio-8)*.008,-.12,.14):0))/2;
        const rockedInitiative=((sim.rocked.opp?1:0)-(sim.rocked.player?1:0))*fightRule('fightFinishes.rockedInitiativeSwing',.12);
        const pInitiative=clamp(.5+(P.speed-O.speed)*.022+(P.cardio-O.cardio)*.008+initiativeMod+edge+tendencyInitiative+paceInitiative+rockedInitiative+crowdEdge,.14,.86);
        const side=Math.random()<pInitiative?'player':'opp',defenderSide=side==='player'?'opp':'player',A=side==='player'?P:O,D=side==='player'?O:P,aStats=rs[side],attackPlan=side==='player'?playerRound:opponentRound,defensePlan=side==='player'?opponentRound:playerRound,attackingStyle=attackPlan.style,signedEdge=side==='player'?edge:-edge,attackerRocked=sim.rocked[side]>0,defenderRocked=sim.rocked[defenderSide]>0;
        const type=plannedTechnique(attackingStyle,attackPlan.plan.offense);aStats.attempted++;
        const fatiguePlan=(attackPlan.fast?fightRule('fatigue.fastPaceMultiplier',1.35):fightRule('fatigue.slowPaceMultiplier',.7))*(attackPlan.aggressive?fightRule('fatigue.aggressiveOffenseMultiplier',1.12):fightRule('fatigue.conservativeOffenseMultiplier',.9)),cardioTax=LOGIC.cardioImbalanceFatigue(A),roundFatigue=((round-1)*fightRule('fatigue.laterRoundPenalty',.025)+ex*(Math.max(0,fightRule('fatigue.lowCardioTarget',10)-A.cardio)*fightRule('fatigue.lowCardioPenaltyPerExchange',.0018)+cardioTax))*fatiguePlan;
        ...
        chance+=signedEdge*.72+(side==='player'?crowdEdge:-crowdEdge)+(attackPlan.aggressive?-.045:.05)+(defensePlan.aggressive?.04:-.035);
        if(attackingStyle==='striker'&&type!=='takedown')chance+=.035;
        if(attackingStyle==='grappler'&&type==='takedown')chance+=.14;
        chance+=defenderRocked?fightRule('fightFinishes.rockedAttackAccuracyBonus',.06):0;chance-=attackerRocked?fightRule('fightFinishes.rockedAccuracyPenalty',.09):0;
        ...
        if(landed){
          ...
          const styleDamage={striker:1.12,grappler:.88}[attackingStyle]||1,tacticalDamage=(1+signedEdge)*styleDamage;
          const offenseDamage=(attackPlan.aggressive?1.16:.86);damage=Math.max(1,Math.round((base+A.power*powerScale)*rand(.72,1.22)*clamp(1-D.chin*.014,.55,.94)*tacticalDamage*offenseDamage));
          ...
          const intentKnockdown=(attackPlan.aggressive?.045:-.02),kdChance=clamp(.025+(A.power-D.chin)*.012+(damage-8)*.018+(attackingStyle==='striker'?.02:0)+intentKnockdown+(defenderRocked?fightRule('fightFinishes.rockedKnockdownBonus',.12):0),0,.48);
        ...
        if(landed&&type==='takedown'&&attackingStyle==='grappler'){
          const targetCondition=side==='player'?sim.oppCondition:sim.playerCondition,subChance=LOGIC.submissionFinishChance({speed:A.speed,opponentSpeed:D.speed,cardio:A.cardio,opponentCardio:D.cardio,targetCondition,signature:(side==='player'?playerStyle:opponentStyle)==='grappler',rocked:defenderRocked});
          if(Math.random()<subChance){const submissionMove=selectSubmissionFinish();sim.winner=side;sim.method='SUBMISSION';sim.submissionMove=submissionMove;sim.finishRound=round;sim.finishClock=clock;stopped=true;sim.timeline.push({type:'submission',round,clock,text:`TAP! ${A.name} ${submissionMove.call}! ${D.name} has nowhere to go!`,className:'ko',playerCondition:sim.playerCondition,oppCondition:sim.oppCondition,big:true,healthDamage:side==='opp'?LOGIC.liveFightHealthDamage({finish:'SUBMISSION'}):0})}
        }
        if(!stopped){const targetCondition=side==='player'?sim.oppCondition:sim.playerCondition,koChance=landed?LOGIC.knockoutFinishChance({targetCondition,rocked:defenderRocked,knockdown:kd,damage,power:A.power,chin:D.chin,significant:type!=='jab'&&type!=='takedown'}):0;if(koChance&&Math.random()<koChance){sim.winner=side;sim.method=targetCondition<=0?'KO':'TKO';sim.finishRound=round;sim.finishClock=clock;stopped=true;sim.timeline.push({type:'ko',round,clock,text:`IT'S OVER! ${A.name} gets the stoppage!`,className:'ko',playerCondition:sim.playerCondition,oppCondition:sim.oppCondition,healthDamage:side==='opp'?LOGIC.liveFightHealthDamage({finish:sim.method}):0})}}
        let newlyRocked='';
        if(!stopped&&landed&&type!=='jab'&&type!=='takedown'){
          const rockedNow=kd||Math.random()<LOGIC.rockedChance({significant:true,power:A.power,chin:D.chin,damage,aggressive:attackPlan.aggressive});
          if(rockedNow){sim.rocked[defenderSide]=Math.round(fightRule('fightFinishes.rockedExchanges',2));newlyRocked=defenderSide;if(!defenderRocked)sim.timeline.push({type:'rocked',round,clock,text:`${D.name} is rocked! ${A.name} surges forward looking for the finish.`,className:'rocked',fighterName:D.name,side:defenderSide,playerCondition:sim.playerCondition,oppCondition:sim.oppCondition,big:true})}
        }
        if(!stopped){
          for(const recoveringSide of ['player','opp']){
            if(!sim.rocked[recoveringSide]||recoveringSide===newlyRocked)continue;
            const recoveringFighter=recoveringSide==='player'?P:O,exchangesRocked=Math.max(1,Math.round(fightRule('fightFinishes.rockedExchanges',2))-sim.rocked[recoveringSide]+1);
            sim.rocked[recoveringSide]=Math.max(0,sim.rocked[recoveringSide]-1);
            if(!sim.rocked[recoveringSide]||Math.random()<LOGIC.rockedRecoveryChance({chin:recoveringFighter.chin,cardio:recoveringFighter.cardio,exchangesRocked})){
              sim.rocked[recoveringSide]=0;sim.timeline.push({type:'recovery',round,clock,text:`${recoveringFighter.name} clears the cobwebs and recovers.`,className:'recovery',side:recoveringSide,playerCondition:sim.playerCondition,oppCondition:sim.oppCondition});
            }
          }
        }
  ...
  function settleFightDecision(sim){const score=LOGIC.fightScore(sim.rounds),margin=Math.abs(score.player-score.opponent);sim.winner=(score.player>score.opponent||(score.player===score.opponent&&Math.random()<.5))?'player':'opp';sim.method=margin<=1&&Math.random()<.45?'SPLIT DECISION':'UNANIMOUS DECISION';sim.finishRound=3;sim.finishClock='0:00';sim.finalDecisionPending=false}

  function renderTapeBreakdown(f=fight){
    if(!f?.o)return;const aura=Math.floor(effectiveAura()),opponentAura=f.opponentAura??opponentFightAura(f.o),crowd=aura===opponentAura?'CROWD EVEN':aura>opponentAura?'CROWD FAVORS YOU':`CROWD FAVORS ${f.o.name.toUpperCase()}`,callout=Boolean(f.feedChallengePostId),lowerLevel=f.o.min<state.level&&!callout&&!LOGIC.sameFighterTier(f.attributeRewardContext),upset=f.opp.power+f.opp.speed+f.opp.chin+f.opp.cardio>=f.player.power+f.player.speed+f.player.chin+f.player.cardio+4,rivalry=(f.o.meetings||0)>=2,followerArgs={opponentBaseFollowers:f.o.fans,aura,upset,rivalry,won:true},followerMinimum=LOGIC.fightFollowerReward({...followerArgs,randomMultiplier:.9}),followerMaximum=LOGIC.fightFollowerReward({...followerArgs,randomMultiplier:1.2});$('#tapeAuraBonus').textContent=`CROWD SUPPORT · ${crowd}`;$('#tapeBreakdownFollowers').textContent=lowerLevel?`WIN · −${fightRule('experienceRewards.lowerLevelOpponentFollowerLossPercent',5)}% FOLLOWER BACKLASH`:`WIN · ${fmt(followerMinimum)}–${fmt(followerMaximum)} FOLLOWERS · 0.75 PAYOUT`;$('#tapeBreakdownMode').textContent='PLANNED FULL SIM';$('#tapeEnergy').textContent=`ABOVE 0 REQUIRED · UP TO ${FIGHT_ENERGY_COST} USED`;$('#tapeBreakdownHaymaker').textContent='THE CROWD CAN LIFT ITS FAVORITE WHEN A COMEBACK IS NEEDED.';$('#tapeBreakdownNetwork').hidden=!f.o.network;
  }
  ...
  function renderTapeAttributes(f){
    ...
    attributes.forEach(([label,key,playerId,oppId])=>{
      const playerValue=Number(f.player[key])||0,oppValue=Number(f.opp[key])||0,even=Math.abs(playerValue-oppValue)<.005,playerAdvantage=!even&&playerValue>oppValue,oppAdvantage=!even&&oppValue>playerValue,playerValueEl=$('#'+playerId),oppValueEl=$('#'+oppId),playerMeter=$('#'+playerId+'Meter').parentElement,oppMeter=$('#'+oppId+'Meter').parentElement,row=playerValueEl.closest('.tape-attribute-row');
      playerValueEl.textContent=formatStat(playerValue);oppValueEl.textContent=formatStat(oppValue);playerValueEl.classList.toggle('advantage',playerAdvantage);oppValueEl.classList.toggle('advantage',oppAdvantage);playerValueEl.classList.toggle('even',even);oppValueEl.classList.toggle('even',even);playerMeter.classList.toggle('advantage',playerAdvantage);oppMeter.classList.toggle('advantage',oppAdvantage);playerMeter.classList.toggle('even',even);oppMeter.classList.toggle('even',even);row.classList.toggle('even',even);row.querySelector('b').textContent=`${label}${even?' · TIE':''}`;$('#'+playerId+'Meter').style.width=`${clamp(playerValue/scale*100,4,100)}%`;$('#'+oppId+'Meter').style.width=`${clamp(oppValue/scale*100,4,100)}%`;
    });
  }
  function statsFighterBio({city,country,style,wins=0,losses=0,favorite=false,even=false,showEven=true}){
    const cityName=fighterCities.find(item=>item.id===city)?.name,location=cityName?cityName.toLowerCase().replace(/\b\w/g,letter=>letter.toUpperCase()):country?opponentCountry(country).name:'the Cage Circuit',archetype=style==='grappler'?'A grappler':'A striker';
    const note=favorite?'Comes in as the favorite on attributes.':even?(showEven?'An evenly matched test on attributes.':''):'Comes in as the underdog on attributes.';
    return `${archetype} from ${location} with a ${wins}-${losses} pro record.${note?' '+note:''}`;
  }
  function renderStatsFightSkin(selector,aura,{compact=false}={}){
    const tier=LOGIC.auraTitle(Number(aura)||0),skin=auraFightSkins.find(item=>item.key===tier.key)||auraFightSkins[0],container=$(selector);
    container.style.setProperty('--fight-skin-accent',skin.accent);
    if(compact){container.innerHTML=`<div class="home-fight-skin-display"><img src="${skin.glovesAsset}?v=${ICON_ASSET_VERSION}" alt="${skin.colorName} gloves"><div><div class="home-fight-skin-accessories"><img src="${skin.wrapsAsset}?v=${ICON_ASSET_VERSION}" alt="${skin.colorName} hand wraps"><img src="${skin.mouthguardAsset}?v=${ICON_ASSET_VERSION}" alt="${skin.colorName} mouthguard"></div><b>${skin.label}</b></div><img src="${skin.shortsAsset}?v=${ICON_ASSET_VERSION}" alt="${skin.colorName} shorts"></div>`;return}
    container.innerHTML=`${compact?'':`<small class="tape-stats-kit-owner">${selector==='#tapeStatsPlayerSkin'?'YOUR KIT':'OPPONENT KIT'}</small>`}<div class="tape-stats-kit-items">${[['gloves','gloves'],['wraps','hand wraps'],['mouthguard','mouthguard'],['shorts','shorts']].map(([key,label])=>`<img src="${skin[key+'Asset']}?v=${ICON_ASSET_VERSION}" alt="${skin.colorName} ${label}">`).join('')}</div><b>${skin.label}</b>${compact?'':'<small>FIGHT SKIN · COSMETIC</small>'}`;
  }
  function renderTapeMatchupStats(f){
    const local=CAGE_FIGHT_HISTORY.finishStats(state.fightHistory),shared=sharedFinishStats[state.socialProfileId],playerStats=shared&&shared.wins>local.wins?shared:local,opponentStats=f.o.selfProfile?playerStats:f.o.network?sharedFinishStats[f.o.sourceProfileId]:CAGE_FIGHT_HISTORY.finishStats(state.fightHistory,{opponent:f.o.name,opponentId:f.o.key}),odds=LOGIC.matchupOdds({player:f.player,opponent:combatStatsPending(f.o)?null:f.opp,playerCondition:f.playerCondition,opponentCondition:f.oppCondition});
    const percentage=(stats,key)=>stats?.wins>0?`${Math.round(Math.max(0,Math.min(stats.wins,Number(stats[key])||0))/stats.wins*100)}%`:'—';
    $('#tapePKo').textContent=percentage(playerStats,'ko');$('#tapeOKo').textContent=percentage(opponentStats,'ko');$('#tapePSub').textContent=percentage(playerStats,'sub');$('#tapeOSub').textContent=percentage(opponentStats,'sub');
    $('#tapePOdds').textContent=odds?.playerOdds||'—';$('#tapeOOdds').textContent=odds?.opponentOdds||'—';
    $('#tapeFinishBasis').textContent=`RECORDED WINS · YOU ${playerStats.wins||0} · OPPONENT ${opponentStats?.wins||0}${!f.o.network&&!f.o.selfProfile?' VS YOU':''}`;
  }
  function fillTape(f){
    renderTapeMatchupStats(f);
    renderStatsFightSkin('#tapeStatsPlayerSkin',f.playerAura);
    renderStatsFightSkin('#tapeStatsOpponentSkin',f.o.fightSkinAura||0);
    ...
    const playerAccent=currentAuraFightSkin().accent,opponentAccent=f.o.network?fighterAccent(f.o.networkCity):f.o.color||DEFAULT_FIGHTER_ACCENT,poster=$('#tapeStage .matchup-poster'),playerCard=$('#tapeStage .player-card'),opponentCard=$('#tapeStage .opponent-card');poster.style.setProperty('--player-accent',playerAccent);poster.style.setProperty('--opponent-accent',opponentAccent);playerCard.style.setProperty('--fighter-accent',playerAccent);opponentCard.style.setProperty('--fighter-accent',opponentAccent);applyPortraitStyle(playerCard,state.name);applyPortraitStyle(opponentCard,f.o.networkHandle||f.o.name);
    const titleBout=!!f.o.globalChampionship,rookieShowcase=f.o.rookieShowcase===true,firstContract=f.o.firstContract===true,unknownOpponent=!f.o.network&&!f.o.portraitAsset,playerIsChampion=titleBout&&sharedChampionship?.is_champion===true,playerTag=playerIsChampion?'REIGNING WORLD CHAMPION':currentStyle()?.name||'NO ARCHETYPE',opponentTag=titleBout&&!playerIsChampion?'REIGNING WORLD CHAMPION':f.o.tag||'UNKNOWN STYLE',playerRecord=`PRO ${state.wins}-${state.losses}`,opponentRecord=`PRO ${f.o.wins}-${f.o.losses}`;$('#tapePlayerName').textContent=f.player.name;$('#tapePlayerTag').textContent=playerTag;$('#tapeOppName').textContent=f.opp.name;$('#tapeOppTag').textContent=opponentTag;$('#tapePosterPlayerName').textContent=f.player.name;$('#tapePosterOppName').textContent=f.opp.name;$('#tapeFightDate').textContent=fightPosterDate();$('#tapeStatsPlayerName').textContent=f.player.name;$('#tapeStatsOppName').textContent=f.opp.name;$('#tapeStage .matchup-promo-card').classList.toggle('solo-player-poster',unknownOpponent);
    $('#tapePlayerArt').src=$('#heroFighterArt').src;$('#tapeOppSprite').src=silhouetteForOpponent(f.o);$('#tapeOppSprite').alt=`${f.o.name} ${f.o.network||f.o.portraitAsset?'portrait':'silhouette'}`;$('#tapeOppSprite').classList.toggle('network-portrait',!!f.o.network);$('#tapeOppSprite').classList.toggle('real-portrait',!!f.o.portraitAsset);$('#tapeOppSprite').classList.toggle('story-portrait',rookieShowcase||firstContract);$('#tapeOppSprite').classList.toggle('unknown-silhouette',unknownOpponent);
    renderTapeAttributes(f);$('#tapeTitleBout').hidden=!titleBout;$('#tapeStage .tape-card').classList.toggle('title-bout',titleBout);$('#tapeChampionLabel').textContent=titleBout?(playerIsChampion?`${state.name} IS THE REIGNING CHAMPION`:`${f.o.name} IS THE REIGNING CHAMPION`):'';const advice=firstContract?{tone:'step-up',headline:'YOUR FIRST CONTRACT',message:'Handle Diego’s grappling pressure to prove your debut was no fluke.'}:LOGIC.matchupAdvice({playerLevel:state.level,opponentLevel:f.o.tier,playerRating:f.player.power+f.player.speed+f.player.chin+f.player.cardio,opponentRating:f.opp.power+f.opp.speed+f.opp.chin+f.opp.cardio,titleBout,playerIsChampion,rookieShowcase,...f.attributeRewardContext});const agentRead=$('#tapeAgentRead');agentRead.className=`tape-agent-read ${advice.tone}`;$('#tapeAgentHeadline').textContent=advice.headline;$('#tapeAgentMessage').textContent=advice.message;
    $('#tapeBoutClass').textContent=titleBout?'WORLD TITLE':f.o.network?'ROSTER BOUT':'UNRANKED PRO BOUT';$('#tapeBoutRounds').textContent='3 ROUNDS';$('#tapeCardPlacement').textContent=titleBout?'MAIN EVENT':'MAIN CARD';const xpTier=opponentXpTier(f.o),reward=fightWinRewardPreview(f.o,f.attributeRewardContext),xpStatus=$('#tapeXpStatus'),needsEnergy=opponentCleared&&fightsLeft>0&&!hasActionEnergy(),fightButton=$('#opponentProfileFight'),callout=Boolean(f.feedChallengePostId),playerRating=f.player.power+f.player.speed+f.player.chin+f.player.cardio,opponentRating=f.o.power+f.o.speed+f.o.chin+f.o.cardio,auraWin=LOGIC.auraFightChange({...fightRewardRanks(f.o),won:true,callout,playerLevel:state.level,opponentLevel:f.o.min,...f.attributeRewardContext,upset:opponentRating>=playerRating+4,titleWon:titleBout&&!f.o.championDefense,titleDefense:!!f.o.championDefense,exhausted:xpTier.tier==='exhausted',currentAura:state.aura});xpStatus.textContent=`WIN REWARD · +${reward.xp} XP · +${reward.points} ATTR PT${reward.points===1?'':'S'}${callout?` · CALLOUT +${auraWin} / −10 AURA`:` · ${auraWin>0?'+':''}${auraWin} AURA`}`;xpStatus.className=`tape-xp-status ${xpTier.tier}`;fightButton.hidden=false;fightButton.disabled=fightBooking||!cleared;$('#tapeClearance').classList.toggle('recovery-needed',needsEnergy);const rematchRemaining=opponentRematchRemaining(f.o);if(rematchRemaining>0)$('#tapeClearance').dataset.rematchUntil=Date.now()+rematchRemaining;else delete $('#tapeClearance').dataset.rematchUntil;$('#tapeClearance').textContent=rematchRemaining>0?`REMATCH IN ${rematchTimeLabel(rematchRemaining)}`:f.o.selfProfile?'YOUR FIGHTER · CANNOT FIGHT YOURSELF':combatStatsPending(f.o)?'FIGHTER STATS AWAITING SYNC':!opponentCleared?(f.o.titleCooldown?(f.o.championDefense?'TITLE DEFENSE USED · AVAILABLE AT MIDNIGHT':'TITLE ATTEMPT USED · AVAILABLE AT MIDNIGHT'):'MATCHUP UNAVAILABLE'):!fightsLeft?'DAILY FIGHT LIMIT REACHED · NEW FIGHTS AT LOCAL MIDNIGHT':state.health<MINIMUM_FIGHT_HEALTH?`MEDICAL CLEARANCE REQUIRES ${MINIMUM_FIGHT_HEALTH} HEALTH`:needsEnergy?`0 ENERGY · CHARGING AUTOMATICALLY · ${LOGIC.formatCountdown(LOGIC.recoveryTimeRemaining(state.energy,state.maxEnergy,state.energyRecoveryAt,energyRecoveryInterval()))}`:startingCondition<100?`LIMITED CLEARANCE · ${startingCondition}% STARTING CONDITION`:`CLEARED · FIGHT USES UP TO ${FIGHT_ENERGY_COST} ENERGY`;
    const odds=LOGIC.matchupOdds({player:f.player,opponent:combatStatsPending(f.o)?null:f.opp,playerCondition:f.playerCondition,opponentCondition:f.oppCondition}),playerFavorite=!!odds&&odds.playerProbability>.5,oppFavorite=!!odds&&odds.playerProbability<.5;$('#tapeStatsPlayerMeta').textContent=statsFighterBio({city:state.fighterCity,style:state.fighterStyle,wins:state.wins,losses:state.losses,favorite:playerFavorite,even:!playerFavorite&&!oppFavorite});$('#tapeStatsOppMeta').textContent=statsFighterBio({city:f.o.networkCity,country:f.o.country,style:f.o.tendency,wins:f.o.wins,losses:f.o.losses,favorite:oppFavorite,showEven:false,even:!playerFavorite&&!oppFavorite});
    const edge=(f.player.power+f.player.speed+f.player.chin+f.player.cardio)-(f.opp.power+f.opp.speed+f.opp.chin+f.opp.cardio),matchup=edge>4?'YOU HAVE THE STATISTICAL EDGE':edge<-4?'OPPONENT HAS THE STATISTICAL EDGE':'ATTRIBUTES ARE EVENLY MATCHED',titleAction=f.o.titleMode==='defense'?'DEFEND THE TITLE':f.o.titleMode==='rematch'?'RECLAIM THE TITLE':'FIGHT FOR THE TITLE';$('#walkoutText').textContent=rookieShowcase?ROOKIE_SHOWCASE.headline:firstContract?FIRST_CONTRACT.headline:matchup;$('#tapeFightBtn').textContent='SET FIGHT PLAN';$('#opponentProfileFight').textContent=titleBout?titleAction:'START FIGHT';$('#fightPlanConfirm').textContent=titleBout?titleAction:firstContract?FIRST_CONTRACT.actionLabel:'LOCK IN FIGHT PLAN';renderTapeBreakdown();renderOpponentProfile(f);
  }

  function renderProfileSponsor({wallpaper,surface},sponsor){
    surface.classList.toggle('sponsored',!!sponsor);
    wallpaper.hidden=!sponsor;wallpaper.style.backgroundImage=sponsor?`url("${sponsorLogo(sponsor)}")`:'';
  }
  function renderProfileAttributes({chart,totalLabel,legend},stats){
    const attributes=[['power','var(--attribute-power)'],['speed','var(--attribute-speed)'],['chin','var(--attribute-chin)'],['cardio','var(--attribute-cardio)']].map(([key,color])=>({key,color,value:Math.max(0,Number(stats[key])||0)})),total=attributes.reduce((sum,attribute)=>sum+attribute.value,0);let offset=0;
    const segments=attributes.map(({color,value})=>{const start=offset;offset+=total?value/total*100:0;return `${color} ${start}% ${offset}%`});
    chart.style.background=total?`conic-gradient(${segments.join(',')})`:'#30475c';totalLabel.textContent=formatStat(total);
    chart.setAttribute('aria-label',`Total attributes ${formatStat(total)}: ${attributes.map(({key,value})=>`${key} ${formatStat(value)}`).join(', ')}`);
    legend.innerHTML=attributes.map(({key,color,value})=>`<div style="--attribute-color:${color}"><small>${key.toUpperCase()}</small><b>${formatStat(value)}</b></div>`).join('');
  }
  function renderOpponentProfile(f){
    $('#opponentProfileAvatar').src=silhouetteForOpponent(f.o);$('#opponentProfileAvatar').alt=`${f.o.name} portrait`;
    $('#opponentProfileName').textContent=f.o.networkHandle||f.o.name;$('#opponentProfileName').title=f.o.networkHandle||f.o.name;
    $('#opponentProfileMeta').textContent=`LVL ${f.o.tier}`;$('#opponentProfileRecord').textContent=`${f.o.wins}-${f.o.losses}`;$('#opponentProfileStyle').textContent=f.o.tag||'UNKNOWN STYLE';
    const location=networkOpponentLocation(f.o)?.name||(f.o.country?opponentCountry(f.o.country).name:'Cage Circuit');$('#opponentProfileCity').textContent=location;
    renderProfileAttributes({chart:$('#opponentAttributeDonut'),totalLabel:$('#opponentAttributeTotal'),legend:$('#opponentProfileStats')},f.opp);
    const sponsorPost=f.o.sourceProfileId&&sharedSocialPosts.find(post=>post.targetProfileId===f.o.sourceProfileId&&post.sponsorId),sponsor=f.o.selfProfile?endorsementDefs.find(item=>item.id===state.activeEndorsement?.id):sponsorPost&&sponsorPost.kind!=='sponsor-drop'?endorsementDefs.find(item=>item.id===sponsorPost.sponsorId):null;
    renderProfileSponsor({wallpaper:$('#opponentProfileSponsorWallpaper'),surface:$('#opponentProfileVisual')},sponsor);
    renderStatsFightSkin('#opponentProfileSkin',f.o.fightSkinAura||0,{compact:true});
    const statsPending=combatStatsPending(f.o);$('#opponentProfileRewards').hidden=statsPending||!!f.o.selfProfile;$('#tapeAgentRead').hidden=statsPending||!!f.o.selfProfile;
    if(statsPending){$('#opponentAttributeDonut').style.background='#30475c';$('#opponentAttributeTotal').textContent='—';$('#opponentAttributeDonut').setAttribute('aria-label','Attributes awaiting sync');$('#opponentProfileStats').innerHTML=['power','speed','chin','cardio'].map(key=>`<div><small>${key.toUpperCase()}</small><b>—</b></div>`).join('')}
    $('#opponentProfileRewards').textContent=$('#tapeXpStatus').textContent.replace('WIN REWARD · ','Win reward: ').replaceAll(' · ', ', ');const clearance=$('#tapeClearance').textContent.replace('CLEARED · FIGHT USES UP TO','cleared to fight for up to').replace(' ENERGY',' energy').replaceAll(' · ', ', ');$('#opponentProfileClearance').textContent=($('#opponentProfileRewards').hidden?'':'; ')+clearance+($('#opponentProfileFight').disabled?'':', with no backing out')+'.';
  }

  function showFightStage(stage){['tapeStage','planStage','liveStage','winnerStage'].forEach(id=>$('#'+id).classList.toggle('hidden',id!==stage));$('#opponentProfileModal').classList.toggle('open',stage==='opponentProfileModal');$('#opponentProfileModal').setAttribute('aria-hidden',String(stage!=='opponentProfileModal'));walkoutMusic.setScene('off',effectiveAura());if(stage==='planStage'){walkoutPending=false;$('#fightPlanConfirm').disabled=false;$$('[data-plan-setting]').forEach(button=>button.disabled=false);renderLockerMusic()}}
  ...
  function renderLockerMusic(){
    $('#entranceMusicTrackName').textContent=globalThis.CAGE_MUSIC.trackNameFor(effectiveAura());
    $('#entranceMusicState').textContent=!walkoutMusic.supported?'UNAVAILABLE':walkoutMusic.enabled?'ON':'OFF';
    const toggle=$('#lockerMusicToggle');toggle.disabled=!walkoutMusic.supported;toggle.innerHTML=walkoutMusic.enabled?'<i class="ti ti-music" aria-hidden="true"></i> MUSIC ON':'<i class="ti ti-music-off" aria-hidden="true"></i> MUSIC OFF';toggle.setAttribute('aria-pressed',String(walkoutMusic.enabled));
    $('#lockerMusicVolume').value=Math.round(walkoutMusic.volume*100);$('#lockerMusicVolume').disabled=!walkoutMusic.supported||!walkoutMusic.enabled;
    $('#lockerMusicStatus').textContent=!walkoutMusic.supported?'Music is unavailable in this browser.':'Plays during your walkout.';
  }
  ...
  renderLockerMusic();
  function walkoutCrowdMessage(aura){
    if(aura>=99)return 'A deafening roar shakes the arena. They know a legend has arrived.';
    if(aura>=80)return 'The arena erupts. Your name echoes through the stands.';
    if(aura>=60)return 'The crowd is on its feet, cheering your arrival.';
    if(aura>=40)return 'Cheers build as fans recognize you on the walk to the cage.';
    return 'The fans give you polite applause as you approach the cage.';
  }
  function confirmFightPlan(){
    ...
    const bout=fight,modal=$('#walkoutModal'),message=$('#walkoutMessage'),progress=$('#walkoutProgress'),musicOn=walkoutMusic.enabled&&walkoutMusic.supported,opening=bout.o?.rookieShowcase===true,duration=opening?1500:3000;
    modal.style.setProperty('--walkout-duration',`${duration}ms`);
    $('#walkoutTitle').textContent='LEAVE THE LOCKER ROOM';message.textContent='Plan locked. Time to put it to work.';progress.setAttribute('aria-valuenow','0');
    modal.classList.remove('open');void modal.offsetWidth;modal.classList.add('open');modal.setAttribute('aria-hidden','false');$('#walkoutModal .walkout-dialog').focus();
    walkoutMusic.setScene('walkout',effectiveAura());
    if(opening){$('#walkoutTitle').textContent='YOUR FIRST WALKOUT';message.textContent='The crowd is waiting. Step into the cage.'}else{
    scheduleFight(()=>{$('#walkoutTitle').textContent='MAKE YOUR ENTRANCE';message.textContent=musicOn?'Your entrance music hits. You head toward the cage.':'You walk silently toward the cage.';progress.setAttribute('aria-valuenow','25')},750);
    scheduleFight(()=>{$('#walkoutTitle').textContent='HEAR THE CROWD';message.textContent=walkoutCrowdMessage(effectiveAura());progress.setAttribute('aria-valuenow','50')},1500);
    scheduleFight(()=>{$('#walkoutTitle').textContent='ENTER THE CAGE';message.textContent='You step inside. Time to go to work.';progress.setAttribute('aria-valuenow','75')},2250);
    }
    scheduleFight(()=>walkoutMusic.stop(),duration-200);
    scheduleFight(()=>{if(fight!==bout||!walkoutPending)return;progress.setAttribute('aria-valuenow','100');modal.classList.remove('open');modal.setAttribute('aria-hidden','true');walkoutPending=false;fightPlanFeature.confirm()},duration);
  }
  ...
  function openTaleOfTape(o,options={}){
    if(combatLocked||fightBooking)return;
    const fromFeed=options.fromFeed===true;
    closeTapeStats(false);closeTapeBreakdown(false);clearFightTimers();fight=createFight(o);if(fromFeed&&/^shared-\d+$/.test(String(options.feedChallengePostId||'')))fight.feedChallengePostId=String(options.feedChallengePostId);combatLocked=false;fightTimelineIndex=0;trackEvent('fight_matchup_viewed',{opponent_key:o.key,opponent_archetype:o.tendency,is_rematch:(o.meetings||0)>0,is_title:!!o.globalChampionship,rookie_showcase:!!o.rookieShowcase,first_contract:!!o.firstContract});$('#opponentProfileBack').textContent=fromFeed?'BACK TO FEED':'GO BACK';showFightStage('opponentProfileModal');$('#opponentProfileModal').scrollTop=0;fillTape(fight);$('#opponentProfileBack').focus();writeHistory('preview','push');sfx.tap();
  }
  function closeFightPreview(){const fromHistory=arguments[0]===true;if(combatLocked||fightBooking)return;if(!fromHistory&&history.state?.[HISTORY_KEY]&&history.state.layer==='preview'){history.back();return}closeTapeStats(false);closeTapeBreakdown(false);clearFightTimers();$('#opponentProfileModal').classList.remove('open');$('#opponentProfileModal').setAttribute('aria-hidden','true');$('#opponentProfileBack').textContent='GO BACK';fight=null;sfx.tap()}
  ...
  async function commitFight(o=fight?.o){
    if(!o)return;
    if(combatStatsPending(o)){toast('FIGHTER STATS AWAITING SYNC · TRY AGAIN AFTER THEY RETURN','#ffcf78');return}
    if(combatLocked||fightBooking||state.pendingFight)return;
    const rematchRemaining=opponentRematchRemaining(o);if(rematchRemaining>0){toast(`NO REMATCH · AVAILABLE IN ${rematchTimeLabel(rematchRemaining)}`,'#ffcf78');fillTape(fight);return}
...
fightBooking=true;$('#opponentProfileFight').disabled=true;$('#opponentProfileBack').disabled=true;
    try{
let championshipBout=null;if(o.globalChampionship){$('#opponentProfileFight').textContent='BOOKING TITLE FIGHT…';try{if(!await connectSharedSocial(true))throw new Error(sharedSocialError||'Cage Network connection required.');championshipBout=await SHARED_FEED.beginChampionshipBout(o.sourceProfileId);if(!championshipBout?.challenge_id)throw new Error('The title fight could not be booked.')}catch(error){if(o.championDefense&&String(error?.message||error).includes('That ranked fighter is not available for a title defense')){o=nonTitleOpponent(o);championshipBout=null;fight.o=o;fillTape(fight);toast('NON-TITLE FIGHT · YOUR BELT IS NOT ON THE LINE','#78dfff')}else{toast(fighterSessionMessage(error),'#ff766d');fillTape(fight);return}}}
    ...
    const feedChallengePostId=fight.feedChallengePostId;initAudio();clearFightTimers();fight=createFight(o);state.pendingFight.rankingSnapshot={...fight.rankingSnapshot};state.pendingFight.attributeRewardContext={...fight.attributeRewardContext};fight.championshipBout=championshipBout;if(feedChallengePostId)fight.feedChallengePostId=feedChallengePostId;combatLocked=true;fightTimelineIndex=0;trackEvent('fight_started',{fight_mode:'planned',player_archetype:state.fighterStyle,opponent_key:o.key,opponent_archetype:o.tendency,is_rematch:(o.meetings||0)>0,is_title:!!o.globalChampionship,rookie_showcase:!!o.rookieShowcase,first_contract:!!o.firstContract,energy_spent:booking.energySpent});
    $('#fightOverlay').classList.add('active');$('#actionFeed').innerHTML='';$('#cornerChoice').innerHTML='';writeHistory('fight','replace');sfx.tap();saveState();updateUI();fillTape(fight);showFightStage('tapeStage');$('#tapeStage').scrollTop=0;$('#tapeFightBtn').focus();
    }finally{fightBooking=false;$('#opponentProfileBack').disabled=false;if(!combatLocked&&fight)fillTape(fight)}
  }
  ...
  function resolveFightMoment(choiceId){
    ...
    const playerConditionBefore=fight.playerCondition;fight.playerCondition=clamp(fight.playerCondition-selfDamage,0,100);fight.oppCondition=clamp(fight.oppCondition-damage,0,100);const playerConditionLoss=playerConditionBefore-fight.playerCondition;
  ...
  function showFightEventToast(item){
    if(item.type==='rocked'){toast(`${String(item.fighterName||'FIGHTER').toUpperCase()} IS ROCKED!`,'#fff','','',1900,'fight-rocked-toast');return}
    if(!['ko','submission'].includes(item.type)||!fight?.winner)return;
    const winner=fight.winner==='player'?fight.player:fight.opp,loser=fight.winner==='player'?fight.opp:fight.player,playerWon=fight.winner==='player',message=item.type==='submission'?`${fightMethodLabel(fight).toUpperCase()} · ${winner.name.toUpperCase()} FORCES ${loser.name.toUpperCase()} TO TAP`:`${fight.method} · ${winner.name.toUpperCase()} STOPS ${loser.name.toUpperCase()}`;
    toast(message,'#fff','','',2400,`fight-finish-toast ${playerWon?'fight-finish-win':'fight-finish-loss'}`);
  }
  ...
  function playFightTimeline(index){
    ...
    if(item.type==='roundStart')appendFightLine({type:'roundStart',clock:item.clock,text:`ROUND ${item.round} BEGINS`,className:'round-divider'});else if(item.type==='roundEnd'){appendFightLine({type:'roundEnd',clock:item.clock,text:`ROUND ${item.round} ENDS`,className:'round-divider',playerCondition:item.playerCondition,oppCondition:item.oppCondition});unofficialScoreLine(item.round)}else{appendFightLine(item);showFightEventToast(item)}
    const delay=item.type==='roundStart'?430:item.type==='roundEnd'?560:['ko','submission'].includes(item.type)?900:item.type==='rocked'?650:300;
  ...
  function setResultSectionExpanded(sectionId,expanded){
    const section=$('#'+sectionId);if(!section)return;const toggle=section.querySelector('.result-section-toggle'),body=section.querySelector('.result-section-body');section.classList.toggle('expanded',expanded);toggle?.setAttribute('aria-expanded',String(expanded));if(body)body.hidden=!expanded;
  }

  function toggleResultSection(sectionId){const section=$('#'+sectionId),expanded=section?.classList.contains('expanded')===true;setResultSectionExpanded(sectionId,!expanded);sfx.tap()}

  function buildResultDetails(f){
    $('#resultMethod').textContent=`${fightMethodLabel(f)} · ROUND ${f.finishRound} · ${f.finishClock}`;
    const planAssessment=f.planAssessment||assessFightPlan(f);f.planAssessment=planAssessment;
    const planLabel=fightPlanLabel(f.gamePlan),gradeClass=planAssessment.grade.toLowerCase(),resultTape=$('#resultTape'),playerPortrait=currentAvatar()?.asset||$('#heroFighterArt').src,opponentPortrait=silhouetteForOpponent(f.o),playerTag=fighterCityCode(state.fighterCity),opponentTag=f.o.network?fighterCityCode(f.o.networkCity):opponentCountry(f.o.country).code;
    $('#resultStickyPlayer').textContent=f.player.name;$('#resultStickyOpponent').textContent=f.opp.name;const tapeSummary=$('#resultTapeSummary');tapeSummary.textContent=`${planLabel} — ${planAssessment.grade}`;tapeSummary.className=`result-section-summary ${gradeClass}`;
    resultTape.style.setProperty('--player-accent',currentAuraFightSkin().accent);resultTape.style.setProperty('--opponent-accent',f.o.network?fighterAccent(f.o.networkCity):f.o.color||DEFAULT_FIGHTER_ACCENT);resultTape.innerHTML=`<div class="rt-name rt-player">${escapeHtml(f.player.name)}</div><div class="rt-vs">VS</div><div class="rt-name rt-opponent">${escapeHtml(f.opp.name)}</div><div class="rt-portrait rt-player"><img src="${escapeHtml(playerPortrait)}" alt="${escapeHtml(f.player.name)}"></div><div class="rt-vs rt-portrait-vs">VS</div><div class="rt-portrait rt-opponent"><img src="${escapeHtml(opponentPortrait)}" alt="${escapeHtml(f.opp.name)} ${f.o.network||f.o.portraitAsset?'portrait':'silhouette'}"></div><div class="rt-stats"><div class="rt-player"><b>${formatStat(f.player.power)}/${formatStat(f.player.speed)}/${formatStat(f.player.chin)}/${formatStat(f.player.cardio)}</b><br><small>PWR · SPD · CHN · CAR</small></div><div class="rt-vs">RATINGS</div><div class="rt-opponent"><b>${formatStat(f.opp.power)}/${formatStat(f.opp.speed)}/${formatStat(f.opp.chin)}/${formatStat(f.opp.cardio)}</b><br><small>PWR · SPD · CHN · CAR</small></div></div><div class="rt-focus">FIGHT PLAN · <b>${planLabel}</b><br>PLAN GRADE · <b class="plan-grade ${gradeClass}">${planAssessment.grade}</b></div>`;
    ...
    $('#fightTotals').innerHTML=`<div class="totals-grid"><div class="totals-fighter totals-tag-user">${escapeHtml(playerTag)}</div><div class="totals-vs">VS</div><div class="totals-fighter totals-tag-opponent">${escapeHtml(opponentTag)}</div><div><b>${p.landed}/${p.attempted}</b></div><div class="label">Strikes</div><div><b>${o.landed}/${o.attempted}</b></div><div><b>${p.sig}</b></div><div class="label">Significant</div><div><b>${o.sig}</b></div><div><b>${p.takedowns}</b></div><div class="label">Takedowns</div><div><b>${o.takedowns}</b></div><div><b>${p.control}s</b></div><div class="label">Control</div><div><b>${o.control}s</b></div><div><b>${p.kd}</b></div><div class="label">Knockdowns</div><div><b>${o.kd}</b></div><div><b>${p.damage}</b></div><div class="label">Damage</div><div><b>${o.damage}</b></div></div>`;
    setResultSectionExpanded('resultScorecardSection',false);setResultSectionExpanded('resultTapeSection',false);
  }

  function renderResultBonuses(notes=[],{auraInRewards=false}={}){
    const summary=$('#resultBonuses');if(!summary)return;summary.hidden=!notes.length;
    const context=[],messages=[];
    notes.forEach(note=>{
      const text=String(note.text),split=text.match(/^(CURRENT SPONSOR|AURA) · (.+)$/),ranked=text.match(/^(ROSTER FIGHT BONUS) (.+)$/),parts=split||ranked;
      if(auraInRewards&&split?.[1]==='AURA')return;
      if(parts){const icon=ranked?'🏆':parts[1]==='AURA'?'✦':'⚡';context.push(`<div class="result-context-item${note.kind==='penalty'?' non-positive':''}"><span class="result-context-icon" aria-hidden="true">${icon}</span><b>${escapeHtml(parts[2])}</b><small>${escapeHtml(parts[1])}</small></div>`)}
    ...
    summary.hidden=!context.length&&!messages.length;
  }
  ...
  function renderDailyHeatResult({won=false,lowerLevelWin=false,rankRepeatExhausted=false,forfeited=false,dailyStreakBefore=0,dailyBonusAwarded=false}={}){
    ...
    else if(lowerLevelWin){treatment='reset';heading='LOWER-LEVEL WIN NOT ELIGIBLE';message=dailyStreakBefore?'QUALIFYING STREAK RESET TO ZERO':bonusActive?'DAILY BONUS REMAINS ACTIVE':'WIN AT YOUR LEVEL OR HIGHER TO BUILD HEAT'}
    else if(!won){treatment='reset';heading=forfeited?'FORFEIT · STREAK RESET':'LOSS · STREAK RESET';message=bonusActive?'DAILY BONUS REMAINS ACTIVE':'WIN AT YOUR LEVEL OR HIGHER TO START AGAIN'}
  ...
  function showResultStage(stage='outcome'){
    if(stage==='outcome'&&fight){
      ...
      $('#resultVerdict').classList.toggle('is-loss',!won);
      $('#resultContinueBtn').textContent='COLLECT REWARDS';
      $('#detailsToggle').setAttribute('aria-expanded','false');
    }
    const rewards=stage==='rewards',rewardsStage=$('#resultRewardsStage'),card=$('#resultModal .result-card');
    showFightStage('winnerStage');
    rewardsStage.hidden=!rewards;card.classList.toggle('showing-rewards',rewards);card.scrollTop=0;
    $('#resultDetails').classList.remove('open');
    $('#resultScorecardClose').hidden=true;
    card.setAttribute('aria-labelledby','resultRewardsTitle');
    $('#resultModal').style.display=rewards?'flex':'none';
  ...
  function setRewardClaimReady(ready){const button=$('#continueBtn');if(!button)return;$$('#resultModal button').forEach(action=>{action.disabled=!ready});button.setAttribute('aria-busy',String(!ready));button.classList.toggle('rewards-counting',!ready);renderFirstWinUpgradeGate();if(ready&&$('#resultRewardsStage')?.hidden===false)requestAnimationFrame(()=>{const target=firstWinUpgradePending()? $('[data-first-win-assignment] [data-assign-attribute]'):button;target?.focus()})}
  ...
  function authoritativeChampionshipResult(result,pending){
    return LOGIC.championshipSettlementPresentation({status:String(result?.status||''),mode:pending.mode,defenses:result?.defenses,championHandle:result?.champion_handle});
  }
  function settleChampionshipResult(pending=state.pendingChampionshipResult){
    ...
    if(fight?.championshipBout?.challenge_id===pending.challengeId)renderChampionshipSettlement('pending','CONFIRMING TITLE RESULT');
    championshipSettlementPromise=(async()=>{try{
      const result=await SHARED_FEED.settleChampionshipBout(pending);state.pendingChampionshipResult=null;saveState();
      // Interpret the settled bout from the booked side, never a cached champion flag.
      const official=authoritativeChampionshipResult(result,pending);
      if(fight?.championshipBout?.challenge_id===pending.challengeId){
        renderChampionshipSettlement(`settled ${official.outcome}`,`${official.heading} · ${official.message}`);
        $('#resultTitle').textContent=official.heading;$('#resultTitle').className=official.outcome;$('#resultLine').textContent=official.message;
      }
      trackEvent('championship_result_settled',{settlement_status:String(result?.status||'unknown'),championship_mode:pending.mode});
      toast(official.heading,official.outcome==='loss'?'#ff808b':'#ffd36e','title-world','👑');
      await connectSharedSocial(true);
      // A shared refresh may have started before settlement. Fetch title state after it finishes.
      try{const championship=await SHARED_FEED.loadChampionship();landingFeature.setAvailability(championship,true,false);queueTitleLossPresentation(sharedChampionship)}catch(error){landingFeature.setAvailability(null,true,true);console.warn('Championship refresh will retry on the next sync.',error)}
      updateUI();requestAnimationFrame(showPostFightFollowup);return result;
    }catch(error){if(fight?.championshipBout?.challenge_id===pending.challengeId)renderChampionshipSettlement('pending','CONFIRMING TITLE RESULT · SAVED FOR RETRY');saveState();console.warn('Championship result settlement will retry.',error);setTimeout(()=>{if(state.pendingChampionshipResult&&!championshipSettlementPromise)settleChampionshipResult()},15000);return null}finally{championshipSettlementPromise=null}})();
  ...
  function finishFightSimulation(){
    ...
    const o=fight.o,playerLevelBefore=state.level,ranks=fightRewardRanks(),win=fight.winner==='player',lowerLevelWin=win&&o.min<state.level,sameTier=LOGIC.sameFighterTier(fight.attributeRewardContext),backlashWin=lowerLevelWin&&!sameTier,calloutFight=Boolean(fight.feedChallengePostId),circuitOpponent=!o.network&&!o.rookieShowcase&&!o.firstContract&&!o.globalChampionship,firstCareerWin=win&&state.wins===0,firstContractUnlocked=LOGIC.firstContractUnlockEligible({won:win,rookieShowcase:o.rookieShowcase===true}),winsToday=opponentWinsToday(o),xpTier=fightRewardTier(o),dropEligible=LOGIC.fightDropEligible(winsToday),isRematch=(o.meetings||0)>0,rivalry=(o.meetings||0)>=2,ordinaryRival=!o.network&&!o.globalChampionship&&(o.lossesToPlayer||0)>0,playerRating=fight.player.power+fight.player.speed+fight.player.chin+fight.player.cardio,oppRating=fight.opp.power+fight.opp.chin+fight.opp.speed+fight.opp.cardio,upset=win&&oppRating>=playerRating+4,healthLoss=finalizePersistentFightDamage(fight),titleWon=!!(win&&o.globalChampionship&&!fight.championshipBout?.player_is_champion),titleDefense=!!(win&&o.globalChampionship&&fight.championshipBout?.player_is_champion),xpResult=LOGIC.fightXp({...ranks,playerLevel:state.level,opponentLevel:o.min,won:win,forfeited:!!fight.forfeited,upset,ranked:!!o.network&&!o.globalChampionship,championship:!!o.globalChampionship,titleWon,rival:ordinaryRival,opponentWinsToday:winsToday}),auraBefore=state.aura,auraTitleBefore=LOGIC.auraTitle(state.aura),auraBaseChange=LOGIC.auraFightChange({...ranks,won:win,forfeited:!!fight.forfeited,callout:calloutFight,playerLevel:state.level,opponentLevel:o.min,...fight.attributeRewardContext,upset,titleWon,titleDefense,exhausted:xpTier.tier==='exhausted',currentAura:state.aura});let fans=0,xp=xpResult.xp,auraAppliedChange=0,ceoAura=0,gearDrop=null,attributePoint=0;const lootNotes=[];o.meetings=(o.meetings||0)+1;
    const victoryPackEligible=win&&LOGIC.victoryPackWinEligible({...ranks,playerLevel:state.level,opponentLevel:o.min,repeatEligible:dropEligible}),victoryPackProgressSteps=victoryPackEligible&&(upset||rivalry||fight.method.includes('KO'))?2:victoryPackEligible?1:0;
    if(win){
      o.losses=(o.losses||0)+1;o.lossesToPlayer=(o.lossesToPlayer||0)+1;o.rematchAccepted=false;state.wins++;state.winStreak++;state.bestStreak=Math.max(state.bestStreak,state.winStreak);attributePoint=LOGIC.awardVictoryAttributePoint(state,{...ranks,opponentWinsToday:winsToday,won:true,forfeited:false,playerLevel:state.level,opponentLevel:o.min,...fight.attributeRewardContext});if(backlashWin&&!calloutFight){const penalty=LOGIC.lowerLevelFollowerPenalty(state.fans,{won:true,playerLevel:state.level,opponentLevel:o.min,...fight.attributeRewardContext});fans=changeFollowers(-penalty)}else{fans=changeFollowers(LOGIC.fightFollowerReward({opponentBaseFollowers:o.fans,aura:effectiveAura(),upset,rivalry,randomMultiplier:rand(.9,1.2),won:true}))}state.aura=clamp(state.aura+auraBaseChange,0,100);auraAppliedChange=state.aura-auraBefore;sfx.win();confettiBurst();
      if(victoryPackEligible||titleWon||firstCareerWin)gearDrop=awardDeterministicGearDrop({opponent:o,titleWon,guaranteed:firstCareerWin,progressSteps:victoryPackProgressSteps});ceoAura=awardCeoPerformanceRecognition({upset,ko:fight.method.includes('KO'),titleWon});
      if(backlashWin&&!calloutFight)lootNotes.push({kind:'penalty',text:`FAN BACKLASH · ${fmt(Math.abs(fans))} FOLLOWERS LOST (5%)`});const currentSponsor=endorsementDefs.find(item=>item.id===state.activeEndorsement?.id);if(currentSponsor)lootNotes.push({kind:'sponsor',text:`CURRENT SPONSOR · ${currentSponsor.brand}`});if(firstContractUnlocked)lootNotes.push({kind:'milestone',text:'FIRST CONTRACT UNLOCKED · DIEGORAMOSBR'});if(o.globalChampionship)lootNotes.push({kind:'milestone',text:'CONFIRMING TITLE RESULT'});if(ceoAura)lootNotes.push({kind:'milestone',text:`CEO NOTICED · +${ceoAura} AURA`});if(upset)lootNotes.push({kind:'milestone',text:'UPSET VICTORY'});if(rivalry)lootNotes.push({kind:'milestone',text:'RIVALRY WIN'});if(circuitOpponent)state.circuitLossStreak=0;
      $('#resultTitle').textContent='YOU WIN';$('#resultTitle').className='win';$('#resultLine').textContent=fight.lastChanceLanded?`Ten seconds left, behind on the cards, and one haymaker changed everything.`:fight.method==='SUBMISSION'?`${o.name} taps to a ${(fight.submissionMove?.name||'submission').toLowerCase()}. Your grappling just made a statement.`:fight.method.includes('KO')?`${o.name} could not answer the damage. Your stock just jumped.`:`The scorecards are in. Your hand gets raised.`;
    }else{
      ...
      if(fight.forfeited){$('#resultTitle').textContent='FIGHT FORFEITED';$('#resultLine').textContent=`You left the cage. ${o.name} receives the win, and the loss is official.`}else{$('#resultTitle').textContent='YOU LOST';$('#resultLine').textContent=fight.cornerTowel?`Your corner protected you. ${o.name} gets the TKO win.`:fight.haymakerMiss?'The last-chance haymaker missed, and the counter ended the fight.':fight.method==='SUBMISSION'?`${o.name} forced the tap with a ${(fight.submissionMove?.name||'submission').toLowerCase()}. Rebuild your defense and come back sharper.`:fight.method.includes('KO')?'The referee saves you from more damage. Back to the gym.':'Close the scorecard, remember the lesson, and come back better.'}$('#resultTitle').className='loss';
    }
    state.fightHistory=globalThis.CAGE_FIGHT_HISTORY.append(state.fightHistory,fight,Date.now(),state.fighterStyle);
    dailyNews.record({resultId:fight.resultId,bout:state.wins+state.losses,at:new Date().toISOString(),won:win,opponent:o.networkHandle||o.name,playerRank:ranks.playerRank||0,opponentRank:o.network?ranks.opponentRank||0:0,seedId:o.seeded?o.sourceProfileId:null,opponentId:o.network&&!o.seeded?o.sourceProfileId:null,finishMethod:fight.forfeited?'FORFEIT':fight.method,playerLevel:fight.playerLevelAtBooking,rankingSnapshot:fight.rankingSnapshot,seedRankingSnapshot:fight.seedRankingSnapshot});
    const clock=String(fight.finishClock||'0:00').split(':').map(Number),finishSeconds=(fight.finishRound-1)*300+300-(clock[0]*60+clock[1]);
    const highlights=LOGIC.careerHighlights(state.personalBests,{won:win,forfeited:fight.forfeited,lowerLevel:lowerLevelWin,finish:fight.method.includes('KO')||fight.method==='SUBMISSION',finishSeconds,ratingGap:oppRating-playerRating,streak:state.winStreak});state.personalBests=highlights.records;
    ...
    fight.firstCareerWin=firstCareerWin;fight.firstWinUpgradeRequired=firstCareerWin&&attributePoint>0;
    pendingPostFightText=selectPostFightText({firstCareerWin,won:win,forfeited:!!fight.forfeited,lowerLevelWin,titleWon,titleFight:!!o.globalChampionship,winStreak:state.winStreak,opponent:o.name});
    state.rankingHistory=LOGIC.appendRankingResult(state.rankingHistory,fight.rankingSnapshot,fight.resultId,win?'win':'loss');
    ...
    if(calloutFight)lootNotes.push({kind:win?'milestone':'penalty',text:`CALLOUT ${win?'ANSWERED':'LOST'} · ${auraAppliedChange>0?'+':''}${auraAppliedChange} AURA`});else if(auraAppliedChange)lootNotes.push({kind:auraAppliedChange>0?'milestone':'penalty',text:`AURA · ${auraAppliedChange>0?'+':''}${auraAppliedChange}`});if(xp)gainXp(xp);xpResult.modifiers.forEach(text=>lootNotes.push({kind:'milestone',text}));if(win&&xpTier.tier==='exhausted'&&!calloutFight)lootNotes.push({kind:sameTier?'milestone':'penalty',text:sameTier?'XP USED TODAY · NO VICTORY PACK PROGRESS':`STALE MATCHUP · ${auraAppliedChange} AURA · NO VICTORY PACK PROGRESS`});const auraTitleAfter=LOGIC.auraTitle(state.aura);if(auraTitleAfter.key!==auraTitleBefore.key&&state.aura>auraBefore)lootNotes.push({kind:'milestone',text:`AURA STATUS · ${auraTitleAfter.label}`});
    ...
    if(consumeFeedChallenge(fight.feedChallengePostId,win?`@${state.name}`:`@${o.networkHandle||o.name}`))trackEvent('feed_challenge_completed',{opponent_id:o.sourceProfileId||'',result:win?'win':'loss',forfeited:!!fight.forfeited,aura_change:state.aura-auraBefore});
    openSocialCycle('fight',{win,opponent:o.name,method:fightMethodLabel(fight),startingHealthTier:fight.startingHealthTier,winStreak:state.winStreak,title:'',lowerLevelWin:backlashWin&&!calloutFight,followersLost:Math.abs(fans),fighterLevel:state.level,opponentLevel:o.min});
    renderResultBonuses(lootNotes,{auraInRewards:!win});renderDailyHeatResult({won:win,lowerLevelWin,rankRepeatExhausted:false,forfeited:!!fight.forfeited,dailyStreakBefore,dailyBonusAwarded});
    const xpResultLabel=fight.forfeited?'FORFEIT · NO XP':xpTier.resultLabel,planAssessment=fight.planAssessment||assessFightPlan(fight),auraChange=state.aura-auraBefore;trackEvent('fight_completed',{result:win?'win':'loss',fight_mode:'planned',plan_pace:fight.gamePlan.pace,plan_offense:fight.gamePlan.offense,plan_tactics:fight.gamePlan.tactics,plan_grade:planAssessment.grade.toLowerCase(),plan_modifier:Number(planAssessment.modifier.toFixed(3)),health_lost:healthLoss,method:String(fight.method).toLowerCase().replace(/\s+/g,'_'),finish_round:fight.finishRound,rounds_fought:fight.rounds.length,player_archetype:state.fighterStyle,opponent_key:o.key,opponent_archetype:o.tendency,is_rematch:isRematch,is_title:!!o.globalChampionship,is_callout:calloutFight,rookie_showcase:!!o.rookieShowcase,first_contract:!!o.firstContract,title_won:titleWon,upset,rivalry,attribute_points_earned:attributePoint,followers_change:fans,aura_change:auraChange,xp_earned:xp,xp_category:xpResult.category,xp_repeat_tier:xpTier.tier,gear_rarity:gearDrop?.rarity?.toLowerCase()||'none'});if(o.globalChampionship&&fight.championshipBout)state.pendingChampionshipResult={challengeId:fight.championshipBout.challenge_id,challengerId:fight.championshipBout.challenger_id,challengerWon:fight.championshipBout.player_is_champion===true?!win:win,mode:fight.championshipBout.player_is_champion===true?'defense':o.titleMode==='rematch'?'rematch':'challenge'};state.pendingFight=null;pendingResultDrop=gearDrop;resultDropRevealed=false;buildResultDetails(fight);renderChampionshipSettlement(o.globalChampionship?'pending':'',o.globalChampionship?'CONFIRMING TITLE RESULT':'');prepareRewardMetric('#rewardPrimary',win?attributePoint:auraChange,win||auraChange>0);$('#rewardPrimaryLabel').textContent=win?(attributePoint===1?'ATTRIBUTE POINT':'ATTRIBUTE POINTS'):'AURA';prepareRewardMetric('#rewardFans',fans,fans>=0);$('#rewardFansLabel').textContent=fans<0?'FOLLOWERS LOST':'FOLLOWERS';prepareRewardMetric('#rewardXp',xp,true);styleResultMetric('#rewardPrimary',win?attributePoint:auraChange);styleResultMetric('#rewardFans',fans);styleResultMetric('#rewardXp',xp);const rewardXpLabel=$('#rewardXpLabel');rewardXpLabel.textContent='XP';rewardXpLabel.title=xpResultLabel;rewardXpLabel.setAttribute('aria-label',`XP · ${xpResultLabel}`);lastFightShareData=win?{opponent:o.networkHandle||o.name,method:fight.method,round:fight.finishRound,record:`${state.wins}-${state.losses}`,winStreak:state.winStreak,titleWon}:null;$('#shareWinBtn').hidden=!win;renderPostFightTutorial(win);renderAttributeAssignment();armResultAction(fight.forfeited?'CONTINUE':'CLAIM REWARDS');showResultStage('outcome');const lootBox=$('#lootBox');lootBox.style.display='none';lootBox.className='loot';lootBox.innerHTML='';$('#resultDetails').classList.remove('open');const detailsToggle=$('#detailsToggle');detailsToggle.style.display='';detailsToggle.textContent='VIEW SCORECARD';const card=$('#resultModal .result-card');card.classList.remove('revealing','drop-celebration','fight-win','fight-loss');card.classList.add(win?'fight-win':'fight-loss');void card.offsetWidth;card.classList.add('revealing');card.scrollTop=0;writeHistory('result','replace');saveState();if(o.globalChampionship)settleChampionshipResult();scheduleFight(()=>{requestAnimationFrame(()=>$('#resultContinueBtn').focus())},180);
    queueMicrotask(()=>connectSharedSocial(true));
    if(lastFightShareData)lastFightShareData.method=fightMethodLabel(fight);
    if(win){const victoryLoot=$('#lootBox');victoryLoot.style.display='block';victoryLoot.className='loot victory-pack-progress';victoryLoot.innerHTML=victoryPackResultHtml({earned:!!gearDrop,eligible:victoryPackEligible,steps:victoryPackProgressSteps,lowerLevel:lowerLevelWin,repeatEligible:dropEligible,titleWon,firstCareerWin})}
  }

  function firstWinUpgradePending(){return fight?.firstCareerWin===true&&fight.firstWinUpgradeRequired===true&&state.attributePoints>0}
  function renderFirstWinUpgradeGate(){
    const pending=firstWinUpgradePending(),button=$('#continueBtn'),hint=$('#firstWinUpgradeHint');
    $('#resultRewardsStage').classList.toggle('first-win-upgrade-pending',pending);
    button.disabled=button.getAttribute('aria-busy')==='true'||pending;
    hint.hidden=!pending&&!fight?.firstWinUpgradeAttribute;
    hint.textContent=pending?'Choose an attribute to continue':fight?.firstWinUpgradeAttribute?`+1 ${fight.firstWinUpgradeAttribute.toUpperCase()} added`:'';
  }
  function armResultAction(label){
    const button=$('#continueBtn');button.textContent=label;renderFirstWinUpgradeGate();
  }
  ...
  function selectPostFightText({firstCareerWin=false,won=false,forfeited=false,lowerLevelWin=false,titleWon=false,titleFight=false,winStreak=0,opponent=''}={}){
    const config=STRINGS.postFightTexts,contacts=Array.isArray(config?.contacts)?config.contacts:[],firstWin=firstCareerWin&&won&&!forfeited;if(forfeited||(lowerLevelWin&&!firstWin)||!contacts.length)return null;if(firstWin){const contact=contacts.find(item=>item.id==='mom')||contacts.find(item=>item.id==='tommy');if(contact){lastPostFightTextContactId=contact.id;return {contact,messages:STRINGS.postFightTexts.firstWin.map(message=>copyText(message,{name:state.name}))}}}const notable=titleWon||titleFight||(won&&winStreak>0&&winStreak%5===0);if(!notable&&Math.random()>clamp(Number(config.chance)||.32,0,1))return null;let eligible=contacts.filter(contact=>contact.id!==lastPostFightTextContactId);if(!eligible.length)eligible=contacts;const contact=eligible[rint(0,eligible.length-1)],threadKey=titleWon?'titleWin':won?'win':'loss',threads=contact.messages||contact[threadKey]||contact[won?'win':'loss']||[],thread=threads[rint(0,Math.max(0,threads.length-1))]||[];if(!thread.length)return null;lastPostFightTextContactId=contact.id;return {contact,messages:thread.map(message=>copyText(message,{name:state.name,opponent,winStreak}))};
  }
  function showPendingPostFightText(){
  ...
  function revealDropClaim(){
    ...
    const drop=Object.assign({},normalized,{item});$('#dropClaimStage').innerHTML=collectibleCardHtml(item,{dropStatus:drop.autoEquipped?'EQUIPPED · '+globalThis.CAGE_CAREER_FEEDBACK.perkLabel(item):'NEW · '+globalThis.CAGE_CAREER_FEEDBACK.perkLabel(item)});$('#dropRevealBtn').hidden=true;$('#dropCloseBtn').hidden=false;resultDropRevealed=true;$('#dropCloseBtn').focus();
    trackEvent('gear_drop_revealed',{gear_id:item.id,gear_rarity:drop.rarity.toLowerCase(),is_new:drop.isNew,auto_equipped:drop.autoEquipped,drop_reason:drop.reason.toLowerCase().replace(/\s+/g,'_')});
  ...
  function closeResult(){
    if(firstWinUpgradePending()){renderFirstWinUpgradeGate();$('[data-first-win-assignment] [data-assign-attribute]')?.focus();return}
    const victoryDrop=pendingResultDrop;if(!state.postFightTutorialSeen){state.postFightTutorialSeen=true;saveState()}stopConfetti();clearFightTimers();$('#resultModal').style.display='none';$('#fightOverlay').classList.remove('active');fight=null;pendingResultDrop=victoryDrop;resultDropRevealed=false;combatLocked=false;updateUI();navTo('fight','replace');if(victoryDrop)openDropClaim(victoryDrop,{kind:'victory',eyebrow:'VICTORY PACK EARNED',title:'VICTORY PACK',message:'A surprise collectible pack landed after your win.'});else requestAnimationFrame(showPostFightFollowup);
  }
  function closeDropClaim(){
    const context=pendingDropContext||{};$('#dropClaimModal').classList.remove('open');$('#dropClaimModal').setAttribute('aria-hidden','true');pendingResultDrop=null;pendingDropContext=null;resultDropRevealed=false;stopConfetti();if(context.kind==='daily')queueMicrotask(maybeGrantInstallReward);if(context.kind==='victory'||context.kind==='referral')requestAnimationFrame(showPostFightFollowup);if(context.kind==='referral')queueMicrotask(syncReferralRewards);
  }
  ...
  document.addEventListener('keydown',e=>{if(e.target.matches('[data-gear-filter]')&&['ArrowLeft','ArrowRight','Home','End'].includes(e.key)){e.preventDefault();moveGearFilter(e.key)}});
  document.addEventListener('click',e=>{
    ...
    const sparTarget=e.target.closest('[data-spar-target]');if(sparTarget){selectSparTarget(sparTarget.dataset.sparTarget);return}
    const sparSetting=e.target.closest('[data-spar-setting]');if(sparSetting){selectSparSetting(sparSetting.dataset.sparSetting,sparSetting.dataset.sparValue);return}
    ...
    const loadoutCategory=e.target.closest('[data-loadout-category]');if(loadoutCategory){setGearFilter(loadoutCategory.dataset.loadoutCategory);return}
    const gearFilterButton=e.target.closest('[data-gear-filter]');if(gearFilterButton){setGearFilter(gearFilterButton.dataset.gearFilter);return}
    ...
    if(e.target.closest('[data-own-fighter-profile]')){openTaleOfTape({key:'own-profile',selfProfile:true,network:true,statsReady:true,sourceProfileId:state.socialProfileId,networkHandle:state.name,networkCity:state.fighterCity,networkPortrait:currentAvatar()?.asset,name:state.name,tier:state.level,min:state.level,tag:currentStyle()?.name,tendency:state.fighterStyle,wins:state.wins,losses:state.losses,fans:0,fightSkinAura:effectiveAura(),...Object.fromEntries(['power','speed','chin','cardio'].map(key=>[key,effectiveStat(key)]))});return}
  ...
  $('#resultScorecardClose').addEventListener('click',()=>{
    showResultStage('outcome');
    $('#detailsToggle').focus();
  });
  $('#landingEnterBtn').addEventListener('click',enterGameFromLanding);
  $('#fighterBuildCity').addEventListener('change',event=>chooseCity(event.target.value));
  window.addEventListener('online',()=>{if(identityRetryable&&!identityPending&&!state.nameLocked)lockFighterIdentity()});
  ...
  $('#opponentProfileModal').addEventListener('click',event=>{if(event.target===$('#opponentProfileModal'))closeFightPreview()});
  $('#opponentProfileModal').addEventListener('keydown',event=>{if(event.key==='Escape'){event.preventDefault();closeFightPreview()}if(event.key==='Tab'){const buttons=[$('#opponentProfileBack'),$('#opponentProfileFight')].filter(button=>!button.disabled);if(!buttons.length){event.preventDefault();return}const first=buttons[0],last=buttons[buttons.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}}});
  $('#opponentProfileBack').addEventListener('click',closeFightPreview);$('#opponentProfileFight').addEventListener('click',()=>commitFight());$('#tapeFightBtn').addEventListener('click',()=>{if(combatLocked)beginFightPlan()});$('#fightPlanConfirm').addEventListener('click',confirmFightPlan);$('#tapeStatsToggle').addEventListener('click',openTapeStats);$('#tapeStatsClose').addEventListener('click',()=>closeTapeStats());$('#tapeStatsBackdrop').addEventListener('click',()=>closeTapeStats());$('#tapeStatsPanel').addEventListener('keydown',e=>{if(e.key==='Escape')closeTapeStats()});$('#tapeTermsToggle').addEventListener('click',openTapeBreakdown);$('#tapeBreakdownClose').addEventListener('click',()=>closeTapeBreakdown());$('#tapeBreakdownBackdrop').addEventListener('click',()=>closeTapeBreakdown());$('#tapeBreakdown').addEventListener('keydown',e=>{if(e.key==='Escape')closeTapeBreakdown()});
  $('#detailsToggle').addEventListener('click',()=>{resetRewardAnimations();$('#resultRewardsStage').hidden=true;$('#resultDetails').classList.add('open');$('#resultScorecardClose').hidden=false;$('#resultModal .result-card').classList.remove('showing-rewards');$('#resultModal .result-card').setAttribute('aria-labelledby','resultScorecardToggle');$('#resultModal .result-card').scrollTop=0;$('#resultModal').style.display='flex';$('#detailsToggle').setAttribute('aria-expanded','true');$('#resultScorecardClose').focus()});$('#resultScorecardToggle').addEventListener('click',()=>toggleResultSection('resultScorecardSection'));$('#resultTapeToggle').addEventListener('click',()=>toggleResultSection('resultTapeSection'));$('#shareWinBtn').addEventListener('click',shareFightWin);
  ...
  $('#postFightMessageClose').addEventListener('click',closePostFightText);$('#postFightMessageModal').addEventListener('keydown',event=>{if(event.key==='Escape')closePostFightText()});
  $('#randomFighterAvatarBtn').addEventListener('click',randomFighterAvatar);$('#shuffleFighterAttributesBtn').addEventListener('click',shuffleFighterAttributes);$('#lockFighterBuildBtn').addEventListener('click',lockFighterBuild);
  ...
  $('#opponentList').addEventListener('scroll',maybeLoadMoreFightRankings,{passive:true});
  $('#fighterPostSearch').addEventListener('input',renderFighterPostResults);$('#fighterPostRedraft').addEventListener('click',()=>{fighterPostDraftOffset++;renderFighterPostPreview();sfx.tap()});$('#fighterPostSend').addEventListener('click',()=>handleFighterInteraction(currentFighterPostDraft(),fighterPostTarget));$('#fighterPostClose').addEventListener('click',cancelFighterPostComposer);$('#fighterPostModal').addEventListener('click',e=>{if(e.target===$('#fighterPostModal'))closeFighterPostComposer()});$('#fighterPostModal').addEventListener('keydown',e=>{if(e.key==='Escape')cancelFighterPostComposer()});
  ...
  $('#sparFighterPicker').addEventListener('click',openSparScout);
  $('#sparScoutSearch').addEventListener('input',renderOpenGym);
  $('#sparScoutClose').addEventListener('click',closeSparScout);
  $('#sparScoutModal').addEventListener('click',e=>{if(e.target===$('#sparScoutModal'))closeSparScout()});
  $('#sparScoutModal').addEventListener('keydown',e=>{
    if(e.key==='Escape'){e.preventDefault();closeSparScout();return}
    if(e.key!=='Tab')return;
    const buttons=[...$('#sparScoutModal').querySelectorAll('input:not(:disabled),button:not(:disabled)')],first=buttons[0],last=buttons[buttons.length-1];
  ...
  setInterval(updateRematchClocks,1000);

  document.addEventListener('visibilitychange',()=>{if(!document.hidden)updatePassiveRecovery(true)});
  window.addEventListener('pageshow',()=>updatePassiveRecovery(true));
  ...
  updateUI();
  renderLanding();
  const resumedBuild=landingFeature.resumeBuild();
  if(!resumedBuild)observeLandingFeatures();
  writeHistory('screen','replace');
  if(state.nameLocked)connectSharedSocial(true);
  const initialLandingMode=landingFeature.status().mode;trackEvent('game_open',{returning_career:initialLandingMode==='returning',setup_complete:initialLandingMode==='returning'});if(!resumedBuild)trackEvent('landing_view',{career_state:initialLandingMode});
})();
